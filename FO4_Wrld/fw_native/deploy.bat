...
REM Mirrors build.bat's flavour flag: `deploy.bat --minimal` ships the
REM FW_MINIMAL DLL out of build-minimal\ instead of build\.
set "OUTDIR=build"
if /i "%~1"=="--minimal" (
    set "OUTDIR=build-minimal"
) else if not "%~1"=="" (
    echo [deploy] ERROR: unknown argument "%~1"  ^(expected --minimal or nothing^)
    exit /b 1
)

set "DLL=%~dp0%OUTDIR%\dxgi.dll"
if not exist "%DLL%" (
    echo [deploy] ERROR: %DLL% not found. Run build.bat first.
    exit /b 1
)

REM Prefer the machine's real install; fall back to the default Steam path.
set "STEAM_DIR=C:\Games\Steam\steamapps\common\Fallout 4"
if not exist "%STEAM_DIR%" set "STEAM_DIR=C:\Program Files (x86)\Steam\steamapps\common\Fallout 4"
