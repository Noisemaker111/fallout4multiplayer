...
// B4 QuestStage — Papyrus Quest.SetCurrentStageID.
// Signature: char(VM*, uint32_t stack_id, TESQuest*, uint32_t stage).
// MAIN THREAD ONLY (TLS-affine stage workers).
using QuestSetStageFn = char (*)(void* vm, std::uint32_t stack_id,
                                 void* quest, std::uint32_t stage);
QuestSetStageFn      g_quest_set_stage    = nullptr;

// B6.7 — Actor.SetFactionRank engine worker.
// Signature: void(Actor* rcx, TESFaction* rdx, int rank r8).
using ActorSetFactionRankWorkerFn = void (*)(
    void* actor, void* faction, int rank);
ActorSetFactionRankWorkerFn g_set_faction_rank_worker = nullptr;

// A4 — Game.RewardPlayerXP Papyrus native (1.10.163 @ 0x13C1EA0).
// Signature: void(VM*, uint32 stack_id, void* static_tag, int amount, bool suppress)
using RewardPlayerXPFn = void (*)(
    void* vm, std::uint32_t stack_id, void* static_tag,
    int amount, bool suppress);
RewardPlayerXPFn g_reward_player_xp = nullptr;

// B6.11 — Sky* getter + weather SetActive worker.
using SkyGetterFn = void* (*)();
using WeatherSetActiveWorkerFn = void (*)(
    void* sky, void* weather, bool ab_override, bool ab_accelerate);
SkyGetterFn              g_sky_getter = nullptr;
WeatherSetActiveWorkerFn g_weather_set_active = nullptr;

// B6.8 — SetPlayerTeammate worker.
// Signature from prolog: (Actor* rcx, bool teammate dl, bool canFavor r8, bool r9)
using SetPlayerTeammateWorkerFn = void (*)(
    void* actor, bool teammate, bool can_do_favor, bool a4);
SetPlayerTeammateWorkerFn g_set_player_teammate = nullptr;

// B6.9 — Cell.SetCleared worker: void/char(TESObjectCELL* rcx, bool cleared dl)
using CellSetClearedWorkerFn = void (*)(void* cell, bool cleared);
CellSetClearedWorkerFn g_cell_set_cleared = nullptr;

// v23: PassTime worker (after papyrus thunk). (void* a1, int hours, char flag?)
using PassTimeWorkerFn = void (*)(void* a1, int hours, char flag);
PassTimeWorkerFn g_pass_time_worker = nullptr;

// SetRelationshipRank papyrus for apply (call under ApplyingRemoteGuard).
using SetRelationshipRankFn = void (*)(
    void* vm, std::uint32_t stack_id, void* actor_a, void* actor_b, int rank);
SetRelationshipRankFn g_set_relationship_rank = nullptr;
...
// sub_141B0EE10 — engine NiHeap allocator. Used at the canonical vanilla
...
bool init(std::uintptr_t module_base) {
    ...
    // B4 QuestStage: Papyrus SetCurrentStageID for receive-side apply.
    g_quest_set_stage = reinterpret_cast<QuestSetStageFn>(
        module_base + offsets::PAPYRUS_QUEST_SETSTAGE_RVA);
    FW_LOG("engine: quest_set_stage resolved at %p (RVA 0x%lX)",
           reinterpret_cast<void*>(g_quest_set_stage),
           static_cast<unsigned long>(offsets::PAPYRUS_QUEST_SETSTAGE_RVA));

    // B6.7: Actor.SetFactionRank worker for PC faction apply.
    g_set_faction_rank_worker = reinterpret_cast<ActorSetFactionRankWorkerFn>(
        module_base + offsets::ACTOR_SETFACTIONRANK_WORKER_RVA);
    FW_LOG("engine: set_faction_rank_worker resolved at %p (RVA 0x%lX)",
           reinterpret_cast<void*>(g_set_faction_rank_worker),
           static_cast<unsigned long>(offsets::ACTOR_SETFACTIONRANK_WORKER_RVA));

    // A4: Game.RewardPlayerXP for proximity XP apply.
    g_reward_player_xp = reinterpret_cast<RewardPlayerXPFn>(
        module_base + offsets::PAPYRUS_REWARD_PLAYER_XP_RVA);
    FW_LOG("engine: reward_player_xp resolved at %p (RVA 0x%lX)",
           reinterpret_cast<void*>(g_reward_player_xp),
           static_cast<unsigned long>(offsets::PAPYRUS_REWARD_PLAYER_XP_RVA));

    // B6.11: Sky getter + weather SetActive worker.
    g_sky_getter = reinterpret_cast<SkyGetterFn>(
        module_base + offsets::SKY_GETTER_RVA);
    g_weather_set_active = reinterpret_cast<WeatherSetActiveWorkerFn>(
        module_base + offsets::WEATHER_SETACTIVE_WORKER_RVA);
    FW_LOG("engine: sky_getter=%p weather_set_active=%p",
           reinterpret_cast<void*>(g_sky_getter),
           reinterpret_cast<void*>(g_weather_set_active));

    // B6.8: SetPlayerTeammate worker.
    g_set_player_teammate = reinterpret_cast<SetPlayerTeammateWorkerFn>(
        module_base + offsets::ACTOR_SET_PLAYER_TEAMMATE_WORKER_RVA);
    FW_LOG("engine: set_player_teammate=%p",
           reinterpret_cast<void*>(g_set_player_teammate));

    // B6.9: Cell.SetCleared worker.
    g_cell_set_cleared = reinterpret_cast<CellSetClearedWorkerFn>(
        module_base + offsets::CELL_SETCLEARED_WORKER_RVA);
    FW_LOG("engine: cell_set_cleared=%p",
           reinterpret_cast<void*>(g_cell_set_cleared));

    // v23: PassTime + relationship.
    g_pass_time_worker = reinterpret_cast<PassTimeWorkerFn>(
        module_base + 0x00D5E480);  // PassTime worker (from 0x13C1B70 jmp)
    g_set_relationship_rank = reinterpret_cast<SetRelationshipRankFn>(
        module_base + offsets::PAPYRUS_SET_RELATIONSHIP_RANK_RVA);
    FW_LOG("engine: pass_time=%p set_relationship=%p",
           reinterpret_cast<void*>(g_pass_time_worker),
           reinterpret_cast<void*>(g_set_relationship_rank));
    ...
    // FW_MINIMAL: the INT variant has no callers at all — only the float and
    // bool setters above are used. Dead in a full build too, but harmless
    // there; here it would pin an address for nothing.
#if !FW_MINIMAL
    ...
    // ========================================================================
    // FW_MINIMAL: the spawn/proxy allocation cluster. Every pointer between
    // here and the marker below is read ONLY by spawn_ghost_player /
    // spawn_ghost_proxy, both of which are compiled out (see the block comment
    // at their definitions). Resolving them would pin ~15 addresses that
    // nothing calls.
    //
    // Verified by reachability, not by eye: each pointer's readers were
    // enumerated across the whole tree with `#if !FW_MINIMAL` regions
    // stripped, and every one came back empty. Two near-misses are worth
    // recording, because they are the shape of mistake this guard could make:
    // `g_bsfs_make` and `g_set_position_engine` LOOK like part of this cluster
    // but are read by init() itself and by apply_ghost_pos respectively, so
    // they stay resolved below. A pointer left null here would not fail the
    // build — it would crash in-game at first call.
    // ========================================================================
#if !FW_MINIMAL
...
#endif
    // KEEP: apply_ghost_pos (net/client.cpp) drives the visible ghost through
    // this, so it is NOT part of the proxy cluster despite sitting in it.
    g_set_position_engine = reinterpret_cast<SetPositionEngineFn>(
        module_base + offsets::SETPOSITION_NIPOINT3_RVA); // 1.10.163 SetPosition
    g_set_rotation_engine = reinterpret_cast<SetRotationEngineFn>(
        module_base + 0x0513790);            // SetRotation — still NG TODO
#if !FW_MINIMAL
...
#endif
    // KEEP: read by synthesize_combat_extension_for_ghost_target, which is
    // still compiled — it is not part of the spawn cluster.
    ...
    //
    // 2026-08-02: under FW_MINIMAL, ghost spawn is compiled out — skip the
    // vtable clone so early boot does zero engine memory reads beyond
    // pointer arithmetic. Full builds still need it before first spawn.
#if !FW_MINIMAL
    init_duplicate_vtable(module_base);
#else
    FW_LOG("[ghost-native] duplicate vtable deferred (FW_MINIMAL — no spawn)");
#endif

    // Mint cached BSFixedStrings for the hot anim variables we drive in
    // w3.b.
    //
    // 2026-08-02 HARD STOP on early mint: live smoke on 1.10.163 always
    // SEH-faulted here (pool not ready / mid-boot). Calling into the
    // minter before the string pool is live can partially write heap
    // before the AV — that is a strong suspect for the ~2s post-boot
    // crash at game RVA 0x9E5CC0 (null +0x20). Mint is deferred to
    // ensure_bsfs_cache() which load_game_by_name / anim paths call once
    // the world is up. g_bs_* stay null → graph setters short-circuit.
    FW_LOG("engine: BSFixedString mint DEFERRED until post-LoadGame "
           "(early mint SEH-faulted on 1.10.163 smoke)");
...
// sub_14011B8C0 / etc. iterate the table and call vt[N] on EVERY entry.
...
// ============================================================================
// FW_MINIMAL: compiled out. This is the receiver-side "make raider X target
// peer Y" apply; it calls enter_combat_raider_vs_ghost and
// synthesize_combat_extension_for_ghost_target, both in the guarded combat
// tier further down. The compiler found this dependency — the tree-wide
// caller scan did not, because the calls are internal to this file.
#if !FW_MINIMAL
...
//
#endif // !FW_MINIMAL — apply_ghost_pos below is KEPT (drives the visible ghost)
...
bool load_game_by_name(const char* save_name) {
    ...
    // User-mode pointer heuristic for Win64 FO4:
    //   heap-ish  0x00000100_00000000 .. 0x00007FF0_00000000
    //   image-ish 0x00007FF0_00000000 .. 0x00007FFF_FFFFFFFF
    // Rejects sentinels like 0x4000000000 (live SAVE_DEV slot content)
    // and packed garbage like 0x000104017006320A (live SAVE_LOAD_MGR).
    auto plausible_ptr = [](const void* p) -> bool {
        const auto u = reinterpret_cast<std::uintptr_t>(p);
        if (u < 0x0000010000000000ULL) return false;
        if (u >= 0x0000800000000000ULL) return false;
        // Canonical user-mode only (bit 47 clear path already covered).
        return true;
    };

    bool ok = false;
    __try {
        // Snapshot once first (menu may still be initializing).
        void* save_dev = *g_save_dev_slot;
        void* mgr = *g_save_load_mgr_slot;
        FW_LOG("engine: LoadGame t0 save_dev=%p (ok=%d) mgr=%p (ok=%d) "
               "dev_slot=%p mgr_slot=%p",
               save_dev, plausible_ptr(save_dev) ? 1 : 0,
               mgr, plausible_ptr(mgr) ? 1 : 0,
               static_cast<void*>(g_save_dev_slot),
               static_cast<void*>(g_save_load_mgr_slot));
        // Dump slot neighbourhood for re-derive (first 4 qwords each).
        __try {
            const auto* ds =
                reinterpret_cast<const std::uint64_t*>(g_save_dev_slot);
            const auto* ms =
                reinterpret_cast<const std::uint64_t*>(g_save_load_mgr_slot);
            FW_LOG("engine: SAVE_DEV slot qwords: %016llX %016llX %016llX %016llX",
                   ds[0], ds[1], ds[2], ds[3]);
            FW_LOG("engine: SAVE_MGR slot qwords: %016llX %016llX %016llX %016llX",
                   ms[0], ms[1], ms[2], ms[3]);
        } __except (EXCEPTION_EXECUTE_HANDLER) {
            FW_WRN("engine: SEH dumping singleton slots");
        }
        // Live prologue of LoadGame — look for rip-relative LEA/MOV to
        // recover the true singleton when RVAs are stale.
        __try {
            const auto* code =
                reinterpret_cast<const std::uint8_t*>(g_load_game);
            FW_LOG("engine: LoadGame prologue: "
            ...
            FW_WRN("engine: SEH reading LoadGame prologue");
        }

        // Poll for real pointers. Sentinel 0x4000000000 was observed live
        // when SAVE_DEV slot was unreadied; wait up to ~6s.
        for (int i = 0; i < 60; ++i) {
            if (plausible_ptr(mgr) && plausible_ptr(save_dev))
                break;
            if (plausible_ptr(mgr) && i >= 20)  // 2s: try with mgr only
                break;
            Sleep(100);
            save_dev = *g_save_dev_slot;
            mgr = *g_save_load_mgr_slot;
            if ((i % 10) == 9) {
                FW_LOG("engine: LoadGame poll i=%d save_dev=%p mgr=%p",
                       i + 1, save_dev, mgr);
            }
        }
        FW_LOG("engine: LoadGame ready save_dev=%p (ok=%d) mgr=%p (ok=%d)",
               save_dev, plausible_ptr(save_dev) ? 1 : 0,
               mgr, plausible_ptr(mgr) ? 1 : 0);

        if (plausible_ptr(save_dev)) {
            std::uint8_t precond = 0;
            bool precond_ok = false;
            __try {
                precond = g_load_precond(save_dev, 0, 0);
                precond_ok = true;
            } __except (EXCEPTION_EXECUTE_HANDLER) {
                FW_WRN("engine: LoadGame precond SEH — continuing anyway");
            }
            if (precond_ok) {
                FW_LOG("engine: LoadGame precond returned %u", precond);
            }
        } else {
            FW_WRN("engine: save_dev not a plausible pointer — skip precond "
                   "(SAVE_DEV_SINGLETON_RVA may need re-derive on 1.10.163)");
        }

        if (!plausible_ptr(mgr)) {
            FW_ERR("engine: LoadGame aborted — save_load_manager %p not a "
                   "plausible pointer (SAVE_LOAD_MGR_SINGLETON_RVA?)",
                   mgr);
            return false;
        }

        // 1.10.163: LOAD_PREP_FN_RVA (0xC37140) and LOAD_IN_PROGRESS_FLAG
        // are still wrong — calling them AVs and BOOT-SKIP leaves the
        // process half-corrupted so LoadGame SEHs immediately after.
        // Live callers of LoadGame (sub_140CED1D0) only need a valid
        // BGSSaveLoadManager* in rcx + filename; prep/flag are optional
        // console-exec scaffolding. Skip them until re-derived.
        if (g_load_prep) {
            // Intentionally not invoked on 1.10.163 until RVA verified.
            FW_LOG("engine: LoadGame prep SKIPPED (1.10.163 soft-port)");
        }
        (void)g_load_in_progress;

        // Live "Continue" call site (0xCEE3B2) passes filename=nullptr.
        // Named-load call sites pass a char* save stem. Try named first;
        // if the engine returns 0 (not found / unreadable — common when
        // the exitsave was written with DLC masters we currently have
        // disabled), fall back to Continue (null name).
        auto invoke = [&](const char* name, const char* tag) -> std::uint8_t {
            FW_LOG("engine: LoadGame try [%s] name=%s mgr=%p",
                   tag, name ? name : "(null/Continue)", mgr);
            return g_load_game(
                mgr, name, /*unk_neg1=*/-1, /*flags=*/0,
                /*one=*/1, /*zero=*/0);
        };

        std::uint8_t rc = invoke(save_name, "named");
        if (rc == 0) {
            FW_WRN("engine: named LoadGame returned 0 for '%s' — "
                   "retrying Continue (null filename)", save_name);
            rc = invoke(nullptr, "continue");
        }

        if (rc == 0) {
            FW_ERR("engine: LoadGame returned 0 — savefile '%s' not found "
                   "or unreadable (named + Continue both failed). "
                   "If DLC was disabled, re-enable matching masters or "
                   "make a new base-game save.", save_name);
        } else {
            FW_LOG("engine: LoadGame accepted (rc=%u) — engine transitioning "
                   "to load screen for '%s'", rc, save_name);
            ok = true;
        }
    }
    __except (EXCEPTION_EXECUTE_HANDLER) {
...
bool apply_quest_stage(std::uint32_t quest_form_id, std::uint16_t stage) {
    if (!g_ready.load(std::memory_order_acquire)) return false;
    if (quest_form_id == 0) return false;
    if (!g_quest_set_stage) {
        FW_WRN("engine: apply_quest_stage: SetCurrentStageID not resolved");
        return false;
    }

    void* quest = lookup_by_form_id(quest_form_id);
    if (!quest) {
        // Quest form not in the local data handler yet (rare — quests are
        // permanent forms). Log and skip; bootstrap will not re-fire.
        FW_DBG("engine: apply_quest_stage lookup_null form=0x%X stage=%u",
               quest_form_id, static_cast<unsigned>(stage));
        return false;
    }

    bool ok = false;
    __try {
        // vm=nullptr, stack_id=0 — happy-path stage workers are TLS-bound
        // but do not require a live Papyrus stack frame. Observe hook is
        // silenced by caller's ApplyingRemoteGuard.
        g_quest_set_stage(/*vm=*/nullptr, /*stack_id=*/0, quest,
                          static_cast<std::uint32_t>(stage));
        ok = true;
    }
    __except (EXCEPTION_EXECUTE_HANDLER) {
        FW_ERR("engine: SEH in apply_quest_stage(0x%X, %u)",
               quest_form_id, static_cast<unsigned>(stage));
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: apply_quest_stage 0x%X -> stage %u",
               quest_form_id, static_cast<unsigned>(stage));
    }
    return ok;
}

bool apply_faction_rank(std::uint32_t faction_form_id, std::int32_t rank) {
    if (!g_ready.load(std::memory_order_acquire)) return false;
    if (faction_form_id == 0) return false;
    if (!g_set_faction_rank_worker) {
        FW_WRN("engine: apply_faction_rank: worker not resolved");
        return false;
    }

    void* player = get_local_player();
    if (!player) {
        FW_DBG("engine: apply_faction_rank: local player null");
        return false;
    }

    void* faction = lookup_by_form_id(faction_form_id);
    if (!faction) {
        FW_DBG("engine: apply_faction_rank lookup_null faction=0x%X",
               faction_form_id);
        return false;
    }
    ...
    bool ok = false;
    __try {
        g_set_faction_rank_worker(player, faction, static_cast<int>(rank));
        ok = true;
    }
    __except (EXCEPTION_EXECUTE_HANDLER) {
        FW_ERR("engine: SEH in apply_faction_rank(0x%X, %d)",
               faction_form_id, rank);
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: apply_faction_rank faction=0x%X rank=%d",
               faction_form_id, rank);
    }
    return ok;
}

bool reward_player_xp(std::int32_t amount, bool suppress_notifications) {
    if (!g_ready.load(std::memory_order_acquire)) return false;
    if (amount == 0) return true;
    if (!g_reward_player_xp) {
        FW_WRN("engine: reward_player_xp: native not resolved");
        return false;
    }
    bool ok = false;
    __try {
        g_reward_player_xp(/*vm=*/nullptr, /*stack_id=*/0, /*tag=*/nullptr,
                           static_cast<int>(amount),
                           suppress_notifications);
        ok = true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        FW_ERR("engine: SEH in reward_player_xp(amount=%d)", amount);
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: reward_player_xp amount=%d suppress=%d",
               amount, suppress_notifications ? 1 : 0);
    }
    return ok;
}

std::uint32_t get_current_weather_form_id() noexcept {
    if (!g_ready.load(std::memory_order_acquire) || !g_sky_getter) return 0;
    std::uint32_t form_id = 0;
    __try {
        void* sky = g_sky_getter();
        if (!sky) return 0;
        void* weather = *reinterpret_cast<void**>(
            reinterpret_cast<std::uint8_t*>(sky) +
            offsets::SKY_CURRENT_WEATHER_OFF);
        if (!weather) return 0;
...
bool apply_weather(std::uint32_t weather_form_id, bool ab_override,
                   bool ab_accelerate) {
    if (!g_ready.load(std::memory_order_acquire)) return false;
    if (weather_form_id == 0) return false;
    if (!g_sky_getter || !g_weather_set_active) {
        FW_WRN("engine: apply_weather: sky/worker not resolved");
        return false;
    }
    void* weather = lookup_by_form_id(weather_form_id);
    if (!weather) {
        FW_DBG("engine: apply_weather lookup_null form=0x%X", weather_form_id);
        return false;
    }
    bool ok = false;
    __try {
        void* sky = g_sky_getter();
        if (!sky) return false;
        g_weather_set_active(sky, weather, ab_override, ab_accelerate);
        ok = true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        FW_ERR("engine: SEH in apply_weather(0x%X)", weather_form_id);
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: apply_weather form=0x%X override=%d accel=%d",
               weather_form_id, ab_override ? 1 : 0, ab_accelerate ? 1 : 0);
    }
    return ok;
}

bool apply_player_teammate(std::uint32_t actor_form_id, bool teammate,
                           bool can_do_favor) {
    if (!g_ready.load(std::memory_order_acquire)) return false;
    if (actor_form_id == 0) return false;
    if (!g_set_player_teammate) {
        FW_WRN("engine: apply_player_teammate: worker not resolved");
        return false;
    }
    void* actor = lookup_by_form_id(actor_form_id);
    if (!actor) {
        FW_DBG("engine: apply_player_teammate lookup_null form=0x%X",
               actor_form_id);
        return false;
    }
    bool ok = false;
    __try {
        g_set_player_teammate(actor, teammate, can_do_favor, /*a4=*/false);
        ok = true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        FW_ERR("engine: SEH in apply_player_teammate(0x%X)", actor_form_id);
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: apply_player_teammate form=0x%X teammate=%d favor=%d",
               actor_form_id, teammate ? 1 : 0, can_do_favor ? 1 : 0);
        note_local_companion(actor_form_id, teammate);
    }
    return ok;
}

namespace {
std::mutex g_companion_roster_mtx;
std::unordered_set<std::uint32_t> g_local_companion_forms;
} // namespace

void note_local_companion(std::uint32_t actor_form_id, bool teammate) {
    if (actor_form_id == 0) return;
    std::lock_guard lk(g_companion_roster_mtx);
    if (teammate) {
        g_local_companion_forms.insert(actor_form_id);
    } else {
        g_local_companion_forms.erase(actor_form_id);
    }
}

void pull_local_companions_to_player(float pos_x, float pos_y, float pos_z,
                                     float yaw_rad) {
    if (!g_ready.load(std::memory_order_acquire)) return;

    std::uint32_t forms[16]{};
    std::size_t n = 0;
    {
        std::lock_guard lk(g_companion_roster_mtx);
        for (std::uint32_t fid : g_local_companion_forms) {
            if (n >= 16) break;
            forms[n++] = fid;
        }
    }
    ...
    // Fan companions slightly behind/side of the PC so they don't stack.
    constexpr float kRing = 64.f; // game units
    std::size_t pulled = 0;
    for (std::size_t i = 0; i < n; ++i) {
        void* actor = lookup_by_form_id(forms[i]);
        if (!actor) continue;
        // Simple offset: alternate left/right + back along -Y from yaw.
        const float side = (i & 1u) ? kRing : -kRing;
        const float back = -kRing * (1.f + 0.25f * static_cast<float>(i / 2));
        const float c = std::cos(yaw_rad);
        const float s = std::sin(yaw_rad);
        // Bethesda yaw: 0 = +Y. Forward=(sin, cos), right=(cos, -sin).
        const float ox = pos_x + s * back + c * side;
        const float oy = pos_y + c * back - s * side;
        const float oz = pos_z;
        // Cross-cell party travel needs process update (handoff path).
        if (actor_teleport_handoff(actor, ox, oy, oz, yaw_rad) ||
            actor_atomic_teleport(actor, ox, oy, oz, yaw_rad)) {
            ++pulled;
        }
    }
    if (pulled > 0) {
        FW_LOG("engine: pull_local_companions pulled=%zu of %zu to "
               "(%.1f,%.1f,%.1f)",
               pulled, n, pos_x, pos_y, pos_z);
    }
}

bool get_cell_cleared(std::uint32_t cell_form_id, bool* out_cleared) noexcept {
    if (!out_cleared) return false;
    *out_cleared = false;
    if (!g_ready.load(std::memory_order_acquire) || cell_form_id == 0) {
        return false;
    }
    void* cell = lookup_by_form_id(cell_form_id);
    if (!cell) return false;
    bool ok = false;
    __try {
        const auto flag = *reinterpret_cast<const std::uint8_t*>(
            reinterpret_cast<const std::uint8_t*>(cell) +
            offsets::TESOBJECTCELL_CLEARED_OFF);
        *out_cleared = (flag != 0);
        ok = true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        ok = false;
...
bool apply_cell_cleared(std::uint32_t cell_form_id, bool cleared) {
    if (!g_ready.load(std::memory_order_acquire) || cell_form_id == 0) {
        return false;
    }
    if (!g_cell_set_cleared) {
        FW_WRN("engine: apply_cell_cleared: worker not resolved");
        return false;
    }
    void* cell = lookup_by_form_id(cell_form_id);
    if (!cell) {
        FW_DBG("engine: apply_cell_cleared lookup_null cell=0x%X", cell_form_id);
        return false;
    }
    bool ok = false;
    __try {
        g_cell_set_cleared(cell, cleared);
        ok = true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        FW_ERR("engine: SEH in apply_cell_cleared(0x%X)", cell_form_id);
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: apply_cell_cleared cell=0x%X cleared=%d",
               cell_form_id, cleared ? 1 : 0);
    }
    return ok;
}

bool apply_relationship_rank(std::uint32_t actor_a_form_id,
                             std::uint32_t actor_b_form_id,
                             std::int32_t rank) {
    if (!g_ready.load(std::memory_order_acquire)) return false;
    if (!g_set_relationship_rank) return false;
    void* a = lookup_by_form_id(actor_a_form_id);
    void* b = lookup_by_form_id(actor_b_form_id);
    if (!a || !b) {
        FW_DBG("engine: apply_relationship lookup miss a=0x%X b=0x%X",
               actor_a_form_id, actor_b_form_id);
        return false;
    }
    bool ok = false;
    __try {
        g_set_relationship_rank(nullptr, 0, a, b, static_cast<int>(rank));
        ok = true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        FW_ERR("engine: SEH apply_relationship 0x%X->0x%X",
               actor_a_form_id, actor_b_form_id);
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: apply_relationship 0x%X->0x%X rank=%d",
               actor_a_form_id, actor_b_form_id, rank);
    }
    return ok;
}

bool apply_pass_time(std::int32_t hours) {
    if (!g_ready.load(std::memory_order_acquire)) return false;
    if (hours == 0) return true;
    // Prefer GameHour bump if PassTime worker is awkward (needs calendar ctx).
    // Strategy: read GameHour, add hours mod 24, write via apply_global_var;
    // also bump GameDaysPassed by hours/24.
    const std::uint32_t gh = offsets::GLOBAL_FORM_GAME_HOUR;
    const std::uint32_t gd = offsets::GLOBAL_FORM_GAME_DAYS_PASSED;
    void* gh_form = lookup_by_form_id(gh);
    void* gd_form = lookup_by_form_id(gd);
    if (!gh_form) {
        FW_WRN("engine: apply_pass_time: GameHour form missing");
        return false;
    }
    bool ok = false;
    __try {
        auto* ghb = reinterpret_cast<std::uint8_t*>(gh_form);
        float hour = *reinterpret_cast<float*>(
            ghb + offsets::TESGLOBAL_VALUE_OFF);
        double total = static_cast<double>(hour) + static_cast<double>(hours);
        double days_add = 0.0;
        while (total >= 24.0) { total -= 24.0; days_add += 1.0; }
        while (total < 0.0) { total += 24.0; days_add -= 1.0; }
        *reinterpret_cast<float*>(ghb + offsets::TESGLOBAL_VALUE_OFF) =
            static_cast<float>(total);
        if (gd_form && days_add != 0.0) {
            auto* gdb = reinterpret_cast<std::uint8_t*>(gd_form);
            float days = *reinterpret_cast<float*>(
                gdb + offsets::TESGLOBAL_VALUE_OFF);
            *reinterpret_cast<float*>(gdb + offsets::TESGLOBAL_VALUE_OFF) =
                days + static_cast<float>(days_add);
        }
        ok = true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        FW_ERR("engine: SEH apply_pass_time hours=%d", hours);
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: apply_pass_time hours=%d (GameHour/DaysPassed)",
               hours);
    }
    return ok;
}

bool apply_player_item_grant(std::uint32_t item_form_id, std::int32_t count,
                             bool silent) {
    if (!g_ready.load(std::memory_order_acquire)) return false;
    if (item_form_id == 0 || item_form_id == offsets::FORM_CAPS_SEPTIMS) {
        return false;
    }
    if (count <= 0 || count > offsets::ITEM_GRANT_MAX_COUNT) return false;
    if (!g_engine_add_item) {
        FW_WRN("engine: apply_player_item_grant: AddItem not resolved");
        return false;
    }

    void* pc = get_local_player();
    if (!pc) {
        FW_DBG("engine: apply_player_item_grant: no local PC");
        return false;
    }
    void* item_form = lookup_by_form_id(item_form_id);
    if (!item_form) {
        FW_DBG("engine: apply_player_item_grant lookup miss item=0x%X",
               item_form_id);
        return false;
    }

    // Dedup unique single grants if the player already holds the item —
    // covers the case where local SetStage already ran the reward script.
    if (count == 1) {
        std::uint32_t ids[64]{};
        std::int32_t  counts[64]{};
        const std::size_t n =
            scan_container_inventory(pc, ids, counts, 64);
        for (std::size_t i = 0; i < n; ++i) {
            if (ids[i] == item_form_id && counts[i] >= 1) {
                FW_DBG("engine: apply_player_item_grant skip dedup item=0x%X "
                       "have=%d", item_form_id, counts[i]);
                return true;
            }
        }
    }

    (void)force_materialize_inventory(pc);
    ...
    __try {
        ...
        FW_ERR("engine: SEH apply_player_item_grant item=0x%X count=%d",
               item_form_id, count);
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: apply_player_item_grant item=0x%X count=%d silent=%d",
...
bool player_teleport_to(float pos_x, float pos_y, float pos_z, float yaw_rad) {
    if (!g_actor_atomic_teleport) return false;
    if (!g_ready.load(std::memory_order_acquire)) return false;
    void* player = get_local_player();
    if (!player) {
        FW_DBG("engine: player_teleport_to: local player null");
        return false;
    }
    bool ok = false;
    __try {
        float pos[3] = { pos_x, pos_y, pos_z };
        // Player path: skip_player_check=0 so camera/cell/control branches run.
        // doProcessUpdate=1 for cross-cell party travel. skip_area_check=1 is
        // best-effort when the destination cell is not streamed yet.
        g_actor_atomic_teleport(
            player, pos, yaw_rad, 0.0f /*pitch*/,
            nullptr /*target_cell*/, nullptr /*target_marker*/,
            /*do_process_update=*/static_cast<char>(1),
            /*skip_player_check=*/static_cast<char>(0),
            /*skip_area_check=*/static_cast<char>(1));
        ok = true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        FW_ERR("engine: SEH in player_teleport_to (%.1f,%.1f,%.1f)",
               pos_x, pos_y, pos_z);
        ok = false;
    }
    if (ok) {
        FW_LOG("engine: player_teleport_to (%.1f, %.1f, %.1f) yaw=%.2f",
               pos_x, pos_y, pos_z, yaw_rad);
    }
    return ok;
}
...
//
// ============================================================================
// FW_MINIMAL: start of the puppet-fire / combat tier. Everything from here to
// the matching #endif (just before is_actor_sneaking) is reachable only from
// the NPC fire and perception drains in main_thread_dispatch.cpp, which are
// themselves compiled out, or from hooks in minimal_exclude.txt. Verified by
// stripping guarded regions tree-wide and re-checking for call sites.
//
// This is the block START_HERE calls "the combat tier": weapon fire, aim
// controller resolution, EnterCombat, combat-extension synthesis, perception
// triggering. None of it is needed to see another player move.
// ============================================================================
#if !FW_MINIMAL
...
// sub_140CA6220).
// ============================================================================
#endif // !FW_MINIMAL — end of combat tier; posture readers below are KEPT
       // (player_pos_hook needs crouch/sprint state for pose replication).
...
// FW_MINIMAL: combat tier resumes here, through disable_actor_movement. Same
// reachability argument as the block above.
#if !FW_MINIMAL
...
#endif // !FW_MINIMAL — the yaw setter below is KEPT.
       //
       // It sits in the middle of the combat tier for historical reasons, but
       // it is a plain "write Actor+0xC0 rotation" helper: the NPC owner-state
       // drain in main_thread_dispatch.cpp calls it alongside apply_npc_pos to
       // face a replicated NPC. Position sync, not combat.
...
//
#endif // !FW_MINIMAL — end of combat tier; apply_npc_pos below is KEPT.
...
//
// ============================================================================
// FW_MINIMAL: both spawn paths below are compiled out, and they are the single
// densest block of version-pinned addresses left in this file (~35 between
// them). Neither runs today:
//   - spawn_ghost_player is DEPRECATED (see the note at the end of its body);
//     nothing calls it.
//   - spawn_ghost_proxy is reachable only from actor_hijack.cpp, which
//     early-returns on `g_proxy_spawn_disabled{true}` — an unconditional
//     constant. START_HERE section 6 records the proxy Actor as dead code and
//     says not to port its addresses.
// The visible ghost is the injected NiNode from scene_inject.cpp, which is
