...
// B4 QuestStage: Apply a Quest.SetCurrentStageID received from another peer
// (or from QUEST_STATE_BOOT on join).
//
// Calls the Papyrus SetCurrentStageID native so stage fragments / aliases
// run on this client the same way a local SetStage would. MAIN THREAD ONLY
// — stage workers read TEB TLS. Callers must route through
// fw::dispatch::enqueue_quest_stage_apply (WndProc drain).
//
// Pass vm=nullptr / stack_id=0: happy-path stage workers do not need a live
// Papyrus stack; error-report paths that touch VM are SEH-caged.
//
// Caller must hold ApplyingRemoteGuard so the observe hook does not echo
// QUEST_STAGE_SET back to the server.
//
// Returns true if the native was invoked without SEH and the quest form
// resolved; false on lookup failure / not ready / SEH.
bool apply_quest_stage(std::uint32_t quest_form_id, std::uint16_t stage);

// B6.7 / Tier B1: Apply Actor.SetFactionRank on the local PlayerCharacter.
//
// Calls the engine worker at ACTOR_SETFACTIONRANK_WORKER_RVA
// (Actor*, TESFaction*, int rank). MAIN THREAD preferred (faction list
// mutates actor extra data). Caller must hold ApplyingRemoteGuard.
//
// Returns true if player + faction resolved and the call did not SEH.
...
// Tier A2 — teleport the LOCAL PlayerCharacter to a world position.
// Uses Actor::MoveTo with doProcessUpdate=1 and skip_player_check=0 so the
// engine runs the real player cell/camera path. MAIN THREAD ONLY.
// yaw_rad is Bethesda convention (same as actor_atomic_teleport).
bool player_teleport_to(float pos_x, float pos_y, float pos_z, float yaw_rad);

// Tier A4 — grant XP to the local player via Game.RewardPlayerXP native.
// suppress_notifications=true avoids double toast spam on the receiver.
// MAIN THREAD ONLY (Papyrus-adjacent / UI). Call under ApplyingRemoteGuard
// when applying a remote grant so the observe hook does not re-emit.
bool reward_player_xp(std::int32_t amount, bool suppress_notifications = true);

// B6.11 — read current TESWeather formID from Sky (any thread, SEH-caged).
// Returns 0 if sky/weather unavailable.
std::uint32_t get_current_weather_form_id() noexcept;

// B6.11 — force-set active weather on this client (MAIN THREAD preferred).
// ab_override=true matches console ForceWeather-ish stickiness.
bool apply_weather(std::uint32_t weather_form_id, bool ab_override = true,
                   bool ab_accelerate = true);

// B6.8 — SetPlayerTeammate on a local actor (MAIN THREAD).
// Used to mirror companion recruit/dismiss across peers.
bool apply_player_teammate(std::uint32_t actor_form_id, bool teammate,
                           bool can_do_favor = true);

// Local companion roster (best-effort). Updated when SetPlayerTeammate
// fires or is applied. Used to pull companions along on party teleport.
void note_local_companion(std::uint32_t actor_form_id, bool teammate);
// After PC party-teleport: MoveTo each known teammate near the PC.
// MAIN THREAD. Offsets companions slightly so they don't stack.
void pull_local_companions_to_player(float pos_x, float pos_y, float pos_z,
                                     float yaw_rad);

// B6.9 — read/write TESObjectCELL cleared flag (u8 at +0x134).
// get: any thread SEH-caged. set: MAIN THREAD preferred (worker side effects).
...
// v23: apply SetRelationshipRank between two actors (MAIN THREAD).
// Typically actor_a is local PC and actor_b is an NPC.
bool apply_relationship_rank(std::uint32_t actor_a_form_id,
                             std::uint32_t actor_b_form_id,
                             std::int32_t rank);

// v23: apply Game.PassTime(hours) so Wait/Sleep jumps match (MAIN THREAD).
bool apply_pass_time(std::int32_t hours);

// v25: grant item to local PlayerCharacter (MAIN THREAD).
// Uses engine AddItem on the PC REFR. Silent flag suppresses UI spam.
// Dedups unique (count==1) grants when the PC already holds the form.
...
//
// FW_MINIMAL: compiled out along with spawn_ghost_proxy below — see the block
// comment above their definitions in engine_calls.cpp. Both are dead paths;
// the visible ghost is the injected NiNode from scene_inject.cpp.
#if !FW_MINIMAL
...
// marshalling.
//
// FW_MINIMAL: start of the combat-tier declarations, mirroring the guarded
// block in engine_calls.cpp. See the banner there.
#if !FW_MINIMAL
...
#endif // !FW_MINIMAL — posture readers below are KEPT (POS_BROADCAST needs them)
...
// FW_MINIMAL: combat-tier declarations resume, through disable_actor_movement.
#if !FW_MINIMAL
...
#endif // !FW_MINIMAL — the yaw setter below is KEPT (NPC pos sync uses it)
...
#endif // !FW_MINIMAL — apply_npc_pos below is KEPT
...
//   3. Allocate a 40-byte block via `sub_141B0EE10(&unk_143E5E0F0, 0x28, 0, 0)`
