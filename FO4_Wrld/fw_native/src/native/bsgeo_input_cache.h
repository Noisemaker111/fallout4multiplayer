...
// Compiled out (see fw_native/minimal_exclude.txt). A null lookup is the real
// function's own "factory wasn't invoked for this shape" answer, which callers
// in the weapon-mesh path already handle.
inline bool install(std::uintptr_t) { return false; }
inline const CapturedMeshInput* lookup(const void*) { return nullptr; }
