...
// compiling against these stubs: an empty path is exactly the "not known" answer
// the real lookup gives for an unseen node, which its callers already handle.
inline bool install(std::uintptr_t) { return false; }
inline std::string lookup(void*) { return {}; }
