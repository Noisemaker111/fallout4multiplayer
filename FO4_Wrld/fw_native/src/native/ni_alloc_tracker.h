...
//   (sub_141B0EE10, the pool allocator that EVERY NiObject derives from)
...
// Compiled out (see fw_native/minimal_exclude.txt). weapon_witness.cpp keeps
// compiling against these stubs; a null trace is the real function's own
// "pointer not in cache" answer, which its callers already handle.
inline bool install(std::uintptr_t) { return false; }
inline const AllocTrace* lookup(const void*) { return nullptr; }
inline std::uint64_t total_invocations() { return 0; }
inline std::size_t   entry_count()       { return 0; }
inline std::uint64_t total_evictions()   { return 0; }
inline std::uintptr_t caller_rva(const AllocTrace*) { return 0; }
