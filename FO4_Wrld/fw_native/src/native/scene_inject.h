...
//
// Multi-peer: the stash is keyed by `peer_id`, so four peers moving at once
// keep four independent poses instead of overwriting one slot (which made
// every ghost adopt whichever packet landed last). The APPLY side still drives
// a single ghost body — see docs/MULTIPEER.md — so it currently applies the
// most recently updated peer, exactly matching the previous behaviour. Once
// the ghost registry lands, on_pose_apply_message() iterates instead.
//
// `peer_id` may be empty; an empty id is a valid map key and behaves like the
// old single slot.
...
//
// Keyed by `peer_id` for the same reason as store_remote_pose above.
...
//   - A NiNode is allocated via sub_141B0EE10 at our tracked ptr
