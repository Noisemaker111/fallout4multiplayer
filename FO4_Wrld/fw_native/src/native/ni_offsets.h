...
//
// Target: Fallout4.exe 1.10.163 (classic). Originally RE'd on 1.11.191;
// vtables re-derived 2026-08-02 via CommonLibF4 VTABLE_IDs + Address Library.
// Function RVAs below still need IDA residual (step F) unless marked.
//
// Source: re/stradaB_M1_dossier.txt + re/M8P*.txt; AddressLib for vtables.
//
...
//   void* p = sub_141B0EE10(&unk_143E5E0F0, 0x140u, 0x10u, 1);
//
...
// Heap path recovered from live SSN alloc sites on 1.10.163 (same sequence
// as NG: if (flag!=2) pool_init(pool, &flag); allocate(pool, size, align, 1)).
// ALLOCATE = AddressLib MemoryManager::Allocate (ID 652767) — matches call
// shape (pool*, size, align, bool). Flag word lives next to the pool desc.
constexpr std::uintptr_t MEM_POOL_RVA          = 0x038CC980;
constexpr std::uintptr_t ALLOCATE_FN_RVA       = 0x01B0EFD0; // AddressLib MemMgr::Allocate
constexpr std::uintptr_t POOL_INIT_FN_RVA      = 0x01B0F450; // seen at every lazy-init site
constexpr std::uintptr_t POOL_INIT_FLAG_RVA    = 0x038CCE00;
constexpr std::uint32_t  POOL_INIT_FLAG_READY  = 2;
...
// Vtables: 1.10.163 via AddressLib (CommonLib VTABLE_IDs primary entry).
constexpr std::uintptr_t NINODE_VTABLE_RVA     = 0x02E14818; // ID 1048271
constexpr std::uintptr_t NINODE_CTOR_RVA       = 0x01B983C0; // 1.10.163: first .text ref to NiNode vt
...
constexpr std::uintptr_t BSFADENODE_VTABLE_RVA     = 0x03085EE8; // ID 93613
constexpr std::uintptr_t BSLEAFANIMNODE_VTABLE_RVA = 0x03086328; // ID 1448150
...
constexpr std::uintptr_t BSSKIN_INSTANCE_VTABLE_RVA = 0x02E16588; // ID 424506
// BSSkin::Instance copy ctor (clones from another instance) — per M8P3 §11.
constexpr std::uintptr_t BSSKIN_INSTANCE_COPY_CTOR_RVA = 0x01BB2F80; // 1.10.163: 2nd .text ref to BSSkin::Instance vt
// Geometry skin slot.
...
constexpr std::uintptr_t NIAVOBJECT_VTABLE_RVA = 0x02E150A8; // ID 1031341
constexpr std::uintptr_t NIAVOBJECT_CTOR_RVA  = 0x01BA3B50; // 1.10.163: first .text ref to NiAVObject vt
...
constexpr std::uintptr_t NIREFOBJECT_VTABLE_RVA = 0x02C4CC38; // ID 1414337

// ============================================================================
// 3. SCENE ROOT  (1.10.163 — recovered 2026-08-02)
// ============================================================================
// NG (1.11.191) dossier claimed free globals:
//   SSN   qword_143E47A10  — on 1.10.163: **0** rip-rel xrefs (DEAD)
//   World qword_1432D2228  — on 1.10.163: **0** rip-rel xrefs (DEAD)
//
// Live path on 1.10.163 (SSN ctor sites + CommonLib Interface3D.h):
//   1) World SceneGraph** @ 0x05AA4358  (109+ rip-rel refs)
//      SSN = *(SceneGraph* + 0x140)     // SCENEGRAPH_SSN_SHORTCUT_OFF
//   2) Interface3D::Renderer** @ 0x067220E8  (UI only — Pipboy/menu)
//      screenSSN @ Renderer+0x238, offscreenSSN @ +0x240
//      (CommonLib Interface3D::Renderer layout — NOT the world root)
//
// Ghost bodies must attach under the **world** SSN (path 1), not UI SSN.
// SSN is recreated on worldspace unload — read AT ATTACH TIME, never cache.
//
// SSN_SINGLETON_RVA kept as 0: there is no free SSN** on this build.
// Resolve via WORLD_SG + SCENEGRAPH_SSN_SHORTCUT_OFF (see scene_inject).

constexpr std::uintptr_t SSN_SINGLETON_RVA      = 0;          // no free global on 1.10.163
constexpr std::uintptr_t WORLD_SG_SINGLETON_RVA = 0x05AA4358; // SceneGraph** (disasm)

// UI Renderer singleton — do NOT use for world ghost attach.
constexpr std::uintptr_t INTERFACE3D_RENDERER_SINGLETON_RVA = 0x067220E8;
constexpr std::size_t    RENDERER_SCREEN_SSN_OFF            = 0x238;
constexpr std::size_t    RENDERER_OFFSCREEN_SSN_OFF         = 0x240;

// SceneGraph shortcut to ShadowSceneNode:
//   "*(QWORD*)(World + 320) = SSN"  i.e. offset 0x140
// CRITICAL: renderer walks FROM SSN. Attach as SSN child (not World sibling).
...
// Canonical call (from renderer init sub_140D379E0):
...
// Attach/detach from live NiNode vtable @ 0x02E14818 on 1.10.163
// (slots 58/59/60 — same indices as NG dossier).
constexpr std::uintptr_t NINODE_ATTACH_CHILD_FN_RVA    = 0x01B98A10; // vt[58]
constexpr std::uintptr_t NINODE_ATTACH_CHILD_AT_FN_RVA = 0x01B98AE0; // vt[59]
constexpr std::uintptr_t NINODE_DETACH_CHILD_FN_RVA    = 0x01B98BA0; // vt[60]
...
// BSFixedString — recovered by disasm on 1.10.163 (2026-08-02).
// CREATE: (BSFixedString* out, const char* s) zeros *out, interns, stores handle.
// RELEASE: AddressLib Entry::release (ID 1204430).
// SETNAME: pure thunk `add rcx, 0x10; jmp operator=` — expects (NiObjectNET*,
//          BSFixedString* name). Call sites must pass &handle, not the value.
constexpr std::uintptr_t FIXEDSTR_CREATE_RVA   = 0x01B41E70;
constexpr std::uintptr_t FIXEDSTR_RELEASE_RVA  = 0x01B42FD0;
constexpr std::uintptr_t NINODE_SETNAME_RVA    = 0x01B966C0;
...
// Vtables: 1.10.163 AddressLib. Ctors still TODO (IDA residual).
constexpr std::uintptr_t BSGEOMETRY_VTABLE_RVA       = 0x02E161D8; // ID 1436492
constexpr std::uintptr_t BSGEOMETRY_CTOR_RVA         = 0x01BB0BC0; // 1.10.163: first .text ref to BSGeometry vt
constexpr std::size_t    BSGEOMETRY_SIZEOF           = 0x170;

constexpr std::uintptr_t BSTRISHAPE_VTABLE_RVA       = 0x02E16AE8; // ID 183326
constexpr std::uintptr_t BSTRISHAPE_CTOR_RVA         = 0x01BB5FA0; // 1.10.163: first .text ref to BSTriShape vt
constexpr std::size_t    BSTRISHAPE_SIZEOF           = 0x170;

// BSDynamicTriShape — AddressLib primary entry for 1.10.163.
constexpr std::uintptr_t BSDYNAMICTRISHAPE_VTABLE_RVA = 0x02E17BA8; // ID 681999
constexpr std::uintptr_t BSDYNAMICTRISHAPE_CTOR_RVA   = 0x01BC3230; // 1.10.163: first .text ref to BSDynamicTriShape vt
constexpr std::size_t    BSDYNAMICTRISHAPE_SIZEOF     = 0x190;

// Legacy NG alt vtable — kept for diagnostics only (not the 1.10.163 primary).
constexpr std::uintptr_t BSDYNAMICTRISHAPE_VTABLE_ALT_RVA = 0x02E17BA8;

// BSSubIndexTriShape — multi-segment tri shape used for character bodies.
// 1.10.163 AddressLib ID 248868.
constexpr std::uintptr_t BSSUBINDEXTRISHAPE_VTABLE_RVA = 0x02E314E8; // ID 248868
...
// NiAVObject vt[0x30] on 1.10.163 (CommonLib virtual index // 30).
constexpr std::uintptr_t UPDATE_DOWNWARD_PASS_RVA    = 0x01BA3EA0;
...
// 1.10.163: PC::Load3D (vt[134] @ 0x00E9F5B0) calls this after FixedString
// c_str with opts flags 0x3D at +0x08. Thin wrapper: Demand path then
// refcount-swaps entry+0x20 into *out. NOT Gamebryo header writer 0x01BBAB10.
constexpr std::uintptr_t NIF_LOAD_BY_PATH_RVA        = 0x01C8E0C0; // 1.10.163 disasm (was NG 0x17B3E90)
...
// 1.10.163: 5-arg worker — r14=path(rdx), r15=opts(r8), rsi=out(r9);
// tests opts+8 bits 0x02/0x10/0x20; calls BSFadeNode ctor 0x027D04C0 + pool.
// Funnel: public 0x01C8E0C0 → Demand 0x01C8DF60 → EntryDB → this.
constexpr std::uintptr_t NIF_LOAD_WORKER_RVA         = 0x01C8D700; // 1.10.163 disasm (was NG 0x17B3480)
...
constexpr std::uintptr_t PLAYER_SINGLETON_RVA      = 0x05AA4388;  // 1.10.163 AddressLib (same as offsets.h)
...
// 1.10.163: REFR::Load3D post-ProcessEvent(28) calls with
//   (root3D, matSwap, matIdx, a4, 0) — same shape as NG sub_140255BA0.
// Family xrefs `data\materials\%s` @ 0x02C4C350; tree-walks NiNode children.
constexpr std::uintptr_t APPLY_MATERIALS_WALKER_RVA = 0x00053080; // 1.10.163 disasm (was NG 0x255BA0)

// Additional RVAs for manual/targeted use (not needed for the main fix):
constexpr std::uintptr_t APPLY_MATERIALS_PERNODE_RVA = 0x000531B0; // inner apply (was NG 0x256070)
...
//   v47 = sub_141B0EE10(pool, 0x88, 0, 0);          // alloc 0x88 bytes
...
//   v51 = sub_141B0EE10(pool, 0x30, 0, 0);
