...
#include "ghost_registry.h"   // per-peer ghost slots (multi-peer scale-up)
...
// as siblings.
//
// Multi-peer: this is the PRIMARY peer's body, and it is now a read cache for
// `ghosts::primary_body()`. It stays a plain atomic because several readers
// sit inside SEH `__try` blocks, where a std::lock_guard is a hard compile
// error (C2712) — so the fast path must stay lock-free.
//
// The invariant is maintained at the only two writers (inject_debug_cube and
// detach_debug_cube), which update the registry and this atomic together.
// Non-primary peers live solely in the registry; as each of the ~24 readers
// below becomes peer-aware it moves to ghosts::body_for(peer) and this cache
// shrinks toward deletion. See docs/MULTIPEER.md.
...
bool resolve_once() {
    ...
    // 1.10.163: no free SSN** (SSN_SINGLETON_RVA==0). World SSN is
    // *(WORLD_SG + SCENEGRAPH_SSN_SHORTCUT_OFF). Keep ssn_slot null.
    g_r.ssn_slot        = SSN_SINGLETON_RVA
        ? reinterpret_cast<void**>(base + SSN_SINGLETON_RVA)
        : nullptr;
    ...
    {
        void* ssn_ptr = nullptr;
        if (g_r.ssn_slot) {
            ssn_ptr = *g_r.ssn_slot;
        } else if (g_r.world_sg_slot && *g_r.world_sg_slot) {
            ssn_ptr = *reinterpret_cast<void**>(
                reinterpret_cast<char*>(*g_r.world_sg_slot) +
                SCENEGRAPH_SSN_SHORTCUT_OFF);
        }
        FW_LOG("[native]   ssn_slot=%p  ssn=%p (via %s)",
               static_cast<void*>(g_r.ssn_slot), ssn_ptr,
               g_r.ssn_slot ? "global" : "WorldSG+0x140");
        FW_LOG("[native]   world_sg_slot=%p  world_sg=%p",
               static_cast<void*>(g_r.world_sg_slot),
               g_r.world_sg_slot ? *g_r.world_sg_slot : nullptr);
    }
...
bool try_inject(float x, float y, float z, void** out_node) {
    *out_node = nullptr;
    ...
    __try {
        // 2. Pick an attach parent (1.10.163).
        //
        // World SceneGraph** @ WORLD_SG_SINGLETON_RVA (0x05AA4358).
        // World SSN = *(SceneGraph + SCENEGRAPH_SSN_SHORTCUT_OFF=0x140).
        // Prefer SSN as parent so the main renderer walk reaches us.
        // Fall back to World SceneGraph only if SSN shortcut is null.
        //
        // UI Interface3D::Renderer+0x238/0x240 holds menu/Pipboy SSNs only
        // — never use those for world ghost attach.
        void* worldsg = g_r.world_sg_slot ? *g_r.world_sg_slot : nullptr;
        void* ssn     = nullptr;
        if (g_r.ssn_slot) {
            ssn = *g_r.ssn_slot;
        } else if (worldsg) {
            ssn = *reinterpret_cast<void**>(
                reinterpret_cast<char*>(worldsg) + SCENEGRAPH_SSN_SHORTCUT_OFF);
        }
        ...
        void* parent = ssn ? ssn : worldsg;
        if (!parent) {
            FW_WRN("[native] inject: World SceneGraph/SSN null "
                   "(very early boot / loading screen?)");
            return false;
        }
        FW_LOG("[native] inject: parent=%s %p",
               parent == ssn ? "SSN" : "WorldSG", parent);
    ...
    *out_body = nullptr;
    ...
    __try {
        ...
        if (head) {
            __try {
                ...
                if (head_rear) {
                    ...
                    if (rear_name) {
                        g_r.set_name(head_rear, &rear_name);
        ...
        *flags |= NIAV_FLAG_MOVABLE;
        ...
        if (name_handle) {
            g_r.set_name(skel_root, &name_handle);
            g_r.fs_release(&name_handle);
        }
...
bool try_detach(void* node) {
    ...
    __try {
        ...
        void* world_sg = g_r.world_sg_slot ? *g_r.world_sg_slot : nullptr;
        void* ssn      = get_shadow_scene_node();
        if (!ssn && world_sg) {
            ssn = *reinterpret_cast<void**>(
                reinterpret_cast<char*>(world_sg) + SCENEGRAPH_SSN_SHORTCUT_OFF);
        }
...
bool inject_debug_cube(float x, float y, float z) {
    ...
    if (ok && body) {
        // Registry first, then the lock-free read cache — both writers keep
        // g_injected_cube == ghosts::primary_body().
        ghosts::set_primary_body(body, offsets::GHOST_FORMID_BASE);
...
void detach_debug_cube() {
    ...
    // Keep the registry in lockstep with the read cache. take_primary_body
    // also drops that peer's bone tables, which point into the subtree we are
    // about to tear down.
    ghosts::take_primary_body();
...
int ghost_attach_mesh_blob(const char* peer_id, std::uint32_t item_form_id,
    ...
    // Optional: name the root so it shows in scene dumps.
    {
        ...
        if (name_handle) {
            g_r.set_name(weapon_root, &name_handle);
            g_r.fs_release(&name_handle);
        }
    }
...
// Keyed by peer_id — one pose per remote player. `g_remote_pose_last_peer`
// names the most recently written entry, which is what the single-ghost apply
// path consumes (identical to the old single-slot last-writer-wins).
std::unordered_map<std::string, RemotePoseSlot> g_remote_poses;
std::string     g_remote_pose_last_peer;
                       ...
                       std::size_t bone_count)
{
    ...
    if (bone_count > fw::net::MAX_POSE_BONES) bone_count = fw::net::MAX_POSE_BONES;
    {
        ...
        RemotePoseSlot& slot = g_remote_poses[peer_id ? peer_id : ""];
        slot.has_data   = true;
        slot.ts_ms      = ts_ms;
        ...
        g_remote_pose_last_peer = peer_id ? peer_id : "";
    }
    // Wake main thread.
    const HWND h = fw::dispatch::get_target_hwnd();
...
void on_pose_apply_message() {
    ...
    {
        std::lock_guard<std::mutex> lk(g_remote_pose_mutex);
        // Single ghost body today, so drive it from the most recently updated
        // peer — the same last-writer-wins result as the old single slot. The
        // other peers' poses are retained in the map and will be applied
        // per-body once the ghost registry lands (docs/MULTIPEER.md).
        const auto it = g_remote_poses.find(g_remote_pose_last_peer);
        if (it == g_remote_poses.end()) return;
        const RemotePoseSlot& slot = it->second;
        if (!slot.has_data || slot.bone_count == 0) return;
        n = slot.bone_count;
        std::memcpy(quats, slot.quats,
...
// Keyed by peer_id, mirroring g_remote_poses above.
std::unordered_map<std::string, RemoteCrouchSlot> g_remote_crouches;
std::string       g_remote_crouch_last_peer;
                         ...
                         std::size_t count)
{
    ...
    if (count > fw::net::MAX_POSE_CROUCH_BONES) count = fw::net::MAX_POSE_CROUCH_BONES;
    {
        ...
        RemoteCrouchSlot& slot = g_remote_crouches[peer_id ? peer_id : ""];
        slot.has_data = true;
        slot.ts_ms    = ts_ms;
        ...
        g_remote_crouch_last_peer = peer_id ? peer_id : "";
    }
    // Wake main thread.
    const HWND h = fw::dispatch::get_target_hwnd();
...
void on_pose_crouch_apply_message() {
    ...
    {
        std::lock_guard<std::mutex> lk(g_remote_crouch_mutex);
        // Most recently updated peer — see the equivalent note in
        // on_pose_apply_message().
        const auto it = g_remote_crouches.find(g_remote_crouch_last_peer);
        if (it == g_remote_crouches.end()) return;
        const RemoteCrouchSlot& slot = it->second;
        if (!slot.has_data || slot.count == 0) return;
        cnt = slot.count;
        std::memcpy(entries, slot.entries,
