...
public:
    ...
    // B4 QuestStage: reliable QUEST_STAGE_SET — fire-and-forget. Server
    // records last-write-wins stage and BCAST to other peers. Sender already
    // applied locally via the Papyrus native (observe hook fires post-local).
    void enqueue_quest_stage_set(std::uint32_t quest_form_id,
                                 std::uint16_t new_stage);

    // B6.7: reliable FACTION_RANK_SET — local PC SetFactionRank. Server
    // records last-write-wins rank and BCAST to other peers.
    void enqueue_faction_rank_set(std::uint32_t faction_form_id,
                                  std::int32_t rank);

    // A4: reliable XP_GRANT_SET — local PC just gained XP.
    void enqueue_xp_grant(std::int32_t amount);

    // A2: reliable PARTY_WARP — summon peers to my world pos.
    void enqueue_party_warp(float x, float y, float z, float yaw_rad,
                            std::uint32_t cell_id);

    // B6.11: reliable WEATHER_SET — current TESWeather formID.
    void enqueue_weather_set(std::uint32_t weather_form_id);

    // B6.8: reliable COMPANION_SET — SetPlayerTeammate mirror.
    void enqueue_companion_set(std::uint32_t actor_form_id, bool teammate,
                               bool can_do_favor);

    // B6.9: reliable CELL_CLEARED_SET — TESObjectCELL cleared flag.
    void enqueue_cell_cleared(std::uint32_t cell_form_id, bool cleared);

    // v23: relationship rank + PassTime (Wait/Sleep).
    void enqueue_relationship_set(std::uint32_t actor_a, std::uint32_t actor_b,
                                  std::int32_t rank);
    void enqueue_time_pass(std::int32_t hours);

    // v25: silent PC AddItem (quest reward / script grant).
    void enqueue_item_grant(std::uint32_t item_form_id, std::int32_t count,
                            bool silent);
    ...
    //
    // Legacy single-ghost accessor: returns the MOST RECENTLY UPDATED peer.
    // Correct while the client drives one ghost body; every caller that will
    // eventually drive N bodies should move to get_remote_snapshots().
    RemotePlayerSnapshot get_remote_snapshot() const;

    // Multi-peer: every peer we currently hold state for, keyed by peer_id.
    // The server has been verified to fan out to 10 peers
    // (net/tests/test_multipeer.py), so this map is the client-side half of
    // that — the ghost-body layer is what still needs to grow past one.
    std::unordered_map<std::string, RemotePlayerSnapshot>
    get_remote_snapshots() const;

    // One specific peer. has_state=false if we've heard nothing from them.
    RemotePlayerSnapshot get_remote_snapshot_for(const std::string& peer_id) const;

    // Number of distinct peers we hold position state for.
    std::size_t remote_peer_count() const;

    // Drop a peer's cached state — call on PEER_LEAVE so a departed peer's
    // last known position can't keep a stale ghost body alive.
    void forget_remote_peer(const std::string& peer_id);
...
private:
    ...
    // Built by make_transport() at the top of run_loop(). Today always UDP;
    // see transport.h for why the seam is here.
    ...
    // --- ε.pivot: remote player snapshots, keyed by peer_id ---
    // Written by dispatch() (net worker thread) on every POS_BROADCAST;
    // read by the body renderer on every Present.
    //
    // `last_remote_peer_` names the most recently updated entry so the legacy
    // single-ghost accessor keeps its exact previous behaviour (last writer
    // wins) while the map underneath is already N-peer.
    mutable std::mutex remote_mutex_;
    std::unordered_map<std::string, RemotePlayerSnapshot> remote_snapshots_;
    std::string remote_snapshot_last_peer_;
