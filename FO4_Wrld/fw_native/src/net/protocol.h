...
// Defined in protocol_version.h so fw_launcher can advertise the same number
// in Steam lobby data without including this header. Bump both sides there.
constexpr std::uint8_t  PROTOCOL_VERSION  = FW_PROTOCOL_VERSION;  // v25: ITEM_GRANT_*
...
enum class MessageType : std::uint16_t {
    ...
    FACTION_RANK_SET      = 0x0421,  // v19 B6.7: C→S local PC SetFactionRank
    FACTION_RANK_BCAST    = 0x0422,  // v19 B6.7: S→peers PC faction rank
    FACTION_STATE_BOOT    = 0x0024,  // v19 B6.7: S→client faction snapshot
    XP_GRANT_SET          = 0x0431,  // v20 A4: C→S XP gain
    XP_GRANT_BCAST        = 0x0432,  // v20 A4: S→peers XP gain (proximity)
    PARTY_WARP            = 0x0441,  // v20 A2: C→S summon party
    PARTY_WARP_BCAST      = 0x0442,  // v20 A2: S→peers teleport here
    WEATHER_SET           = 0x0451,  // v21 B6.11: C→S weather form
    WEATHER_BCAST         = 0x0452,  // v21 B6.11: S→peers set weather
    WEATHER_STATE_BOOT    = 0x0025,  // v21 B6.11: S→joiner current weather
    COMPANION_SET         = 0x0461,  // v21 B6.8: C→S SetPlayerTeammate
    COMPANION_BCAST       = 0x0462,  // v21 B6.8: S→peers companion flags
    COMPANION_STATE_BOOT  = 0x0026,  // v21 B6.8: S→joiner companion snapshot
    CELL_CLEARED_SET      = 0x0471,  // v22 B6.9: C→S cell cleared flag
    CELL_CLEARED_BCAST    = 0x0472,  // v22 B6.9: S→peers cell cleared
    CELL_CLEARED_BOOT     = 0x0027,  // v22 B6.9: S→joiner cleared cells
    RELATIONSHIP_SET      = 0x0481,  // v23: C→S SetRelationshipRank
    RELATIONSHIP_BCAST    = 0x0482,  // v23: S→peers
    RELATIONSHIP_STATE_BOOT = 0x0028, // v24: S→joiner relationship ranks
    TIME_PASS_SET         = 0x0491,  // v23: C→S Game.PassTime hours
    TIME_PASS_BCAST       = 0x0492,  // v23: S→peers time jump
    ITEM_GRANT_SET        = 0x04A1,  // v25: C→S silent PC AddItem
    ITEM_GRANT_BCAST      = 0x04A2,  // v25: S→peers grant item to PC
};
...
// v4: QUEST_STATE_BOOT entry. Python <IH = 6 bytes.
struct QuestStageStateEntry {
    std::uint32_t quest_form_id;
    std::uint16_t stage;
};
static_assert(sizeof(QuestStageStateEntry) == 6, "QuestStageStateEntry size");
...
// v19 B6.7: FACTION_RANK_SET (client → server). Python <IiQ = 16 bytes.
struct FactionRankSetPayload {
    std::uint32_t faction_form_id;
    std::int32_t  rank;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(FactionRankSetPayload) == 16, "FactionRankSetPayload size");

// v19 B6.7: FACTION_RANK_BCAST (server → other peers). FixedClientId + 16 = 32.
struct FactionRankBroadcastPayload {
    FixedClientId peer_id;
    std::uint32_t faction_form_id;
    std::int32_t  rank;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(FactionRankBroadcastPayload) == 32, "FactionRankBroadcastPayload size");

// v19 B6.7: FACTION_STATE_BOOT entry. Python <Ii = 8 bytes.
struct FactionRankStateEntry {
    std::uint32_t faction_form_id;
    std::int32_t  rank;
};
static_assert(sizeof(FactionRankStateEntry) == 8, "FactionRankStateEntry size");

// v20 A4: XP_GRANT_SET. Python <iQ = 12 bytes.
struct XpGrantSetPayload {
    std::int32_t  amount;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(XpGrantSetPayload) == 12, "XpGrantSetPayload size");

// v20 A4: XP_GRANT_BCAST. FixedClientId + 12 = 28.
struct XpGrantBroadcastPayload {
    FixedClientId peer_id;
    std::int32_t  amount;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(XpGrantBroadcastPayload) == 28, "XpGrantBroadcastPayload size");

// v20 A2: PARTY_WARP. Python <ffffIQ = 28 bytes.
struct PartyWarpPayload {
    float         x, y, z, yaw_rad;
    std::uint32_t cell_id;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(PartyWarpPayload) == 28, "PartyWarpPayload size");

// v20 A2: PARTY_WARP_BCAST. FixedClientId + 28 = 44.
struct PartyWarpBroadcastPayload {
    FixedClientId peer_id;
    float         x, y, z, yaw_rad;
    std::uint32_t cell_id;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(PartyWarpBroadcastPayload) == 44, "PartyWarpBroadcastPayload size");

// v21 B6.11: WEATHER_SET. Python <IQ = 12.
struct WeatherSetPayload {
    std::uint32_t weather_form_id;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(WeatherSetPayload) == 12, "WeatherSetPayload size");

struct WeatherBroadcastPayload {
    FixedClientId peer_id;
    std::uint32_t weather_form_id;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(WeatherBroadcastPayload) == 28, "WeatherBroadcastPayload size");

// v21 B6.8: COMPANION_SET. Python <IBBQ = 14.
struct CompanionSetPayload {
    std::uint32_t actor_form_id;
    std::uint8_t  teammate;
    std::uint8_t  can_do_favor;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(CompanionSetPayload) == 14, "CompanionSetPayload size");

struct CompanionBroadcastPayload {
    FixedClientId peer_id;
    std::uint32_t actor_form_id;
    std::uint8_t  teammate;
    std::uint8_t  can_do_favor;
...
static_assert(sizeof(CompanionBroadcastPayload) == 30, "CompanionBroadcastPayload size");

struct CompanionStateEntry {
    std::uint32_t actor_form_id;
    std::uint8_t  teammate;
    std::uint8_t  can_do_favor;
};
static_assert(sizeof(CompanionStateEntry) == 6, "CompanionStateEntry size");

// v22 B6.9: CELL_CLEARED_SET. Python <IBQ = 13.
struct CellClearedSetPayload {
    std::uint32_t cell_form_id;
    std::uint8_t  cleared;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(CellClearedSetPayload) == 13, "CellClearedSetPayload size");

struct CellClearedBroadcastPayload {
    FixedClientId peer_id;
    std::uint32_t cell_form_id;
    std::uint8_t  cleared;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(CellClearedBroadcastPayload) == 29, "CellClearedBroadcastPayload size");

struct CellClearedStateEntry {
    std::uint32_t cell_form_id;
    std::uint8_t  cleared;
};
static_assert(sizeof(CellClearedStateEntry) == 5, "CellClearedStateEntry size");

// v23: RELATIONSHIP_SET. Python <IIiQ = 20.
struct RelationshipSetPayload {
    std::uint32_t actor_a_form_id;
    std::uint32_t actor_b_form_id;
    std::int32_t  rank;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(RelationshipSetPayload) == 20, "RelationshipSetPayload size");

struct RelationshipBroadcastPayload {
    FixedClientId peer_id;
    std::uint32_t actor_a_form_id;
    std::uint32_t actor_b_form_id;
    std::int32_t  rank;
...
static_assert(sizeof(RelationshipBroadcastPayload) == 36, "RelationshipBroadcastPayload size");

// v24: RELATIONSHIP_STATE_BOOT entry. Python <IIi = 12.
struct RelationshipStateEntry {
    std::uint32_t actor_a_form_id;
    std::uint32_t actor_b_form_id;
    std::int32_t  rank;
};
static_assert(sizeof(RelationshipStateEntry) == 12, "RelationshipStateEntry size");

// v23: TIME_PASS_SET. Python <iQ = 12.
struct TimePassSetPayload {
    std::int32_t  hours;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(TimePassSetPayload) == 12, "TimePassSetPayload size");

struct TimePassBroadcastPayload {
    FixedClientId peer_id;
    std::int32_t  hours;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(TimePassBroadcastPayload) == 28, "TimePassBroadcastPayload size");

// v25: ITEM_GRANT_SET. Python <IiBQ = 17 (packed).
#pragma pack(push, 1)
struct ItemGrantSetPayload {
    std::uint32_t item_form_id;
    std::int32_t  count;
    std::uint8_t  silent;
    std::uint64_t timestamp_ms;
};
static_assert(sizeof(ItemGrantSetPayload) == 17, "ItemGrantSetPayload size");

struct ItemGrantBroadcastPayload {
    FixedClientId peer_id;
    std::uint32_t item_form_id;
    std::int32_t  count;
    std::uint8_t  silent;
...
static_assert(sizeof(ItemGrantBroadcastPayload) == 33, "ItemGrantBroadcastPayload size");
#pragma pack(pop)
