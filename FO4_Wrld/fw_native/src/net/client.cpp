...
#include "../native/ghost_registry.h"
...
bool seh_read_player_xyz(float* out_x, float* out_y, float* out_z) {
    void* player = fw::engine::get_local_player();
    if (!player || !out_x || !out_y || !out_z) return false;
    bool ok = false;
    __try {
...
RemotePlayerSnapshot Client::get_remote_snapshot() const {
    ...
    // Legacy single-ghost view: the most recently updated peer. Identical to
    // the old last-writer-wins slot, so existing callers are unaffected.
    const auto it = remote_snapshots_.find(remote_snapshot_last_peer_);
    if (it == remote_snapshots_.end()) return {};
    return it->second;         // copy out; caller gets stable data
}

std::unordered_map<std::string, RemotePlayerSnapshot>
Client::get_remote_snapshots() const {
    std::lock_guard lk(remote_mutex_);
    return remote_snapshots_;  // copy out; caller iterates without the lock
}

RemotePlayerSnapshot
Client::get_remote_snapshot_for(const std::string& peer_id) const {
    std::lock_guard lk(remote_mutex_);
    const auto it = remote_snapshots_.find(peer_id);
    if (it == remote_snapshots_.end()) return {};
    return it->second;
}
...
std::size_t Client::remote_peer_count() const {
    std::lock_guard lk(remote_mutex_);
    return remote_snapshots_.size();
}

void Client::forget_remote_peer(const std::string& peer_id) {
    std::lock_guard lk(remote_mutex_);
    remote_snapshots_.erase(peer_id);
    if (remote_snapshot_last_peer_ == peer_id) {
        // Don't leave the legacy accessor pointing at a departed peer; pick
        // any surviving one so a 3-peer session degrades to a live ghost
        // rather than a frozen corpse of the peer that left.
        remote_snapshot_last_peer_ =
            remote_snapshots_.empty() ? std::string{}
                                      : remote_snapshots_.begin()->first;
    }
}
...
void Client::enqueue_quest_stage_set(std::uint32_t quest_form_id,
                                     std::uint16_t new_stage) {
    if (!connected_.load() || stopping_.load()) return;
    if (quest_form_id == 0) return;

    QuestStageSetPayload p{};
    p.quest_form_id = quest_form_id;
    p.new_stage     = new_stage;
    {
        using namespace std::chrono;
        p.timestamp_ms = duration_cast<milliseconds>(
            system_clock::now().time_since_epoch()).count();
    ...
    QueuedSend q;
    q.msg_type = MessageType::QUEST_STAGE_SET;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
void Client::enqueue_faction_rank_set(std::uint32_t faction_form_id,
                                      std::int32_t rank) {
    if (!connected_.load() || stopping_.load()) return;
    if (faction_form_id == 0) return;

    FactionRankSetPayload p{};
    p.faction_form_id = faction_form_id;
    p.rank            = rank;
    {
        using namespace std::chrono;
        p.timestamp_ms = duration_cast<milliseconds>(
    ...
    QueuedSend q;
    q.msg_type = MessageType::FACTION_RANK_SET;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
void Client::enqueue_xp_grant(std::int32_t amount) {
    if (!connected_.load() || stopping_.load()) return;
    if (amount == 0) return;
    ...
    QueuedSend q;
    q.msg_type = MessageType::XP_GRANT_SET;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
void Client::enqueue_party_warp(float x, float y, float z, float yaw_rad,
    ...
    QueuedSend q;
    q.msg_type = MessageType::PARTY_WARP;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
void Client::enqueue_weather_set(std::uint32_t weather_form_id) {
    if (!connected_.load() || stopping_.load()) return;
    if (weather_form_id == 0) return;

    WeatherSetPayload p{};
    p.weather_form_id = weather_form_id;
    {
        using namespace std::chrono;
        p.timestamp_ms = duration_cast<milliseconds>(
            system_clock::now().time_since_epoch()).count();
    ...
    QueuedSend q;
    q.msg_type = MessageType::WEATHER_SET;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
void Client::enqueue_companion_set(std::uint32_t actor_form_id, bool teammate,
                                   bool can_do_favor) {
    if (!connected_.load() || stopping_.load()) return;
    if (actor_form_id == 0) return;

    CompanionSetPayload p{};
    p.actor_form_id = actor_form_id;
    p.teammate      = teammate ? 1u : 0u;
    p.can_do_favor  = can_do_favor ? 1u : 0u;
    {
        using namespace std::chrono;
        p.timestamp_ms = duration_cast<milliseconds>(
            system_clock::now().time_since_epoch()).count();
    ...
    QueuedSend q;
    q.msg_type = MessageType::COMPANION_SET;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
void Client::enqueue_cell_cleared(std::uint32_t cell_form_id, bool cleared) {
    if (!connected_.load() || stopping_.load()) return;
    if (cell_form_id == 0) return;

    CellClearedSetPayload p{};
    p.cell_form_id = cell_form_id;
    p.cleared      = cleared ? 1u : 0u;
    {
        using namespace std::chrono;
        p.timestamp_ms = duration_cast<milliseconds>(
            system_clock::now().time_since_epoch()).count();
    ...
    QueuedSend q;
    q.msg_type = MessageType::CELL_CLEARED_SET;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
void Client::enqueue_relationship_set(std::uint32_t actor_a,
                                      std::uint32_t actor_b,
                                      std::int32_t rank) {
    if (!connected_.load() || stopping_.load()) return;
    if (actor_a == 0 || actor_b == 0) return;

    RelationshipSetPayload p{};
    p.actor_a_form_id = actor_a;
    p.actor_b_form_id = actor_b;
    p.rank = rank;
    {
        using namespace std::chrono;
        p.timestamp_ms = duration_cast<milliseconds>(
    ...
    QueuedSend q;
    q.msg_type = MessageType::RELATIONSHIP_SET;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
void Client::enqueue_time_pass(std::int32_t hours) {
    if (!connected_.load() || stopping_.load()) return;
    if (hours == 0) return;

    TimePassSetPayload p{};
    p.hours = hours;
    {
        using namespace std::chrono;
        p.timestamp_ms = duration_cast<milliseconds>(
    ...
    QueuedSend q;
    q.msg_type = MessageType::TIME_PASS_SET;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
void Client::enqueue_item_grant(std::uint32_t item_form_id, std::int32_t count,
                                bool silent) {
    if (!connected_.load() || stopping_.load()) return;
    if (item_form_id == 0 || item_form_id == offsets::FORM_CAPS_SEPTIMS) return;
    if (count <= 0 || count > offsets::ITEM_GRANT_MAX_COUNT) return;

    ItemGrantSetPayload p{};
    p.item_form_id = item_form_id;
    p.count = count;
    p.silent = silent ? 1u : 0u;
    {
        using namespace std::chrono;
        p.timestamp_ms = duration_cast<milliseconds>(
            system_clock::now().time_since_epoch()).count();
    ...
    QueuedSend q;
    q.msg_type = MessageType::ITEM_GRANT_SET;
    q.reliable = true;
    q.payload_bytes.resize(sizeof(p));
    std::memcpy(q.payload_bytes.data(), &p, sizeof(p));
    {
...
bool Client::do_handshake() {
    ...
    if (!transport_->send(frame.data(), frame.size())) {
        FW_ERR("net: initial HELLO send failed (err=%d)", transport_->last_error());
        return false;
    }
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        const int n = transport_->recv(rxbuf, sizeof(rxbuf), 200);
        if (n <= 0) continue;
        ...
        auto delivered = channel_.on_receive(rxbuf, static_cast<std::size_t>(n),
                                              now, &ack_bytes);
        if (!ack_bytes.empty()) {
            transport_->send(ack_bytes.data(), ack_bytes.size());
        }
...
void Client::run_loop() {
    ...
    if (!transport_->open()) {
        FW_ERR("net: %s transport open failed - client will not run",
               transport_->name());
        dead_.store(true);
        return;
    }
    ...
    while (!stopping_.load()) {
        ...
        const int n = transport_->recv(rxbuf, sizeof(rxbuf), 50);
        if (n > 0) {
            std::vector<std::uint8_t> ack_bytes;
            ...
            FW_WRN("net: recv error %d", transport_->last_error());
        }
        ...
        if (channel_.maybe_emit_ack(&opportunistic_ack)) {
            transport_->send(opportunistic_ack.data(), opportunistic_ack.size());
        }
...
void Client::dispatch(const Delivered& d) {
    ...
    case static_cast<std::uint16_t>(MessageType::POSE_BROADCAST): {
        ...
        // Hand off to main thread (stashes + posts WM_APP). Keyed by peer so
        // concurrent senders don't overwrite each other's pose.
        fw::native::store_remote_pose(hdr.peer_id.get().c_str(),
                                      hdr.timestamp_ms, bones, hdr.bone_count);
        break;
    }
    ...
    case static_cast<std::uint16_t>(MessageType::POSE_CROUCH_BROADCAST): {
        ...
        fw::native::store_remote_crouch(hdr.peer_id.get().c_str(),
                                        hdr.timestamp_ms, entries, hdr.count);
        break;
    }
    ...
    case static_cast<std::uint16_t>(MessageType::POS_BROADCAST): {
        ...
        // ε.pivot: publish snapshot for the custom body renderer. Keyed by
        // peer_id, so a 4-peer session keeps four independent positions
        // instead of one slot thrashing between senders. The legacy
        // single-ghost accessor reads whichever peer wrote last, preserving
        // the previous behaviour exactly.
        {
            std::lock_guard lk(remote_mutex_);
            RemotePlayerSnapshot& snap = remote_snapshots_[peer];
            snap.has_state      = true;
            snap.peer_id        = peer;
            snap.pos[0]         = p.x;
            snap.pos[1]         = p.y;
            ...
            remote_snapshot_last_peer_ = peer;
        }
    ...
    case static_cast<std::uint16_t>(MessageType::QUEST_STAGE_BCAST): {
        if (d.payload.size() < sizeof(QuestStageBroadcastPayload)) break;
        QuestStageBroadcastPayload b{};
        std::memcpy(&b, d.payload.data(), sizeof(b));
        if (b.quest_form_id == 0) break;
        FW_LOG("net: QUEST_STAGE_BCAST from %s — quest 0x%X stage=%u",
               b.peer_id.get().c_str(), b.quest_form_id,
               static_cast<unsigned>(b.new_stage));
        // Stage workers are TLS-affine — enqueue for main-thread apply.
        fw::dispatch::PendingQuestStageOp op{};
        op.quest_form_id = b.quest_form_id;
        op.stage         = b.new_stage;
        fw::dispatch::enqueue_quest_stage_apply(op);
        break;
    }

    case static_cast<std::uint16_t>(MessageType::QUEST_STATE_BOOT): {
        // Chunked bootstrap: ChunkHeader (HHH=6) + N × QuestStageStateEntry (6).
        if (d.payload.size() < sizeof(ChunkHeader)) break;
        ChunkHeader h{};
        std::memcpy(&h, d.payload.data(), sizeof(h));
        const std::size_t need =
            sizeof(ChunkHeader) +
            static_cast<std::size_t>(h.num_entries) * sizeof(QuestStageStateEntry);
        if (d.payload.size() < need) {
            FW_WRN("net: QUEST_STATE_BOOT truncated got=%zu need=%zu",
                   d.payload.size(), need);
            break;
        }
        FW_LOG("net: QUEST_STATE_BOOT chunk %u/%u entries=%u",
               static_cast<unsigned>(h.chunk_index),
               static_cast<unsigned>(h.total_chunks),
               static_cast<unsigned>(h.num_entries));
        const auto* entries = reinterpret_cast<const QuestStageStateEntry*>(
            d.payload.data() + sizeof(ChunkHeader));
        for (std::uint16_t i = 0; i < h.num_entries; ++i) {
            if (entries[i].quest_form_id == 0) continue;
            fw::dispatch::PendingQuestStageOp op{};
            op.quest_form_id = entries[i].quest_form_id;
            op.stage         = entries[i].stage;
            fw::dispatch::enqueue_quest_stage_apply(op);
        }
        break;
    }

    case static_cast<std::uint16_t>(MessageType::FACTION_RANK_BCAST): {
        if (d.payload.size() < sizeof(FactionRankBroadcastPayload)) break;
        FactionRankBroadcastPayload b{};
        std::memcpy(&b, d.payload.data(), sizeof(b));
        if (b.faction_form_id == 0) break;
        FW_LOG("net: FACTION_RANK_BCAST from %s — faction 0x%X rank=%d",
               b.peer_id.get().c_str(), b.faction_form_id, b.rank);
        fw::dispatch::PendingFactionRankOp op{};
        op.faction_form_id = b.faction_form_id;
        op.rank            = b.rank;
        fw::dispatch::enqueue_faction_rank_apply(op);
        break;
    }

    case static_cast<std::uint16_t>(MessageType::FACTION_STATE_BOOT): {
        if (d.payload.size() < sizeof(ChunkHeader)) break;
        ChunkHeader h{};
        std::memcpy(&h, d.payload.data(), sizeof(h));
        const std::size_t need =
            sizeof(ChunkHeader) +
            static_cast<std::size_t>(h.num_entries) * sizeof(FactionRankStateEntry);
        if (d.payload.size() < need) {
            FW_WRN("net: FACTION_STATE_BOOT truncated got=%zu need=%zu",
                   d.payload.size(), need);
            break;
        }
        FW_LOG("net: FACTION_STATE_BOOT chunk %u/%u entries=%u",
               static_cast<unsigned>(h.chunk_index),
               static_cast<unsigned>(h.total_chunks),
               static_cast<unsigned>(h.num_entries));
        const auto* entries = reinterpret_cast<const FactionRankStateEntry*>(
            d.payload.data() + sizeof(ChunkHeader));
        for (std::uint16_t i = 0; i < h.num_entries; ++i) {
            if (entries[i].faction_form_id == 0) continue;
            fw::dispatch::PendingFactionRankOp op{};
            op.faction_form_id = entries[i].faction_form_id;
            op.rank            = entries[i].rank;
            fw::dispatch::enqueue_faction_rank_apply(op);
        }
        break;
    }

    case static_cast<std::uint16_t>(MessageType::XP_GRANT_BCAST): {
        if (d.payload.size() < sizeof(XpGrantBroadcastPayload)) break;
        XpGrantBroadcastPayload b{};
        std::memcpy(&b, d.payload.data(), sizeof(b));
        if (b.amount == 0) break;
        // Proximity gate: only share XP if we are near the granting peer.
        const auto snap = get_remote_snapshot_for(b.peer_id.get());
        if (!snap.has_state) {
            FW_DBG("net: XP_GRANT_BCAST from %s amount=%d — no pos, skip",
                   b.peer_id.get().c_str(), b.amount);
            break;
        }
        float lx = 0, ly = 0, lz = 0;
        if (!seh_read_player_xyz(&lx, &ly, &lz)) break;
        const float dx = lx - snap.pos[0];
        const float dy = ly - snap.pos[1];
        const float dz = lz - snap.pos[2];
        const float dist2 = dx * dx + dy * dy + dz * dz;
        const float r = offsets::PROXIMITY_XP_RADIUS_UNITS;
        if (dist2 > r * r) {
            FW_DBG("net: XP_GRANT_BCAST from %s amount=%d — out of range "
                   "(dist2=%.0f r2=%.0f)",
                   b.peer_id.get().c_str(), b.amount, dist2, r * r);
            break;
        }
        FW_LOG("net: XP_GRANT_BCAST from %s amount=%d (in range) — apply",
               b.peer_id.get().c_str(), b.amount);
        fw::dispatch::PendingXpGrantOp op{};
        op.amount = b.amount;
        fw::dispatch::enqueue_xp_grant_apply(op);
        break;
    }

    case static_cast<std::uint16_t>(MessageType::PARTY_WARP_BCAST): {
        if (d.payload.size() < sizeof(PartyWarpBroadcastPayload)) break;
        PartyWarpBroadcastPayload b{};
        std::memcpy(&b, d.payload.data(), sizeof(b));
        FW_LOG("net: PARTY_WARP_BCAST from %s → (%.1f,%.1f,%.1f) cell=0x%X",
               b.peer_id.get().c_str(), b.x, b.y, b.z, b.cell_id);
        fw::dispatch::PendingPartyTeleportOp op{};
        op.pos_x   = b.x;
        op.pos_y   = b.y;
        op.pos_z   = b.z;
        op.yaw_rad = b.yaw_rad;
        op.cell_id = b.cell_id;
        fw::dispatch::enqueue_party_teleport(op);
        break;
    }

    case static_cast<std::uint16_t>(MessageType::WEATHER_BCAST):
    case static_cast<std::uint16_t>(MessageType::WEATHER_STATE_BOOT): {
        std::uint32_t weather_form_id = 0;
        if (d.header.msg_type ==
            static_cast<std::uint16_t>(MessageType::WEATHER_BCAST)) {
            if (d.payload.size() < sizeof(WeatherBroadcastPayload)) break;
            WeatherBroadcastPayload b{};
            std::memcpy(&b, d.payload.data(), sizeof(b));
            weather_form_id = b.weather_form_id;
            FW_LOG("net: WEATHER_BCAST from %s form=0x%X",
                   b.peer_id.get().c_str(), weather_form_id);
        } else {
            if (d.payload.size() < sizeof(WeatherSetPayload)) break;
            WeatherSetPayload b{};
            std::memcpy(&b, d.payload.data(), sizeof(b));
            weather_form_id = b.weather_form_id;
            FW_LOG("net: WEATHER_STATE_BOOT form=0x%X", weather_form_id);
        }
        if (weather_form_id == 0) break;
        fw::dispatch::PendingWeatherOp op{};
        op.weather_form_id = weather_form_id;
        fw::dispatch::enqueue_weather_apply(op);
        break;
    }

    case static_cast<std::uint16_t>(MessageType::COMPANION_BCAST): {
        if (d.payload.size() < sizeof(CompanionBroadcastPayload)) break;
        CompanionBroadcastPayload b{};
        std::memcpy(&b, d.payload.data(), sizeof(b));
        if (b.actor_form_id == 0) break;
        FW_LOG("net: COMPANION_BCAST from %s actor=0x%X teammate=%u favor=%u",
               b.peer_id.get().c_str(), b.actor_form_id,
               static_cast<unsigned>(b.teammate),
               static_cast<unsigned>(b.can_do_favor));
        fw::dispatch::PendingCompanionOp op{};
        op.actor_form_id = b.actor_form_id;
        op.teammate      = b.teammate ? 1u : 0u;
        op.can_do_favor  = b.can_do_favor ? 1u : 0u;
        fw::dispatch::enqueue_companion_apply(op);
        break;
    }

    case static_cast<std::uint16_t>(MessageType::COMPANION_STATE_BOOT): {
        if (d.payload.size() < sizeof(ChunkHeader)) break;
        ChunkHeader h{};
        std::memcpy(&h, d.payload.data(), sizeof(h));
        const std::size_t need =
            sizeof(ChunkHeader) +
            static_cast<std::size_t>(h.num_entries) * sizeof(CompanionStateEntry);
        if (d.payload.size() < need) break;
        FW_LOG("net: COMPANION_STATE_BOOT chunk %u/%u entries=%u",
               static_cast<unsigned>(h.chunk_index),
               static_cast<unsigned>(h.total_chunks),
               static_cast<unsigned>(h.num_entries));
        const auto* entries = reinterpret_cast<const CompanionStateEntry*>(
            d.payload.data() + sizeof(ChunkHeader));
        for (std::uint16_t i = 0; i < h.num_entries; ++i) {
            if (entries[i].actor_form_id == 0) continue;
            fw::dispatch::PendingCompanionOp op{};
            op.actor_form_id = entries[i].actor_form_id;
            op.teammate      = entries[i].teammate ? 1u : 0u;
            op.can_do_favor  = entries[i].can_do_favor ? 1u : 0u;
            fw::dispatch::enqueue_companion_apply(op);
        }
        break;
    }

    case static_cast<std::uint16_t>(MessageType::CELL_CLEARED_BCAST): {
        if (d.payload.size() < sizeof(CellClearedBroadcastPayload)) break;
        CellClearedBroadcastPayload b{};
        std::memcpy(&b, d.payload.data(), sizeof(b));
        if (b.cell_form_id == 0) break;
        FW_LOG("net: CELL_CLEARED_BCAST from %s cell=0x%X cleared=%u",
               b.peer_id.get().c_str(), b.cell_form_id,
               static_cast<unsigned>(b.cleared));
        fw::dispatch::PendingCellClearedOp op{};
        op.cell_form_id = b.cell_form_id;
        op.cleared      = b.cleared ? 1u : 0u;
        fw::dispatch::enqueue_cell_cleared_apply(op);
        break;
    }

    case static_cast<std::uint16_t>(MessageType::CELL_CLEARED_BOOT): {
        if (d.payload.size() < sizeof(ChunkHeader)) break;
        ChunkHeader h{};
        std::memcpy(&h, d.payload.data(), sizeof(h));
        const std::size_t need =
            sizeof(ChunkHeader) +
            static_cast<std::size_t>(h.num_entries) * sizeof(CellClearedStateEntry);
        if (d.payload.size() < need) break;
        FW_LOG("net: CELL_CLEARED_BOOT chunk %u/%u entries=%u",
               static_cast<unsigned>(h.chunk_index),
               static_cast<unsigned>(h.total_chunks),
               static_cast<unsigned>(h.num_entries));
        const auto* entries = reinterpret_cast<const CellClearedStateEntry*>(
            d.payload.data() + sizeof(ChunkHeader));
        for (std::uint16_t i = 0; i < h.num_entries; ++i) {
            if (entries[i].cell_form_id == 0) continue;
            fw::dispatch::PendingCellClearedOp op{};
            op.cell_form_id = entries[i].cell_form_id;
            op.cleared      = entries[i].cleared ? 1u : 0u;
            fw::dispatch::enqueue_cell_cleared_apply(op);
        }
        break;
    }

    case static_cast<std::uint16_t>(MessageType::RELATIONSHIP_BCAST): {
        if (d.payload.size() < sizeof(RelationshipBroadcastPayload)) break;
        RelationshipBroadcastPayload b{};
        std::memcpy(&b, d.payload.data(), sizeof(b));
        if (b.actor_a_form_id == 0 || b.actor_b_form_id == 0) break;
        FW_LOG("net: RELATIONSHIP_BCAST from %s 0x%X->0x%X rank=%d",
               b.peer_id.get().c_str(), b.actor_a_form_id, b.actor_b_form_id,
               b.rank);
        fw::dispatch::PendingRelationshipOp op{};
        op.actor_a_form_id = b.actor_a_form_id;
        op.actor_b_form_id = b.actor_b_form_id;
        op.rank            = b.rank;
        fw::dispatch::enqueue_relationship_apply(op);
        break;
    }

    case static_cast<std::uint16_t>(MessageType::RELATIONSHIP_STATE_BOOT): {
        if (d.payload.size() < sizeof(ChunkHeader)) break;
        ChunkHeader h{};
        std::memcpy(&h, d.payload.data(), sizeof(h));
        const std::size_t need =
            sizeof(ChunkHeader) +
            static_cast<std::size_t>(h.num_entries) * sizeof(RelationshipStateEntry);
        if (d.payload.size() < need) break;
        FW_LOG("net: RELATIONSHIP_STATE_BOOT chunk %u/%u entries=%u",
               static_cast<unsigned>(h.chunk_index),
               static_cast<unsigned>(h.total_chunks),
               static_cast<unsigned>(h.num_entries));
        const auto* entries = reinterpret_cast<const RelationshipStateEntry*>(
            d.payload.data() + sizeof(ChunkHeader));
        for (std::uint16_t i = 0; i < h.num_entries; ++i) {
            if (entries[i].actor_a_form_id == 0 ||
                entries[i].actor_b_form_id == 0) continue;
            fw::dispatch::PendingRelationshipOp op{};
            op.actor_a_form_id = entries[i].actor_a_form_id;
            op.actor_b_form_id = entries[i].actor_b_form_id;
            op.rank            = entries[i].rank;
            fw::dispatch::enqueue_relationship_apply(op);
        }
        break;
    ...
    case static_cast<std::uint16_t>(MessageType::TIME_PASS_BCAST): {
        if (d.payload.size() < sizeof(TimePassBroadcastPayload)) break;
        TimePassBroadcastPayload b{};
        std::memcpy(&b, d.payload.data(), sizeof(b));
        if (b.hours == 0) break;
        FW_LOG("net: TIME_PASS_BCAST from %s hours=%d",
               b.peer_id.get().c_str(), b.hours);
        fw::dispatch::PendingTimePassOp op{};
        op.hours = b.hours;
        fw::dispatch::enqueue_time_pass_apply(op);
        break;
    }

    case static_cast<std::uint16_t>(MessageType::ITEM_GRANT_BCAST): {
        if (d.payload.size() < sizeof(ItemGrantBroadcastPayload)) break;
        ItemGrantBroadcastPayload b{};
        std::memcpy(&b, d.payload.data(), sizeof(b));
        if (b.item_form_id == 0 || b.count <= 0) break;
        FW_LOG("net: ITEM_GRANT_BCAST from %s item=0x%X count=%d silent=%u",
               b.peer_id.get().c_str(), b.item_form_id, b.count,
               static_cast<unsigned>(b.silent));
        fw::dispatch::PendingItemGrantOp op{};
        op.item_form_id = b.item_form_id;
        op.count        = b.count;
        op.silent       = b.silent ? 1u : 0u;
        fw::dispatch::enqueue_item_grant_apply(op);
        break;
    }
    ...
    case static_cast<std::uint16_t>(MessageType::PEER_JOIN): {
        ...
        // Register a ghost slot for this peer. Creating the slot does not
        // inject a body — that still happens once, for the primary peer, via
        // the existing scene-graph path. This makes the registry reflect the
        // real peer set so the body layer has something to key off when it
        // becomes peer-aware (docs/MULTIPEER.md).
        fw::native::ghosts::ensure(p.peer_id.get());
    ...
    case static_cast<std::uint16_t>(MessageType::PEER_LEAVE): {
        ...
        // Drop their cached position so the ghost layer can't keep driving a
        // body from a peer that is gone. Harmless in the 2-peer case (the map
        // just empties); required once several peers come and go in a session.
        forget_remote_peer(p.peer_id.get());
        // Forget the ghost slot too. This only drops our reference — the body
        // node itself is torn down on the main thread by detach_debug_cube,
        // since scene-graph mutation is not safe from the net thread.
        fw::native::ghosts::forget(p.peer_id.get());
        break;
    }
