...
// B4 QuestStage: stage apply queue. Low frequency (one per quest advance /
// bootstrap chunk). Separate mutex so it never blocks door/lock lanes.
std::mutex g_quest_stage_mtx;
std::deque<PendingQuestStageOp> g_quest_stage_queue;

// B6.7 faction rank apply queue.
std::mutex g_faction_rank_mtx;
std::deque<PendingFactionRankOp> g_faction_rank_queue;

// A2 party teleport + A4 XP grant queues.
std::mutex g_party_tp_mtx;
std::deque<PendingPartyTeleportOp> g_party_tp_queue;
std::mutex g_xp_grant_mtx;
std::deque<PendingXpGrantOp> g_xp_grant_queue;
std::mutex g_weather_mtx;
std::deque<PendingWeatherOp> g_weather_queue;
std::mutex g_companion_mtx;
std::deque<PendingCompanionOp> g_companion_queue;
std::mutex g_cell_cleared_mtx;
std::deque<PendingCellClearedOp> g_cell_cleared_queue;
std::mutex g_relationship_mtx;
std::deque<PendingRelationshipOp> g_relationship_queue;
std::mutex g_time_pass_mtx;
std::deque<PendingTimePassOp> g_time_pass_queue;
std::mutex g_item_grant_mtx;
std::deque<PendingItemGrantOp> g_item_grant_queue;
...
void post_wakeup_quest_stage() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_QUEST_STAGE_APPLY, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_QUEST_STAGE_APPLY) failed (err=%lu)",
               GetLastError());
    }
}

void post_wakeup_faction_rank() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_FACTION_RANK_APPLY, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_FACTION_RANK_APPLY) failed (err=%lu)",
               GetLastError());
    }
}

void post_wakeup_party_tp() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_PARTY_TELEPORT, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_PARTY_TELEPORT) failed (err=%lu)",
               GetLastError());
    }
}

void post_wakeup_xp_grant() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_XP_GRANT_APPLY, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_XP_GRANT_APPLY) failed (err=%lu)",
               GetLastError());
    }
}

void post_wakeup_weather() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_WEATHER_APPLY, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_WEATHER_APPLY) failed (err=%lu)",
               GetLastError());
    }
}

void post_wakeup_companion() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_COMPANION_APPLY, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_COMPANION_APPLY) failed (err=%lu)",
               GetLastError());
    }
}

void post_wakeup_cell_cleared() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_CELL_CLEARED_APPLY, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_CELL_CLEARED_APPLY) failed (err=%lu)",
               GetLastError());
    }
}

void post_wakeup_relationship() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_RELATIONSHIP_APPLY, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_RELATIONSHIP_APPLY) failed (err=%lu)",
               GetLastError());
    }
}

void post_wakeup_time_pass() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_TIME_PASS_APPLY, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_TIME_PASS_APPLY) failed (err=%lu)",
               GetLastError());
    }
}

void post_wakeup_item_grant() noexcept {
    HWND h = g_hwnd.load(std::memory_order_acquire);
    if (!h) return;
    if (!PostMessageW(h, FW_MSG_ITEM_GRANT_APPLY, 0, 0)) {
        FW_DBG("dispatch: PostMessage(FW_MSG_ITEM_GRANT_APPLY) failed (err=%lu)",
               GetLastError());
    }
}
...
void enqueue_quest_stage_apply(const PendingQuestStageOp& op) {
    if (op.quest_form_id == 0) return;
    std::size_t qsize;
    {
        std::lock_guard lk(g_quest_stage_mtx);
        g_quest_stage_queue.push_back(op);
        qsize = g_quest_stage_queue.size();
    }
    FW_DBG("dispatch: quest_stage enqueued form=0x%X stage=%u (qsize=%zu)",
           op.quest_form_id, static_cast<unsigned>(op.stage), qsize);
    post_wakeup_quest_stage();
}

void enqueue_faction_rank_apply(const PendingFactionRankOp& op) {
    if (op.faction_form_id == 0) return;
    std::size_t qsize;
    {
        std::lock_guard lk(g_faction_rank_mtx);
        g_faction_rank_queue.push_back(op);
        qsize = g_faction_rank_queue.size();
    }
    FW_DBG("dispatch: faction_rank enqueued form=0x%X rank=%d (qsize=%zu)",
           op.faction_form_id, op.rank, qsize);
    post_wakeup_faction_rank();
}

void enqueue_party_teleport(const PendingPartyTeleportOp& op) {
    std::size_t qsize;
    {
        std::lock_guard lk(g_party_tp_mtx);
        g_party_tp_queue.push_back(op);
        qsize = g_party_tp_queue.size();
    }
    FW_DBG("dispatch: party_tp enqueued (%.1f,%.1f,%.1f) qsize=%zu",
           op.pos_x, op.pos_y, op.pos_z, qsize);
    post_wakeup_party_tp();
}

void enqueue_xp_grant_apply(const PendingXpGrantOp& op) {
    if (op.amount == 0) return;
    std::size_t qsize;
    {
        std::lock_guard lk(g_xp_grant_mtx);
        g_xp_grant_queue.push_back(op);
        qsize = g_xp_grant_queue.size();
    }
    FW_DBG("dispatch: xp_grant enqueued amount=%d qsize=%zu",
           op.amount, qsize);
    post_wakeup_xp_grant();
}

void enqueue_weather_apply(const PendingWeatherOp& op) {
    if (op.weather_form_id == 0) return;
    std::size_t qsize;
    {
        std::lock_guard lk(g_weather_mtx);
        g_weather_queue.push_back(op);
        qsize = g_weather_queue.size();
    }
    FW_DBG("dispatch: weather enqueued form=0x%X qsize=%zu",
           op.weather_form_id, qsize);
    post_wakeup_weather();
}

void enqueue_companion_apply(const PendingCompanionOp& op) {
    if (op.actor_form_id == 0) return;
    std::size_t qsize;
    {
        std::lock_guard lk(g_companion_mtx);
        g_companion_queue.push_back(op);
        qsize = g_companion_queue.size();
    }
    FW_DBG("dispatch: companion enqueued actor=0x%X qsize=%zu",
           op.actor_form_id, qsize);
    post_wakeup_companion();
}

void enqueue_cell_cleared_apply(const PendingCellClearedOp& op) {
    if (op.cell_form_id == 0) return;
    std::size_t qsize;
    {
        std::lock_guard lk(g_cell_cleared_mtx);
        g_cell_cleared_queue.push_back(op);
        qsize = g_cell_cleared_queue.size();
    }
    FW_DBG("dispatch: cell_cleared enqueued cell=0x%X cleared=%u qsize=%zu",
           op.cell_form_id, static_cast<unsigned>(op.cleared), qsize);
    post_wakeup_cell_cleared();
}

void enqueue_relationship_apply(const PendingRelationshipOp& op) {
    if (op.actor_a_form_id == 0 || op.actor_b_form_id == 0) return;
    std::size_t qsize;
    {
        std::lock_guard lk(g_relationship_mtx);
        g_relationship_queue.push_back(op);
        qsize = g_relationship_queue.size();
    }
    FW_DBG("dispatch: relationship enqueued 0x%X->0x%X rank=%d qsize=%zu",
           op.actor_a_form_id, op.actor_b_form_id, op.rank, qsize);
    post_wakeup_relationship();
}

void enqueue_time_pass_apply(const PendingTimePassOp& op) {
    if (op.hours == 0) return;
    std::size_t qsize;
    {
        std::lock_guard lk(g_time_pass_mtx);
        g_time_pass_queue.push_back(op);
        qsize = g_time_pass_queue.size();
    }
    FW_DBG("dispatch: time_pass enqueued hours=%d qsize=%zu",
           op.hours, qsize);
    post_wakeup_time_pass();
}

void enqueue_item_grant_apply(const PendingItemGrantOp& op) {
    if (op.item_form_id == 0 || op.count <= 0) return;
    std::size_t qsize;
    {
        std::lock_guard lk(g_item_grant_mtx);
        g_item_grant_queue.push_back(op);
        qsize = g_item_grant_queue.size();
    }
    FW_DBG("dispatch: item_grant enqueued item=0x%X count=%d qsize=%zu",
           op.item_form_id, op.count, qsize);
    post_wakeup_item_grant();
}
...
void drain_quest_stage_apply_queue() {
    std::deque<PendingQuestStageOp> local;
    {
        std::lock_guard lk(g_quest_stage_mtx);
        local.swap(g_quest_stage_queue);
    }
    if (local.empty()) {
        FW_DBG("dispatch: quest_stage drain with empty queue — no-op");
        return;
    }

    // ApplyingRemoteGuard silences worldstate_hook's SetCurrentStageID
    // detour so our apply does not re-emit QUEST_STAGE_SET.
    fw::hooks::ApplyingRemoteGuard guard;

    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::apply_quest_stage(
            op.quest_form_id, op.stage);
        if (ok) ++applied_ok; else ++failed;
    }
    FW_LOG("dispatch: drained %zu quest_stage ops (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}

void drain_faction_rank_apply_queue() {
    std::deque<PendingFactionRankOp> local;
    {
        std::lock_guard lk(g_faction_rank_mtx);
        local.swap(g_faction_rank_queue);
    }
    if (local.empty()) {
        FW_DBG("dispatch: faction_rank drain with empty queue — no-op");
        return;
    }
    ...
    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::apply_faction_rank(
            op.faction_form_id, op.rank);
        if (ok) ++applied_ok; else ++failed;
    }
    FW_LOG("dispatch: drained %zu faction_rank ops (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}

void drain_party_teleport_queue() {
    std::deque<PendingPartyTeleportOp> local;
    {
        std::lock_guard lk(g_party_tp_mtx);
        local.swap(g_party_tp_queue);
    }
    if (local.empty()) return;

    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::player_teleport_to(
            op.pos_x, op.pos_y, op.pos_z, op.yaw_rad);
        if (ok) {
            ++applied_ok;
            // Best-effort: drag known teammates along so they are not
            // stranded in the old cell after F8 / PARTY_WARP.
            fw::engine::pull_local_companions_to_player(
                op.pos_x, op.pos_y, op.pos_z, op.yaw_rad);
        } else {
            ++failed;
        }
    }
    FW_LOG("dispatch: drained %zu party_tp (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}

void drain_xp_grant_apply_queue() {
    std::deque<PendingXpGrantOp> local;
    {
        std::lock_guard lk(g_xp_grant_mtx);
        local.swap(g_xp_grant_queue);
    }
    if (local.empty()) return;

    // Silence RewardPlayerXP observe hook while applying remote grants.
    fw::hooks::ApplyingRemoteGuard guard;

    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::reward_player_xp(op.amount, /*suppress=*/true);
        if (ok) ++applied_ok; else ++failed;
    }
    FW_LOG("dispatch: drained %zu xp_grant (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}

void drain_weather_apply_queue() {
    std::deque<PendingWeatherOp> local;
    {
        std::lock_guard lk(g_weather_mtx);
        local.swap(g_weather_queue);
    }
    if (local.empty()) return;
    ...
    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::apply_weather(op.weather_form_id);
        if (ok) ++applied_ok; else ++failed;
    }
    FW_LOG("dispatch: drained %zu weather (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}

void drain_companion_apply_queue() {
    std::deque<PendingCompanionOp> local;
    {
        std::lock_guard lk(g_companion_mtx);
        local.swap(g_companion_queue);
    }
    if (local.empty()) return;
    ...
    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::apply_player_teammate(
            op.actor_form_id, op.teammate != 0, op.can_do_favor != 0);
        if (ok) ++applied_ok; else ++failed;
    }
    FW_LOG("dispatch: drained %zu companion (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}

void drain_cell_cleared_apply_queue() {
    std::deque<PendingCellClearedOp> local;
    {
        std::lock_guard lk(g_cell_cleared_mtx);
        local.swap(g_cell_cleared_queue);
    }
    if (local.empty()) return;
    ...
    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::apply_cell_cleared(
            op.cell_form_id, op.cleared != 0);
        if (ok) ++applied_ok; else ++failed;
    }
    FW_LOG("dispatch: drained %zu cell_cleared (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}

void drain_relationship_apply_queue() {
    std::deque<PendingRelationshipOp> local;
    {
        std::lock_guard lk(g_relationship_mtx);
        local.swap(g_relationship_queue);
    }
    if (local.empty()) return;
    ...
    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::apply_relationship_rank(
            op.actor_a_form_id, op.actor_b_form_id, op.rank);
        if (ok) ++applied_ok; else ++failed;
    }
    FW_LOG("dispatch: drained %zu relationship (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}

void drain_time_pass_apply_queue() {
    std::deque<PendingTimePassOp> local;
    {
        std::lock_guard lk(g_time_pass_mtx);
        local.swap(g_time_pass_queue);
    }
    if (local.empty()) return;

    // apply_pass_time writes TESGlobal values directly (no SetValue hook),
    // but guard anyway so any nested engine callbacks stay quiet.
    fw::hooks::ApplyingRemoteGuard guard;
    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::apply_pass_time(op.hours);
        if (ok) ++applied_ok; else ++failed;
    }
    FW_LOG("dispatch: drained %zu time_pass (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}

void drain_item_grant_apply_queue() {
    std::deque<PendingItemGrantOp> local;
    {
        std::lock_guard lk(g_item_grant_mtx);
        local.swap(g_item_grant_queue);
    }
    if (local.empty()) return;
    ...
    std::size_t applied_ok = 0, failed = 0;
    for (const auto& op : local) {
        const bool ok = fw::engine::apply_player_item_grant(
            op.item_form_id, op.count, op.silent != 0);
        if (ok) ++applied_ok; else ++failed;
    }
    FW_LOG("dispatch: drained %zu item_grant (applied=%zu failed=%zu)",
           local.size(), applied_ok, failed);
}
...
void set_target_hwnd(HWND hwnd) {
    ...
    // B4 QuestStage: flush bootstrap / early BCAST stages that arrived
    // before WndProc subclass (same race class as lock bootstrap).
    std::size_t pending_quest = 0;
    {
        std::lock_guard lk(g_quest_stage_mtx);
        pending_quest = g_quest_stage_queue.size();
    }
    if (pending_quest > 0) {
        FW_LOG("dispatch: flushing %zu pre-hwnd queued quest_stage ops",
               pending_quest);
        post_wakeup_quest_stage();
    }
    std::size_t pending_faction = 0;
    {
        std::lock_guard lk(g_faction_rank_mtx);
        pending_faction = g_faction_rank_queue.size();
    }
    if (pending_faction > 0) {
        FW_LOG("dispatch: flushing %zu pre-hwnd queued faction_rank ops",
               pending_faction);
        post_wakeup_faction_rank();
    }
    std::size_t pending_tp = 0;
    {
        std::lock_guard lk(g_party_tp_mtx);
        pending_tp = g_party_tp_queue.size();
    }
    if (pending_tp > 0) {
        FW_LOG("dispatch: flushing %zu pre-hwnd queued party_tp ops",
               pending_tp);
        post_wakeup_party_tp();
    }
    std::size_t pending_xp = 0;
    {
        std::lock_guard lk(g_xp_grant_mtx);
        pending_xp = g_xp_grant_queue.size();
    }
    if (pending_xp > 0) {
        FW_LOG("dispatch: flushing %zu pre-hwnd queued xp_grant ops",
               pending_xp);
        post_wakeup_xp_grant();
    }
}
