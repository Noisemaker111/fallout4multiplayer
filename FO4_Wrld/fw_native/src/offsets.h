// Reverse-engineered offsets for Fallout 4 1.10.163 (classic / pre-NG).
// Originally derived on 1.11.191 (next-gen); ported via CommonLibF4 FIELD
// layouts, F4SE Address Library IDs, and PE-verified matcher rows.
// See docs/START_HERE.md and docs/port_worklist_minimal.csv.
// All code/data values are RVAs relative to Fallout4.exe module base
// (resolved at runtime via GetModuleHandleW). Struct FIELD offsets that
// CommonLibF4 confirmed are unchanged across the port.
...
constexpr std::uintptr_t KILL_ENGINE_RVA       = 0x00C6128D; // 1.10.163 .pdata start
// 1.10.163: free function — ecx=formID, locks allFormsMapLock, probes
// allForms hash, returns TESForm* (or null). AddressLib allForms=0x058D36C8.
constexpr std::uintptr_t LOOKUP_BY_FORMID_RVA  = 0x00152C90; // 1.10.163 disasm (was NG 0x311850)

// TESObjectREFR vtable. vt[0x7A] = AddObjectToContainer (converged entry
// for player-container transfers in both directions).
constexpr std::uintptr_t TESOBJECTREFR_VTABLE_RVA = 0x02C87AE8;
constexpr std::size_t    VT_ADD_TO_CONTAINER_SLOT = 0x7A;

// Player singleton (points to the active PlayerCharacter Actor*).
constexpr std::uintptr_t PLAYER_SINGLETON_RVA  = 0x05AA4388;
...
// 1.10.163: engine GetItemCount worker (papyrus thin wrapper → this).
constexpr std::uintptr_t INVITEM_GET_COUNT_RVA    = 0x013FB700; // 1.10.163 disasm

// --- B3.b main menu auto-load hook ---
// sub_140BBC050 is the MainMenu Scaleform registrar: binds AS3 callbacks
...
constexpr std::uintptr_t MAIN_MENU_REGISTRAR_RVA  = 0x00BBC050;

// --- B4 world-state sync (quest stages + global variables) ---
//
// Re-derived on classic 1.10.163 (Fallout4.exe.unpacked.exe) 2026-08-03 via
// Papyrus native registration LEA scan (class name string → callback LEA):
//
// GlobalVariable.SetValue (RVA 0x13F0A40):
//   Signature: uint8_t __fastcall(VM*, uint32_t stack_id, TESGlobal*, float)
//   MS x64: rcx=VM, rdx=stack_id, r8=TESGlobal*, xmm3=value (4th arg float)
//   Gate:    flags at TESGlobal+0x10 bit 6 (0x40) = const → refuse write
//   Action:  movss [r8+0x30], xmm3
//   GetValue leaf at 0x13F0CF0 is `movss xmm0,[r8+0x30]; ret` (not hooked).
//
// TESGlobal field offsets (class TESGlobal : TESForm):
//   +0x10  TESForm::flags (u32)
//   +0x14  TESForm::formID (u32)
//   +0x30  TESGlobal::value (f32)
//
// Quest.SetCurrentStageID (RVA 0x1449550):
//   Signature: char __fastcall(VM*, uint32_t stack_id, TESQuest*, uint32_t stage)
//   MS x64: rcx=VM, rdx=stack_id, r8=TESQuest*, r9=stage
//   Calls TLS-affine stage workers (gs:[58h] TEB TLS) then quest helper
//   0x5D71E0 which writes current stage at TESQuest+0x2B4 (u16) and
//   dispatches stage fragments. Cross-thread call is unsafe — receive
//   apply MUST go through the main-thread WndProc dispatcher.
//
// Prior RVAs 0x11459E0 / 0x1185DD0 were mid-function / wrong for 1.10.163
// (leftover from an incomplete next-gen port pass).
constexpr std::uintptr_t PAPYRUS_GLOBALVAR_SETVALUE_RVA = 0x013F0A40;
constexpr std::uintptr_t PAPYRUS_QUEST_SETSTAGE_RVA     = 0x01449550;

// B6.7 / Tier B1 — Actor.SetFactionRank Papyrus native (1.10.163).
// Registration LEA scan: class Actor method "SetFactionRank" → 0x138AC30.
// Signature (MSVC x64):
//   void/char __fastcall(VM*, uint32 stack_id, Actor* self,
//                        TESFaction* faction, int rank /* 5th = [rsp+28] */)
// Tail-calls worker 0xDA3AC0(Actor* rcx, TESFaction* rdx, int rank r8).
// Observe on the Papyrus entry; apply via the worker on main thread.
constexpr std::uintptr_t PAPYRUS_ACTOR_SETFACTIONRANK_RVA = 0x0138AC30;
constexpr std::uintptr_t ACTOR_SETFACTIONRANK_WORKER_RVA  = 0x00DA3AC0;

// A4 — Game.RewardPlayerXP (1.10.163). Registration LEA → 0x13C1EA0.
// Signature: void(VM*, uint32 stack_id, StaticFunctionTag*, int amount,
//                 bool suppressNotifications /* 5th stack */)
constexpr std::uintptr_t PAPYRUS_REWARD_PLAYER_XP_RVA = 0x013C1EA0;

// B6.11 — well-known Fallout4.esm GlobalVariable formIDs (TESGlobal).
// Polled and pushed via the existing GLOBAL_VAR_SET path for time sync.
constexpr std::uint32_t GLOBAL_FORM_GAME_HOUR        = 0x00000038;
constexpr std::uint32_t GLOBAL_FORM_GAME_DAYS_PASSED = 0x00000039;
constexpr std::uint32_t GLOBAL_FORM_TIME_SCALE       = 0x0000003A;

// A4 — proximity XP: peers within this distance (game units ≈ inches) share
// kill/quest XP grants. ~4096 u ≈ 100 m-ish FO4 exterior scale; tune later.
constexpr float PROXIMITY_XP_RADIUS_UNITS = 4096.0f;

// B6.11 — Sky / weather (1.10.163).
// GetCurrentWeather: call sky getter → *(Sky*+0x48) = TESWeather*.
// Weather.SetActive papyrus (0x144F950) tails to worker 0x651A60:
//   void(Sky* rcx, TESWeather* rdx, bool abOverride r8, bool abAccelerate r9)
constexpr std::uintptr_t SKY_GETTER_RVA                 = 0x00128CB0;
constexpr std::size_t    SKY_CURRENT_WEATHER_OFF        = 0x48;
constexpr std::uintptr_t PAPYRUS_WEATHER_SETACTIVE_RVA  = 0x0144F950;
constexpr std::uintptr_t WEATHER_SETACTIVE_WORKER_RVA   = 0x00651A60;

// B6.8 — Actor.SetPlayerTeammate (1.10.163). Registration → 0x1391550 thunk
// → worker 0xDA7170(Actor*, bool teammate, bool canDoFavor, bool unused?).
constexpr std::uintptr_t PAPYRUS_SET_PLAYER_TEAMMATE_RVA = 0x01391550;
constexpr std::uintptr_t ACTOR_SET_PLAYER_TEAMMATE_WORKER_RVA = 0x00DA7170;

// B6.9 — Cell cleared flag (1.10.163).
// IsCleared Papyrus leaf: movzx eax, byte [TESObjectCELL+0x134]
// SetCleared worker 0x37F130: mov [cell+0x134], dl  (+ side effects at +0x135)
// Papyrus thunk 0x13AB820: (VM*, stackId, Cell*, bool cleared)
constexpr std::size_t    TESOBJECTCELL_CLEARED_OFF       = 0x134;  // u8
constexpr std::uintptr_t PAPYRUS_CELL_SETCLEARED_RVA     = 0x013AB820;
constexpr std::uintptr_t CELL_SETCLEARED_WORKER_RVA      = 0x0037F130;

// Narrative polish — Actor.SetRelationshipRank (1.10.163 @ 0x138B150).
// Signature (typical): (VM*, stackId, Actor* self, Actor* other, int rank)
constexpr std::uintptr_t PAPYRUS_SET_RELATIONSHIP_RANK_RVA = 0x0138B150;

// Game.PassTime (Wait/Sleep funnel) — thin thunk @ 0x13C1B70 → worker.
// Observe hours and immediately rebroadcast GameHour (and optional TIME_PASS).
constexpr std::uintptr_t PAPYRUS_PASS_TIME_RVA = 0x013C1B70;
// Multiplayer policy: block PassTime longer than this many game-hours unless
// we fan-out (we fan-out GameHour via existing GLOBAL_VAR path after pass).
constexpr int            PASS_TIME_MAX_HOURS_UNBLOCKED = 24;

// ObjectReference.AddItem Papyrus native (1.10.163).
// Registration LEA r9 → 0x1402B40 (class ObjectReference, name AddItem).
// Signature: (VM*, stackId, REFR* self, TESForm* item, int count, bool silent)
// Policy: emit ITEM_GRANT only when self is local PC, abSilent==true,
// count in [1,99], item != caps (0xF). Covers quest-script rewards without
// mirroring open-world loot UI paths (those use silent=false).
constexpr std::uintptr_t PAPYRUS_OBJECTREF_ADDITEM_RVA = 0x01402B40;
constexpr std::uint32_t  FORM_CAPS_SEPTIMS             = 0x0000000F;
constexpr int            ITEM_GRANT_MAX_COUNT          = 99;
// FO4 TESForm formType byte: KEYM / TESKey (CommonLibF4 TESForms.h kKEYM = 0x2F).
constexpr std::uint8_t   FORMTYPE_KEY                  = 0x2F;

// B6.2 — Activate form types beyond doors (toggle world objects).
// Empirical door set (B6.0) kept; expanded for light switches / generators
// that share the Activate worker. Exact LIGH enum varies; liberal filter
// is OK because non-toggle activates still just re-Activate on the peer.
constexpr std::uint8_t FORMTYPE_DOOR_A     = 0x1F;
constexpr std::uint8_t FORMTYPE_ACTI_A     = 0x20;
constexpr std::uint8_t FORMTYPE_ACTI_B     = 0x24;
constexpr std::uint8_t FORMTYPE_DOOR_B     = 0x29;
constexpr std::uint8_t FORMTYPE_LIGH_CAND  = 0x21;  // candidate (MISC-adjacent)
constexpr std::uint8_t FORMTYPE_FURN       = 0x28;  // furniture / some interactables

constexpr std::size_t    TESGLOBAL_VALUE_OFF            = 0x30;
constexpr std::uint32_t  TESGLOBAL_FLAG_CONST           = 0x40;
// TESQuest::currentStage (u16) — written by stage helper 0x5D71E0.
constexpr std::size_t    TESQUEST_CURRENT_STAGE_OFF     = 0x2B4;
...
constexpr std::uintptr_t TESFORM_GET_COMPONENT_RVA     = 0x00313560; // 1.10.163 fn start (was mid+0x10 NG)
...
// withdraw, which ALSO uses vt[0xEC] via sub_1412640B0):
...
constexpr std::uintptr_t REFHANDLE_RESOLVE_RVA            = 0x0000AC90;  // 1.10.163 AddressLib GetSmartPointer
...
constexpr std::uintptr_t INV_ENTRY_TO_FORM_RVA     = 0x00347870; // 1.10.163 fn start
constexpr std::uintptr_t FORM_CACHE_SINGLETON_RVA  = 0x058D36C8;   // 1.10.163 AddressLib allForms map
...
constexpr std::uintptr_t ENGINE_REMOVE_ITEM_RVA = 0x011824F0; // 1.10.163 fn start
...
constexpr std::uintptr_t ENGINE_ACTIVATE_WORKER_RVA = 0x00514170; // 1.10.163 fn start
...
//   - lockpick minigame success  → sub_14129D520 → ForceUnlock
...
constexpr std::uintptr_t ENGINE_LOCK_PAPYRUS_APPLY_RVA = 0x01158600; // 1.10.163 fn start
...
// Live callers invoke the wrapper sub_140CED1D0 (not the inner
// sub_140CED400). Wrapper does a readiness check then LoadGame.
// Calling the inner leaf alone SEHs at 0xCDA466 on 1.10.163.
constexpr std::uintptr_t LOAD_GAME_FN_RVA          = 0x00CED1D0;   // 1.10.163 wrapper
// 1.10.163 live RE (2026-08-02): every caller of LoadGame wrapper
// (sub_140CED1D0) does `mov rcx, [rip+disp]` → this global holds the
// BGSSaveLoadManager*. Confirmed live heap ptr at main menu.
// Old 0x0329D508 was NG-era / wrong — read as packed garbage.
constexpr std::uintptr_t SAVE_LOAD_MGR_SINGLETON_RVA = 0x05A735C8;
// Was 0x01084769 (.pdata start) — live smoke AV'd on first insn when
// called as precond (WRITE at SAVE_DEV+0x18). Worklist/port NOVEL target
// is sub_141084830 = 0x01084830 (true prologue). Still soft-fail if wrong.
constexpr std::uintptr_t LOAD_PRECOND_FN_RVA       = 0x01084830; // 1.10.163
// SAVE_DEV still unconfirmed on 1.10.163 — LoadGame path skips precond
// when this slot is not a plausible pointer.
constexpr std::uintptr_t SAVE_DEV_SINGLETON_RVA    = 0x031E5A90;   // TODO re-derive
constexpr std::uintptr_t LOAD_PREP_FN_RVA          = 0x00C37140; // 1.10.163 (may be soft-wrong)
constexpr std::uintptr_t LOAD_IN_PROGRESS_FLAG_RVA = 0x032D1FEA;   // byte_1432D1FEA (suspect)
...
// 1.10.163: .pdata start of containing fn (NG was mid+0x67).
constexpr std::uintptr_t SCENE_RENDER_RVA            = 0x00C38F19;

// --- B5 camera capture (Agent 1 RE, 2026-04-22) ---
constexpr std::uintptr_t PLAYER_CAMERA_SINGLETON_RVA = 0x058CEB28;
constexpr std::uintptr_t NI_CAMERA_VTABLE_RVA        = 0x02E15E38;
...
constexpr std::uintptr_t MAIN_CULLING_CAMERA_VTABLE_RVA   = 0x02D3BED8; // 1.10.163 AddressLib __MainCullingCamera
...
//
// 1.10.163: Papyrus ObjectReference.PlaceAtMe native registered at
// console/native table site 0x01414C94 (r9 = handler). Name string
// still 0x02CB0B70. Signature (from re/placeatme_calling_convention.txt):
//   void* __fastcall(
...
constexpr std::uintptr_t PLACE_AT_ME_RVA = 0x0140B0E0; // 1.10.163 disasm (was NG 0x1159C10)
...
// 1.10.163: free wrappers (holder, BSFixedString* name, value) — 0x20 apart
// same order as NG. BOOL stores r8b; INT stores r8d; FLOAT stores xmm2.
constexpr std::uintptr_t SET_GRAPH_VAR_BOOL_RVA     = 0x0081D3D0; // 1.10.163 disasm (was NG 0x818D60)
constexpr std::uintptr_t SET_GRAPH_VAR_INT_RVA      = 0x0081D3F0; // 1.10.163 disasm (was NG 0x818D80)
constexpr std::uintptr_t SET_GRAPH_VAR_FLOAT_RVA    = 0x0081D410; // 1.10.163 disasm (was NG 0x818DA0)
// Same as FIXEDSTR_CREATE_RVA in ni_offsets.h (intern cstr → handle).
constexpr std::uintptr_t BSFIXEDSTRING_MAKE_RVA     = 0x01B41E70; // 1.10.163 (was NG 0x167BDC0)
...
// 1.10.163: fn start (NG mid-function +0xA0).
constexpr std::uintptr_t ENGINE_ENABLE_AI_RVA = 0x00C764F0;
...
// Safe.
// 1.10.163: Actor multi-arg MoveTo (pos*, yaw xmm2, pitch xmm3, cell, marker, flags).
// Called from ObjectReference.MoveTo path with stack bools; near vt[202] SetPos.
constexpr std::uintptr_t ACTOR_ATOMIC_TELEPORT_RVA = 0x00D77C50; // 1.10.163 disasm (was NG 0xC60BE0)
...
constexpr std::uintptr_t NIAV_SET_MOTION_TYPE_RVA = 0x01875854; // 1.10.163 .pdata start
...
constexpr std::uintptr_t AIM_SET_TARGET_RVA = 0x00E657F0; // 1.10.163 fn start
...
// HighProcess.
constexpr std::uintptr_t WEAPON_STATE_CLASS_SETTER_RVA = 0x00CD1820; // 1.10.163 fn start
...
constexpr std::uintptr_t ENTER_COMBAT_RVA = 0x00CCF5B0; // 1.10.163 .pdata start
// ============================================================================
...
constexpr std::uintptr_t COMBAT_PROMOTER_FRESH_RVA = 0x00ED2380; // 1.10.163 fn start
...
constexpr std::uintptr_t COMBAT_AIM_CONTROLLER_CTOR_RVA = 0x00E65380; // 1.10.163 fn start
...
// 1.10.163: free REFR SetPosition(refr, float pos[3]) — writes +0xD0/D4/D8,
// cell-grid remove/readd. Called first from Actor::SetPosition vt[202].
constexpr std::uintptr_t SETPOSITION_NIPOINT3_RVA = 0x0040C060; // 1.10.163 disasm (was NG 0x513A80)
...
// 1.10.163: Actor/PC vt[202] live slot (AddressLib Actor vt 0x02D3F3B8).
constexpr std::uintptr_t ACTOR_SETPOS_VT202_RVA = 0x00D77690; // 1.10.163 vtable (was NG 0xC60630)
...
constexpr std::uintptr_t AIPROCESS_SET_COMBAT_TARGET_RVA = 0x0087AB00; // 1.10.163 fn start
...
// as in-cell → no crash.
// 1.10.163: .pdata start (NG mid+0x95).
constexpr std::uintptr_t COMBAT_ENGAGE_GATE_RVA = 0x00CF606B; // 1.10.163 .pdata start
...
constexpr std::uintptr_t WEAPON_FIRE_HANDLER_RVA = 0x00DFF4D0; // 1.10.163 .pdata start
...
// disabled state).
constexpr std::uintptr_t MOVCTRL_SET_DISABLED_RVA = 0x00DC7F28; // 1.10.163 .pdata start
...
//     v3 = sub_141B0EE10(&unk_143E5E0F0, 0xE10u, 0x10u, 1);  // alloc
...
//   - sub_141B0EE10 (funcs_0471.md:6687) is the generic Bethesda
...
constexpr std::uintptr_t MEMMGR_ALLOC_RVA         = 0x01B0EE10; // sub_141B0EE10
...
constexpr std::uintptr_t REFR_SETPOSITION_LEAF_RVA = 0x0040C060; // 1.10.163 (= SETPOSITION_NIPOINT3)
...
constexpr std::uintptr_t HANDLE_GET_OR_ALLOC_RVA   = 0x0000AAC0; // 1.10.163 AddressLib GetHandle
...
//
// Implementation:
//   - block = sub_141B0EE10(&unk_143E5E0F0, 0x28, 0, 0)  — alloc 40-byte selector
...
constexpr std::uintptr_t ADD_COMBAT_TARGET_RVA          = 0x00E91D30; // 1.10.163 fn start
constexpr std::uintptr_t FIXED_SELECTOR_CTOR_RVA        = 0x00EE96D7; // 1.10.163 .pdata start
constexpr std::uintptr_t ENGINE_HEAP_ALLOC_RVA          = 0x01B0EE10; // 1.10.163 matcher-exact
// Static heap descriptor used by the canonical vanilla call site at
// funcs_0358.md:4906: `sub_141B0EE10(&unk_143E5E0F0, 0x28, 0, 0)`.
// Address 0x143E5E0F0 in the static image (image base 0x140000000).
// 1.10.163: same pool descriptor as ni_offsets MEM_POOL_RVA (NG 0x3E5E0F0 dead).
constexpr std::uintptr_t ENGINE_HEAP_DESC_RVA           = 0x038CC980; // 1.10.163 (was NG 0x3E5E0F0)
...
constexpr std::uintptr_t LOCK_RELEASE_WRITE_RVA         = 0x01658F90; // 1.10.163 .pdata start
...
constexpr std::uintptr_t COMBATCTRL_ENSURE_KNOWN_RVA     = 0x00E92980; // 1.10.163 fn start
