...
// NPC AI sync is compiled out (see fw_native/minimal_exclude.txt). Callers in
// engine_calls.cpp, main_thread_dispatch.cpp and net/client.cpp keep compiling
// against these stubs.
//
// The predicates deliberately answer "no NPC is tracked": with the whole AI
// layer absent there is no server-driven NPC state to freeze against, so every
// actor must take the vanilla path. `get_fid_for_aiproc` returning false means
// callers take their MISS branch, which the real module documents as the safe
// one (passthrough, never a false positive).
inline bool install_npc_ai_suppress(std::uintptr_t) { return false; }

inline std::uint64_t get_npc_ai_suppress_fires()    { return 0; }
inline std::uint64_t get_npc_ai_passthrough_fires() { return 0; }
inline std::uint64_t get_npc_ai_seh_failures()      { return 0; }
inline std::uint64_t get_dyn_bail_tracked_count()   { return 0; }
inline std::uint64_t get_aiproc_map_size()          { return 0; }

inline bool is_actor_bail_tracked(std::uint32_t)    { return false; }
inline bool should_freeze_actor(std::uint32_t)      { return false; }
inline bool should_silence_combat(std::uint32_t)    { return false; }
inline bool get_fid_for_aiproc(const void*, std::uint32_t*) { return false; }

inline void mark_dying(std::uint32_t)  {}
inline bool is_dying(std::uint32_t)    { return false; }
inline void clear_dying(std::uint32_t) {}
