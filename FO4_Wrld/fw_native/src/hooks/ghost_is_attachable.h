...
// Compiled out (see fw_native/minimal_exclude.txt). This bypass only exists to
// let the proxy-Actor spawn survive PlaceAtMe, and that spawn is already dead
// code (`g_proxy_spawn_disabled{true}`, START_HERE §6) — engine_calls.cpp keeps
// compiling against these stubs.
inline void set_force_not_attachable(bool) {}
inline bool install_ghost_is_attachable(std::uintptr_t) { return false; }

inline std::uint64_t get_is_attachable_bypass_count()      { return 0; }
inline std::uint64_t get_is_attachable_passthrough_count() { return 0; }
inline std::uint64_t get_attach_refr_bypass_count()        { return 0; }
inline std::uint64_t get_attach_refr_passthrough_count()   { return 0; }
