...
// Shared-pool HP bar is compiled out (see fw_native/minimal_exclude.txt).
// net/client.cpp keeps compiling against these stubs; the RX handler drops the
// pool value on the floor, which is correct when there is no bar to override.
inline bool install_npc_hp_bar(std::uintptr_t) { return false; }
inline void set_npc_pool_hp(std::uint32_t, float, float) noexcept {}
inline std::uint64_t get_hp_bar_overrides() noexcept { return 0; }
