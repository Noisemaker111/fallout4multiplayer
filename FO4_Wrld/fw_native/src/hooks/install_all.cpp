...
// ============================================================================
// FW_MINIMAL (2026-07-28) — see fw_native/minimal_exclude.txt + START_HERE §5-A.
//
// A minimal build compiles out every subsystem that is not needed for "two
// players see each other move", so the 1.10.163 port does not have to
// re-derive their addresses. Two shapes of guard appear below:
//
//   * Modules whose header carries an `#if FW_MINIMAL` stub block
//     (container, npc_ai_suppress, npc_hp_probe, hp_bar, equip_cycle,
//     equip_hook, ghost_is_attachable) are included and called
//     UNCONDITIONALLY — the stub turns the install into a no-op returning
//     false. Nothing here needs to know.
//   * Everything else is wrapped in `#if !FW_MINIMAL`, include and call
//     together, because there is no stub to fall back on.
//
// InstallSummary flags for absent hooks simply stay false, which is already
// how a failed install reads.
// ============================================================================
...
#include "worldstate_hook.h" // B4: GlobalVar + QuestStage — needed for one-campaign co-op
#include "party_ux_hook.h"   // A2: F8 teleport-to-peer / F9 summon
                           ...
                           const fw::config::Settings& cfg)
{
    ...
    // 2026-08-02: still install under FW_MINIMAL — pure diagnostic, does
    // not mutate game state (returns EXCEPTION_CONTINUE_SEARCH).
...
#endif
    // Release-grade co-op core (always on, including FW_MINIMAL):
    ...
    (void)start_party_ux(module_base);
    ...
    //
    // FW_MINIMAL: still compiled out. "Unconditional" meant "regardless of the
    // Build 64 AI rollback"; the crash class it guards (faction-list walk
    // during a ghost-involved combat teardown) cannot arise with the combat
    // tier absent.
#if !FW_MINIMAL
    ...
    //
    // FW_MINIMAL CAVEAT: this one guards a crash that is NOT ghost-specific —
    // the local player dying is reachable in a minimal build too (vanilla
    // raiders still shoot). It is compiled out only because it shares
    // ghost_hostility_guard.cpp with the combat-tier guards. If a minimal
    // build crashes ~8s after the local player dies, this is the first
    // suspect: split it into its own module and drop it from
    // minimal_exclude.txt (costs 1 address, RVA 0x4CAE90).
    ...
    // override. DISABLED 2026-08-02 on 1.10.163: SCENE_RENDER_RVA was
    // set to a .pdata start (0xC38F19) that is not a safe MinHook entry;
    // live smoke crashed / stalled at main menu after install. Re-enable
    // only after a verified true function prologue on this build.
    // (void)fw::render::install_scene_render_hook(module_base);
    FW_LOG("[scene_hook] skipped at boot — SCENE_RENDER_RVA needs re-derive "
           "on 1.10.163 (previous candidate 0xC38F19)");
    // equip_hook installed earlier with the co-op core set (see top of fn).
    ...
    //
    // FW_MINIMAL: everything from here to the end of the function is M9
    // weapon-mod reverse-engineering instrumentation. It observes and logs;
    // none of it produces gameplay, so a minimal build drops the lot.
#if FW_MINIMAL
    const bool nif_cache_ok = false;
#else
