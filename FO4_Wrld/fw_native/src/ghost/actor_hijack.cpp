...
void on_spawn_message() {
    ...
    // FW_MINIMAL: everything below is unreachable anyway — the check above
    // returns unconditionally, because g_proxy_spawn_disabled is a constant
    // `true`. Compiling it out drops the spawn_ghost_proxy dependency and with
    // it the densest block of pinned addresses in engine_calls.cpp.
#if !FW_MINIMAL
