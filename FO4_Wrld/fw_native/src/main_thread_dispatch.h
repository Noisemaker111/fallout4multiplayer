...
//                                                 not dispatched yet)
//   0x58 = FW_MSG_QUEST_STAGE_APPLY (B4)       — owned here
//   0x59 = FW_MSG_FACTION_RANK_APPLY (B6.7)    — owned here
//   0x5A = FW_MSG_PARTY_TELEPORT (A2)          — owned here
//   0x5B = FW_MSG_XP_GRANT_APPLY (A4)          — owned here
//   0x5C = FW_MSG_WEATHER_APPLY (B6.11)        — owned here
//   0x5D = FW_MSG_COMPANION_APPLY (B6.8)       — owned here
//   0x5E = FW_MSG_CELL_CLEARED_APPLY (B6.9)    — owned here
//   0x5F = FW_MSG_RELATIONSHIP_APPLY (v23)     — owned here
//   0x60 = FW_MSG_TIME_PASS_APPLY (v23)        — owned here
//   0x61 = FW_MSG_ITEM_GRANT_APPLY (v25)       — owned here
//
...
constexpr UINT FW_MSG_QUEST_STAGE_APPLY = WM_APP + 0x58;  // B4 QuestStage receive apply
constexpr UINT FW_MSG_FACTION_RANK_APPLY = WM_APP + 0x59; // B6.7 PC faction rank apply
constexpr UINT FW_MSG_PARTY_TELEPORT     = WM_APP + 0x5A; // A2 party travel
constexpr UINT FW_MSG_XP_GRANT_APPLY     = WM_APP + 0x5B; // A4 proximity XP
constexpr UINT FW_MSG_WEATHER_APPLY      = WM_APP + 0x5C; // B6.11 weather
constexpr UINT FW_MSG_COMPANION_APPLY    = WM_APP + 0x5D; // B6.8 companion
constexpr UINT FW_MSG_CELL_CLEARED_APPLY = WM_APP + 0x5E; // B6.9 cell cleared
constexpr UINT FW_MSG_RELATIONSHIP_APPLY = WM_APP + 0x5F; // v23 relationship
constexpr UINT FW_MSG_TIME_PASS_APPLY    = WM_APP + 0x60; // v23 Wait/Sleep
constexpr UINT FW_MSG_ITEM_GRANT_APPLY   = WM_APP + 0x61; // v25 quest item grant
...
// B4 QuestStage: apply queue for QUEST_STAGE_BCAST + QUEST_STATE_BOOT.
// Net thread enqueues; main thread drains and calls
// fw::engine::apply_quest_stage under ApplyingRemoteGuard.
struct PendingQuestStageOp {
...
// B6.7: PC faction-rank apply queue (FACTION_RANK_BCAST + FACTION_STATE_BOOT).
struct PendingFactionRankOp {
...
// FW_MINIMAL: the NPC combat drains are compiled out. They are the only
// remaining callers of engine_calls' combat tier (enter_combat_raider_vs_ghost,
// synthesize_combat_extension_for_ghost_target, fire_actor_at_target,
// aim_actor_at_target), so guarding them is what frees those addresses.
//
// The enqueue side stays a no-op rather than disappearing: net/client.cpp
// still receives NPC packets from a full-featured server and calls these on
// the net thread. Dropping the payload is the correct behaviour for a client
// that has no NPC AI layer to apply it to.
#if FW_MINIMAL
inline void enqueue_npc_fire(const PendingNPCFire&) {}
inline void drain_npc_fire_queue() {}
#else
...
// FW_MINIMAL: compiled out — the apply path calls engine::kill_actor on a
// server-tracked NPC, which only exists with the AI layer present.
#if FW_MINIMAL
inline void enqueue_npc_death_apply(const PendingNPCDeath&) {}
inline void drain_npc_death_apply_queue() {}
#else
...
// FW_MINIMAL: compiled out — the drain calls engine::trigger_npc_perception to
// make a tracked NPC notice a peer, which presupposes the NPC AI layer.
#if FW_MINIMAL
inline void enqueue_npc_perception_trigger(const PendingNPCPerceptionTrigger&) {}
inline void drain_npc_perception_trigger_queue() {}
#else
...
//
// FW_MINIMAL: the set is only ever populated by npc_fire_with_ghost_aim, which
// is compiled out, so the query is a constant false.
#if FW_MINIMAL
inline bool is_raider_currently_ghost_targeted(std::uint32_t) { return false; }
inline void mark_raider_ghost_targeted(std::uint32_t, std::uint64_t) {}
inline std::size_t ghost_targeted_raiders_count() { return 0; }
#else
