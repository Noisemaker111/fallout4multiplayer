...
DWORD WINAPI init_thread(LPVOID) {
    ...
    // --- Port-completeness gate (1.10.163 retarget) ---
    // Version match alone is not enough while the RVA table is still being
    // re-derived. See version.h PORT_READY and docs/START_HERE.md step H.
    if (!fw::version::PORT_READY) {
        FW_ERR("port incomplete: version.h PORT_READY=false "
               "(offset_audit --minimal is below 95%%). "
               "Staying inert — proxy only, no hooks. "
               "Finish step F (IDA residual) then flip PORT_READY.");
        return 0;
    }

    // 2026-08-02 bisect: set fw_config.ini `boot_proxy_only = 1` to prove the
    // DXGI proxy alone keeps FO4 alive at the main menu. No engine init,
    // MinHook, auto-load, or net. Used only when isolating early AVs.
    if (cfg.boot_proxy_only) {
        FW_LOG("boot_proxy_only=1 — proxy inert mode (no engine/hooks/net). "
               "FO4 should reach main menu unattended.");
        return 0;
    }
    ...
    // 2026-08-02: defer Strada B inject + SPAI + ghost until after the
    // engine has a stable main menu / save load. Early workers posting
    // WM_APP to a half-init HWND (or walking resmgr) race the 1.10.163
    // boot crash window. FW_MINIMAL only needs auto-load + pos + net.
#if !FW_MINIMAL
    ...
    fw::ghost::init(base);
#else
    FW_LOG("[native] Strada B / SPAI / ghost init SKIPPED (FW_MINIMAL boot)");
#endif
