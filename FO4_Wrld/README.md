...
Fallout 4 multiplayer mod (FoM-lite framework).  
**Playable target build today: classic `1.10.163`.** (Older notes still mention next-gen 1.11.191 — the active port and version gate are 1.10.163.)  
...
| `fw_launcher/` | C++ launcher wrapper (`FoM.exe`) + Steam shell: Spacewar lobby, invites, Steam↔UDP tunnel |
...
| **B2-S** Steam invites over Spacewar (AppID 480) | ✅ M1 shipped — lobby + invite + Steam P2P tunnel; friends join across different networks with no IP typing. Protocol v25 unchanged. See [docs/STEAM_SPACEWAR.md](docs/STEAM_SPACEWAR.md) |
| **B3** Auto-load save (delayed LoadGame via WndProc subclass) | ✅ done |
| **B4** Worldstate sync (GlobalVar + QuestStage) | ✅ shipped (2026-08-03) — observe hooks + apply + bootstrap; classic 1.10.163 RVAs re-derived |
...
| ↳ **B6.0** Door open/close sync | ✅ done — Activate worker; **also in FW_MINIMAL** (2026-08-03) |
...
| ↳ **B6.2** Lights toggle sync (lamps, lanterns, generators) | ✅ Activate filter expanded (doors+ACTI+toggle candidates); DOOR_OP wire (2026-08-03) |
...
| ↳ **B6.7** NPC dialogue state + faction joined | 🟡 faction rank shipped (PC SetFactionRank); full dialogue trees later |
| ↳ **B6.8** Companion state (recruited / position) | 🟡 SetPlayerTeammate sync (v21); owner AI / pos mirror still N-branch |
| ↳ **B6.9** Cell-cleared status (no respawn after group clear) | ✅ SetCleared observe + apply + poll + boot (proto v22, 2026-08-03) |
| ↳ **B6.10** One-shot loot pickups (bobbleheads, magazines, holotapes, skill books) | ✅ pickup_hook under MINIMAL → ACTOR_EVENT DISABLE (2026-08-03) |
| ↳ **B6.11** Time of day + weather sync | 🟡 GameHour + weather form (v21); polish TOD edge cases later |
