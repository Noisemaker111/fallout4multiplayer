...
PROTOCOL_VERSION: int = 25  # v25: ITEM_GRANT_* (silent PC AddItem / quest rewards)
...
class MessageType(IntEnum):
    ...
    FACTION_STATE_BOOT    = 0x0024   # v19: server -> client: player faction-rank snapshot (chunked)
    WEATHER_STATE_BOOT    = 0x0025   # v21: server -> client: current weather form
    COMPANION_STATE_BOOT  = 0x0026   # v21: server -> client: companion teammate snapshot
    CELL_CLEARED_BOOT     = 0x0027   # v22: server -> client: cleared-cell snapshot
    RELATIONSHIP_STATE_BOOT = 0x0028  # v24: server -> client: relationship ranks
    ...
    FACTION_RANK_SET   = 0x0421  # v19: client -> server: local PC SetFactionRank
    FACTION_RANK_BCAST = 0x0422  # v19: server -> other peers: PC faction rank changed
    XP_GRANT_SET       = 0x0431  # v20: client -> server: I gained amount XP
    XP_GRANT_BCAST     = 0x0432  # v20: server -> other peers: peer gained XP (proximity apply)
    PARTY_WARP         = 0x0441  # v20: client -> server: summon party to my pos
    PARTY_WARP_BCAST   = 0x0442  # v20: server -> other peers: teleport to pos
    WEATHER_SET        = 0x0451  # v21: client -> server: active weather form changed
    WEATHER_BCAST      = 0x0452  # v21: server -> other peers: set weather
    COMPANION_SET      = 0x0461  # v21: client -> server: SetPlayerTeammate
    COMPANION_BCAST    = 0x0462  # v21: server -> other peers: companion recruit/dismiss
    CELL_CLEARED_SET   = 0x0471  # v22: client -> server: cell cleared flag
    CELL_CLEARED_BCAST = 0x0472  # v22: server -> other peers: cell cleared
    RELATIONSHIP_SET   = 0x0481  # v23: client -> server: SetRelationshipRank
    RELATIONSHIP_BCAST = 0x0482  # v23: server -> other peers
    TIME_PASS_SET      = 0x0491  # v23: client -> server: Game.PassTime hours
    TIME_PASS_BCAST    = 0x0492  # v23: server -> other peers: apply time jump
    ITEM_GRANT_SET     = 0x04A1  # v25: client -> server: silent PC AddItem
    ITEM_GRANT_BCAST   = 0x04A2  # v25: server -> other peers: grant item to PC
...
# =================================================================== B6.7
#
# Player faction-rank sync (Tier B1). Narrative flag, not full dialogue trees.
# One shared campaign: when either peer's local PC joins / ranks a faction
# via Actor.SetFactionRank, all peers apply the same rank on their PC.
# Keyed by TESFaction.formID; last-write-wins. Rank is signed i32 (engine
# uses small ints; -1 is a common "not a member" sentinel from GetFactionRank).

@dataclass(frozen=True, slots=True)
class FactionRankSetPayload:
    """Client -> server: local PC SetFactionRank(faction, rank)."""
    faction_form_id: int   # u32
    rank: int              # i32
    timestamp_ms: int      # u64

    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<IiQ")  # 16 bytes

    def encode(self) -> bytes:
        return self._STRUCT.pack(
            self.faction_form_id, self.rank, self.timestamp_ms)

    @classmethod
    def decode(cls, data: bytes) -> "FactionRankSetPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("FACTION_RANK_SET truncated")
        fid, rank, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(faction_form_id=fid, rank=rank, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class FactionRankBroadcastPayload:
    """Server -> other peers: authoritative PC faction rank."""
    peer_id: str
    faction_form_id: int
    rank: int
    timestamp_ms: int
    ...
    @classmethod
    def decode(cls, data: bytes) -> "FactionRankBroadcastPayload":
        off = MAX_CLIENT_ID_LEN + 1
        if len(data) < off + cls._STRUCT.size:
            raise ProtocolError("FACTION_RANK_BCAST truncated")
        pid = _decode_fixed_string(data, MAX_CLIENT_ID_LEN)
        fid, rank, ts = cls._STRUCT.unpack_from(data, off)
        return cls(peer_id=pid, faction_form_id=fid, rank=rank, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class FactionRankStateEntry:
    faction_form_id: int
    rank: int


@dataclass(frozen=True, slots=True)
class FactionStateBootPayload:
    """Server -> client: chunked PC faction-rank snapshot on join.

    Wire: ChunkHeader (HHH) + per entry (I i = 8 bytes).
    """
    entries: tuple[FactionRankStateEntry, ...]
    chunk_index: int = 0
    total_chunks: int = 1

    _ENTRY_STRUCT: ClassVar[struct.Struct] = struct.Struct("<Ii")
    _HEADER_STRUCT: ClassVar[struct.Struct] = struct.Struct("<HHH")

    MAX_ENTRIES_PER_FRAME: ClassVar[int] = (
        (MAX_PAYLOAD_SIZE - 6) // 8  # 174 entries per chunk at MTU 1400
    )

    def encode(self) -> bytes:
        if len(self.entries) > self.MAX_ENTRIES_PER_FRAME:
            raise ProtocolError(
                f"FACTION_STATE_BOOT too many entries: {len(self.entries)} > "
                f"{self.MAX_ENTRIES_PER_FRAME}"
            )
        parts = [self._HEADER_STRUCT.pack(
            len(self.entries), self.chunk_index, self.total_chunks)]
        for e in self.entries:
            parts.append(self._ENTRY_STRUCT.pack(e.faction_form_id, e.rank))
        return b"".join(parts)

    @classmethod
    def decode(cls, data: bytes) -> "FactionStateBootPayload":
        if len(data) < cls._HEADER_STRUCT.size:
            raise ProtocolError("FACTION_STATE_BOOT header truncated")
        num, chunk_idx, total = cls._HEADER_STRUCT.unpack_from(data, 0)
        expected = cls._HEADER_STRUCT.size + num * cls._ENTRY_STRUCT.size
        if len(data) < expected:
            raise ProtocolError(
                f"FACTION_STATE_BOOT payload truncated: got {len(data)}, "
                f"need {expected}")
        entries = []
        off = cls._HEADER_STRUCT.size
        for _ in range(num):
            fid, rank = cls._ENTRY_STRUCT.unpack_from(data, off)
            entries.append(FactionRankStateEntry(faction_form_id=fid, rank=rank))
            off += cls._ENTRY_STRUCT.size
        return cls(entries=tuple(entries), chunk_index=chunk_idx, total_chunks=total)
...
# =================================================================== A4 XP
#
# Proximity XP: when a peer gains XP via Game.RewardPlayerXP, fan-out the
# amount. Receivers apply only if within PROXIMITY radius of the sender
# (checked client-side against last known POS). No server persistence —
# XP is personal; only the grant *event* is shared.

@dataclass(frozen=True, slots=True)
class XpGrantSetPayload:
    """Client -> server: local PC just gained `amount` XP."""
    amount: int            # i32
    timestamp_ms: int      # u64

    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<iQ")  # 12 bytes

    def encode(self) -> bytes:
        return self._STRUCT.pack(self.amount, self.timestamp_ms)

    @classmethod
    def decode(cls, data: bytes) -> "XpGrantSetPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("XP_GRANT_SET truncated")
        amount, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(amount=amount, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class XpGrantBroadcastPayload:
    """Server -> other peers: peer X gained amount XP (apply if nearby)."""
    peer_id: str
    amount: int
    timestamp_ms: int
    ...
    def encode(self) -> bytes:
        return (
            _encode_fixed_string(self.peer_id, MAX_CLIENT_ID_LEN)
            + self._STRUCT.pack(self.amount, self.timestamp_ms)
        )

    @classmethod
    def decode(cls, data: bytes) -> "XpGrantBroadcastPayload":
        off = MAX_CLIENT_ID_LEN + 1
        if len(data) < off + cls._STRUCT.size:
            raise ProtocolError("XP_GRANT_BCAST truncated")
        pid = _decode_fixed_string(data, MAX_CLIENT_ID_LEN)
        amount, ts = cls._STRUCT.unpack_from(data, off)
        return cls(peer_id=pid, amount=amount, timestamp_ms=ts)


# =================================================================== A2 party warp
#
# F9 summon: sender broadcasts current pos; other peers MoveTo it.
# Not persisted. Coordinates are FO4 world units; yaw_rad Bethesda.

@dataclass(frozen=True, slots=True)
class PartyWarpPayload:
    """Client -> server: summon party to my position."""
    x: float
    y: float
    z: float
    yaw_rad: float
    ...
    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<ffffIQ")  # 28 bytes

    def encode(self) -> bytes:
        return self._STRUCT.pack(
            self.x, self.y, self.z, self.yaw_rad,
            self.cell_id, self.timestamp_ms)

    @classmethod
    def decode(cls, data: bytes) -> "PartyWarpPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("PARTY_WARP truncated")
        x, y, z, yaw, cell, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(x=x, y=y, z=z, yaw_rad=yaw, cell_id=cell, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class PartyWarpBroadcastPayload:
    """Server -> other peers: teleport to sender's pos."""
    peer_id: str
    x: float
    y: float
    z: float
    ...
    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<ffffIQ")
    ...
    @classmethod
    def decode(cls, data: bytes) -> "PartyWarpBroadcastPayload":
        off = MAX_CLIENT_ID_LEN + 1
        if len(data) < off + cls._STRUCT.size:
            raise ProtocolError("PARTY_WARP_BCAST truncated")
        pid = _decode_fixed_string(data, MAX_CLIENT_ID_LEN)
        x, y, z, yaw, cell, ts = cls._STRUCT.unpack_from(data, off)
        return cls(peer_id=pid, x=x, y=y, z=z, yaw_rad=yaw,
                   cell_id=cell, timestamp_ms=ts)


# =================================================================== B6.11 weather
#
# Shared sky: last-write-wins TESWeather formID. Client polls Sky+0x48 and/or
# hooks Weather.SetActive; apply via SetActive worker (override+accelerate).

@dataclass(frozen=True, slots=True)
class WeatherSetPayload:
    weather_form_id: int   # u32
    timestamp_ms: int      # u64

    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<IQ")  # 12 bytes

    def encode(self) -> bytes:
        return self._STRUCT.pack(self.weather_form_id, self.timestamp_ms)

    @classmethod
    def decode(cls, data: bytes) -> "WeatherSetPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("WEATHER_SET truncated")
        fid, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(weather_form_id=fid, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class WeatherBroadcastPayload:
    peer_id: str
    weather_form_id: int
    timestamp_ms: int
    ...
    def encode(self) -> bytes:
        return (
            _encode_fixed_string(self.peer_id, MAX_CLIENT_ID_LEN)
            + self._STRUCT.pack(self.weather_form_id, self.timestamp_ms)
        )

    @classmethod
    def decode(cls, data: bytes) -> "WeatherBroadcastPayload":
        off = MAX_CLIENT_ID_LEN + 1
        if len(data) < off + cls._STRUCT.size:
            raise ProtocolError("WEATHER_BCAST truncated")
        pid = _decode_fixed_string(data, MAX_CLIENT_ID_LEN)
        fid, ts = cls._STRUCT.unpack_from(data, off)
        return cls(peer_id=pid, weather_form_id=fid, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class WeatherStateBootPayload:
    """Single-entry bootstrap (current weather only). Wire: <IQ = 12."""
    weather_form_id: int
    timestamp_ms: int = 0
    ...
    @classmethod
    def decode(cls, data: bytes) -> "WeatherStateBootPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("WEATHER_STATE_BOOT truncated")
        fid, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(weather_form_id=fid, timestamp_ms=ts)
...
# =================================================================== B6.8 companion
#
# SetPlayerTeammate fan-out. Keyed by actor form_id (persistent companions
# have stable plugin formIDs; runtime 0xFF refs still work same-session).

@dataclass(frozen=True, slots=True)
class CompanionSetPayload:
    actor_form_id: int     # u32
    teammate: int          # u8 0/1
    can_do_favor: int      # u8 0/1
    timestamp_ms: int      # u64

    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<IBBQ")  # 14 bytes

    def encode(self) -> bytes:
        return self._STRUCT.pack(
            self.actor_form_id,
            self.teammate & 0xFF,
            self.can_do_favor & 0xFF,
            self.timestamp_ms,
        )

    @classmethod
    def decode(cls, data: bytes) -> "CompanionSetPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("COMPANION_SET truncated")
        fid, tm, favor, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(actor_form_id=fid, teammate=tm, can_do_favor=favor,
                   timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class CompanionBroadcastPayload:
    peer_id: str
    actor_form_id: int
    teammate: int
    can_do_favor: int
    ...
    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<IBBQ")
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CompanionBroadcastPayload":
        off = MAX_CLIENT_ID_LEN + 1
        if len(data) < off + cls._STRUCT.size:
            raise ProtocolError("COMPANION_BCAST truncated")
        pid = _decode_fixed_string(data, MAX_CLIENT_ID_LEN)
        fid, tm, favor, ts = cls._STRUCT.unpack_from(data, off)
        return cls(peer_id=pid, actor_form_id=fid, teammate=tm,
                   can_do_favor=favor, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class CompanionStateEntry:
    actor_form_id: int
    teammate: int
    can_do_favor: int
...
@dataclass(frozen=True, slots=True)
class CompanionStateBootPayload:
    """Chunked companion teammate snapshot. Header HHH + entries IBB = 6 B."""
    entries: tuple[CompanionStateEntry, ...]
    chunk_index: int = 0
    total_chunks: int = 1

    _ENTRY_STRUCT: ClassVar[struct.Struct] = struct.Struct("<IBB")
    _HEADER_STRUCT: ClassVar[struct.Struct] = struct.Struct("<HHH")
    MAX_ENTRIES_PER_FRAME: ClassVar[int] = (MAX_PAYLOAD_SIZE - 6) // 6

    def encode(self) -> bytes:
        if len(self.entries) > self.MAX_ENTRIES_PER_FRAME:
            raise ProtocolError("COMPANION_STATE_BOOT too many entries")
        parts = [self._HEADER_STRUCT.pack(
            len(self.entries), self.chunk_index, self.total_chunks)]
        for e in self.entries:
            parts.append(self._ENTRY_STRUCT.pack(
                e.actor_form_id, e.teammate & 0xFF, e.can_do_favor & 0xFF))
        return b"".join(parts)

    @classmethod
    def decode(cls, data: bytes) -> "CompanionStateBootPayload":
        if len(data) < cls._HEADER_STRUCT.size:
            raise ProtocolError("COMPANION_STATE_BOOT header truncated")
        num, chunk_idx, total = cls._HEADER_STRUCT.unpack_from(data, 0)
        expected = cls._HEADER_STRUCT.size + num * cls._ENTRY_STRUCT.size
        if len(data) < expected:
            raise ProtocolError("COMPANION_STATE_BOOT truncated")
        entries = []
        off = cls._HEADER_STRUCT.size
        for _ in range(num):
            fid, tm, favor = cls._ENTRY_STRUCT.unpack_from(data, off)
            entries.append(CompanionStateEntry(
                actor_form_id=fid, teammate=tm, can_do_favor=favor))
            off += cls._ENTRY_STRUCT.size
        return cls(entries=tuple(entries), chunk_index=chunk_idx,
                   total_chunks=total)


# =================================================================== B6.9 cell cleared
#
# Shared TESObjectCELL "cleared" flag (engine u8 at +0x134). Last-write-wins.
# Prevents one peer's clear from not applying on the other (respawn divergence).

@dataclass(frozen=True, slots=True)
class CellClearedSetPayload:
    cell_form_id: int   # u32
    cleared: int        # u8 0/1
    timestamp_ms: int   # u64

    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<IBQ")  # 13 bytes

    def encode(self) -> bytes:
        return self._STRUCT.pack(
            self.cell_form_id, self.cleared & 0xFF, self.timestamp_ms)

    @classmethod
    def decode(cls, data: bytes) -> "CellClearedSetPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("CELL_CLEARED_SET truncated")
        cid, cl, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(cell_form_id=cid, cleared=cl, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class CellClearedBroadcastPayload:
    peer_id: str
    cell_form_id: int
    cleared: int
    timestamp_ms: int
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CellClearedBroadcastPayload":
        off = MAX_CLIENT_ID_LEN + 1
        if len(data) < off + cls._STRUCT.size:
            raise ProtocolError("CELL_CLEARED_BCAST truncated")
        pid = _decode_fixed_string(data, MAX_CLIENT_ID_LEN)
        cid, cl, ts = cls._STRUCT.unpack_from(data, off)
        return cls(peer_id=pid, cell_form_id=cid, cleared=cl, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class CellClearedStateEntry:
    cell_form_id: int
    cleared: int


@dataclass(frozen=True, slots=True)
class CellClearedBootPayload:
    """Chunked cleared-cell snapshot. Header HHH + entries IB = 5 B."""
    entries: tuple[CellClearedStateEntry, ...]
    chunk_index: int = 0
    total_chunks: int = 1

    _ENTRY_STRUCT: ClassVar[struct.Struct] = struct.Struct("<IB")
    _HEADER_STRUCT: ClassVar[struct.Struct] = struct.Struct("<HHH")
    MAX_ENTRIES_PER_FRAME: ClassVar[int] = (MAX_PAYLOAD_SIZE - 6) // 5

    def encode(self) -> bytes:
        if len(self.entries) > self.MAX_ENTRIES_PER_FRAME:
            raise ProtocolError("CELL_CLEARED_BOOT too many entries")
        parts = [self._HEADER_STRUCT.pack(
            len(self.entries), self.chunk_index, self.total_chunks)]
        for e in self.entries:
            parts.append(self._ENTRY_STRUCT.pack(
                e.cell_form_id, e.cleared & 0xFF))
        return b"".join(parts)

    @classmethod
    def decode(cls, data: bytes) -> "CellClearedBootPayload":
        if len(data) < cls._HEADER_STRUCT.size:
            raise ProtocolError("CELL_CLEARED_BOOT header truncated")
        num, chunk_idx, total = cls._HEADER_STRUCT.unpack_from(data, 0)
        expected = cls._HEADER_STRUCT.size + num * cls._ENTRY_STRUCT.size
        if len(data) < expected:
            raise ProtocolError("CELL_CLEARED_BOOT truncated")
        entries = []
        off = cls._HEADER_STRUCT.size
        for _ in range(num):
            cid, cl = cls._ENTRY_STRUCT.unpack_from(data, off)
            entries.append(CellClearedStateEntry(cell_form_id=cid, cleared=cl))
            off += cls._ENTRY_STRUCT.size
        return cls(entries=tuple(entries), chunk_index=chunk_idx,
                   total_chunks=total)


# =================================================================== v23 narrative polish
#
# Relationship ranks (faction/NPC affinity) and PassTime (Wait/Sleep) so
# story affinity + time jumps stay aligned without full dialogue trees.

@dataclass(frozen=True, slots=True)
class RelationshipSetPayload:
    actor_a_form_id: int   # u32 — usually local PC
    actor_b_form_id: int   # u32 — target NPC
    rank: int              # i32
    timestamp_ms: int      # u64

    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<IIiQ")  # 20 bytes

    def encode(self) -> bytes:
        return self._STRUCT.pack(
            self.actor_a_form_id, self.actor_b_form_id,
            self.rank, self.timestamp_ms)

    @classmethod
    def decode(cls, data: bytes) -> "RelationshipSetPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("RELATIONSHIP_SET truncated")
        a, b, rank, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(actor_a_form_id=a, actor_b_form_id=b, rank=rank,
                   timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class RelationshipBroadcastPayload:
    peer_id: str
    actor_a_form_id: int
    actor_b_form_id: int
    rank: int
    ...
    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<IIiQ")
    ...
    @classmethod
    def decode(cls, data: bytes) -> "RelationshipBroadcastPayload":
        off = MAX_CLIENT_ID_LEN + 1
        if len(data) < off + cls._STRUCT.size:
            raise ProtocolError("RELATIONSHIP_BCAST truncated")
        pid = _decode_fixed_string(data, MAX_CLIENT_ID_LEN)
        a, b, rank, ts = cls._STRUCT.unpack_from(data, off)
        return cls(peer_id=pid, actor_a_form_id=a, actor_b_form_id=b,
                   rank=rank, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class TimePassSetPayload:
    """Client -> server: Game.PassTime(hours) just ran (or was requested)."""
    hours: int             # i32 game hours
    timestamp_ms: int      # u64

    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<iQ")  # 12 bytes

    def encode(self) -> bytes:
        return self._STRUCT.pack(self.hours, self.timestamp_ms)

    @classmethod
    def decode(cls, data: bytes) -> "TimePassSetPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("TIME_PASS_SET truncated")
        hours, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(hours=hours, timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class TimePassBroadcastPayload:
    peer_id: str
    hours: int
    timestamp_ms: int
    ...
    def encode(self) -> bytes:
        return (
            _encode_fixed_string(self.peer_id, MAX_CLIENT_ID_LEN)
            + self._STRUCT.pack(self.hours, self.timestamp_ms)
        )

    @classmethod
    def decode(cls, data: bytes) -> "TimePassBroadcastPayload":
        off = MAX_CLIENT_ID_LEN + 1
        if len(data) < off + cls._STRUCT.size:
            raise ProtocolError("TIME_PASS_BCAST truncated")
        pid = _decode_fixed_string(data, MAX_CLIENT_ID_LEN)
        hours, ts = cls._STRUCT.unpack_from(data, off)
        return cls(peer_id=pid, hours=hours, timestamp_ms=ts)
...
@dataclass(frozen=True, slots=True)
class RelationshipBootPayload:
    """Chunked relationship-rank snapshot. Header HHH + entries IIi = 12 B."""
    entries: tuple[RelationshipStateEntry, ...]
    chunk_index: int = 0
    total_chunks: int = 1

    _ENTRY_STRUCT: ClassVar[struct.Struct] = struct.Struct("<IIi")
    _HEADER_STRUCT: ClassVar[struct.Struct] = struct.Struct("<HHH")
    MAX_ENTRIES_PER_FRAME: ClassVar[int] = (MAX_PAYLOAD_SIZE - 6) // 12

    def encode(self) -> bytes:
        if len(self.entries) > self.MAX_ENTRIES_PER_FRAME:
            raise ProtocolError("RELATIONSHIP_STATE_BOOT too many entries")
        parts = [self._HEADER_STRUCT.pack(
            len(self.entries), self.chunk_index, self.total_chunks)]
        for e in self.entries:
            parts.append(self._ENTRY_STRUCT.pack(
                e.actor_a_form_id, e.actor_b_form_id, e.rank))
        return b"".join(parts)

    @classmethod
    def decode(cls, data: bytes) -> "RelationshipBootPayload":
        if len(data) < cls._HEADER_STRUCT.size:
            raise ProtocolError("RELATIONSHIP_STATE_BOOT header truncated")
        num, chunk_idx, total = cls._HEADER_STRUCT.unpack_from(data, 0)
        expected = cls._HEADER_STRUCT.size + num * cls._ENTRY_STRUCT.size
        if len(data) < expected:
            raise ProtocolError("RELATIONSHIP_STATE_BOOT truncated")
        entries = []
        off = cls._HEADER_STRUCT.size
        for _ in range(num):
            a, b, rank = cls._ENTRY_STRUCT.unpack_from(data, off)
            entries.append(RelationshipStateEntry(
                actor_a_form_id=a, actor_b_form_id=b, rank=rank))
            off += cls._ENTRY_STRUCT.size
        return cls(entries=tuple(entries), chunk_index=chunk_idx,
                   total_chunks=total)


# =================================================================== v25 quest item grants
#
# Silent ObjectReference.AddItem on the local player (quest fragments) so
# unique keys/rewards land on both peers without mirroring open loot.

@dataclass(frozen=True, slots=True)
class ItemGrantSetPayload:
    item_form_id: int    # u32 TESForm of item
    count: int           # i32 amount (1..99)
    silent: int          # u8 — 1 if abSilent
    timestamp_ms: int    # u64

    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<IiBQ")  # 17 bytes

    def encode(self) -> bytes:
        return self._STRUCT.pack(
            self.item_form_id, self.count, self.silent & 0xFF,
            self.timestamp_ms)

    @classmethod
    def decode(cls, data: bytes) -> "ItemGrantSetPayload":
        if len(data) < cls._STRUCT.size:
            raise ProtocolError("ITEM_GRANT_SET truncated")
        fid, cnt, silent, ts = cls._STRUCT.unpack_from(data, 0)
        return cls(item_form_id=fid, count=cnt, silent=silent,
                   timestamp_ms=ts)


@dataclass(frozen=True, slots=True)
class ItemGrantBroadcastPayload:
    peer_id: str
    item_form_id: int
    count: int
    silent: int
    ...
    _STRUCT: ClassVar[struct.Struct] = struct.Struct("<IiBQ")
    ...
    @classmethod
    def decode(cls, data: bytes) -> "ItemGrantBroadcastPayload":
        off = MAX_CLIENT_ID_LEN + 1
        if len(data) < off + cls._STRUCT.size:
            raise ProtocolError("ITEM_GRANT_BCAST truncated")
        pid = _decode_fixed_string(data, MAX_CLIENT_ID_LEN)
        fid, cnt, silent, ts = cls._STRUCT.unpack_from(data, off)
        return cls(peer_id=pid, item_form_id=fid, count=cnt, silent=silent,
                   timestamp_ms=ts)
...
_TYPE_TO_PAYLOAD_CLS: dict[int, type] = {
    ...
    MessageType.FACTION_RANK_SET:      FactionRankSetPayload,
    MessageType.FACTION_RANK_BCAST:    FactionRankBroadcastPayload,
    MessageType.FACTION_STATE_BOOT:    FactionStateBootPayload,
    MessageType.XP_GRANT_SET:          XpGrantSetPayload,
    MessageType.XP_GRANT_BCAST:        XpGrantBroadcastPayload,
    MessageType.PARTY_WARP:            PartyWarpPayload,
    MessageType.PARTY_WARP_BCAST:      PartyWarpBroadcastPayload,
    MessageType.WEATHER_SET:           WeatherSetPayload,
    MessageType.WEATHER_BCAST:         WeatherBroadcastPayload,
    MessageType.WEATHER_STATE_BOOT:    WeatherStateBootPayload,
    MessageType.COMPANION_SET:         CompanionSetPayload,
    MessageType.COMPANION_BCAST:       CompanionBroadcastPayload,
    MessageType.COMPANION_STATE_BOOT:  CompanionStateBootPayload,
    MessageType.CELL_CLEARED_SET:      CellClearedSetPayload,
    MessageType.CELL_CLEARED_BCAST:    CellClearedBroadcastPayload,
    MessageType.CELL_CLEARED_BOOT:     CellClearedBootPayload,
    MessageType.RELATIONSHIP_SET:      RelationshipSetPayload,
    MessageType.RELATIONSHIP_BCAST:    RelationshipBroadcastPayload,
    MessageType.RELATIONSHIP_STATE_BOOT: RelationshipBootPayload,
    MessageType.TIME_PASS_SET:         TimePassSetPayload,
    MessageType.TIME_PASS_BCAST:       TimePassBroadcastPayload,
    MessageType.ITEM_GRANT_SET:        ItemGrantSetPayload,
    MessageType.ITEM_GRANT_BCAST:      ItemGrantBroadcastPayload,
