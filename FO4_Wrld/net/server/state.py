...
@dataclass(slots=True)
class FactionRankState:
    """Authoritative PC rank in one TESFaction (B6.7 / Tier B1).

    Keyed by faction_form_id. Shared campaign: both peers' PlayerCharacters
    hold the same rank. Last-write-wins (join / promote / demote all OK).
    """
    faction_form_id: int
    rank: int = 0
    last_owner_peer_id: Optional[str] = None
...
class RelationshipState:
    """Authoritative Actor.SetRelationshipRank pair (last-write-wins)."""
    actor_a_form_id: int
    actor_b_form_id: int
    rank: int = 0
    last_owner_peer_id: Optional[str] = None
...
class CompanionState:
    """B6.8 — one actor's player-teammate flags (shared campaign)."""
    actor_form_id: int
    teammate: bool = True
    can_do_favor: bool = True
    last_owner_peer_id: Optional[str] = None
    last_update_ms: float = 0.0
...
class ServerState:
    ...
    # B6.7: PC faction ranks, keyed by faction_form_id.
    _factions: dict[int, FactionRankState] = field(default_factory=dict)
    # B6.11: last known TESWeather formID (0 = unset).
    weather_form_id: int = 0
    weather_last_peer_id: Optional[str] = None
    weather_last_update_ms: float = 0.0
    # B6.8: companion teammate flags, keyed by actor_form_id.
    _companions: dict[int, CompanionState] = field(default_factory=dict)
    # B6.9: cell cleared flags, keyed by cell_form_id (value True=cleared).
    _cleared_cells: dict[int, bool] = field(default_factory=dict)
    # v24: relationship ranks, keyed by (actor_a_form_id, actor_b_form_id).
    _relationships: dict[tuple[int, int], RelationshipState] = field(
    ...
    def record_faction_rank(
        self, faction_form_id: int, rank: int, by_peer_id: str, now_ms: float
    ) -> Optional[FactionRankState]:
        """Apply a PC SetFactionRank to authoritative state.

        Returns updated FactionRankState, or None if faction_form_id is 0
        or rank is outside i32 (defensive — wire already carries i32).
        Last-write-wins.
        """
        if faction_form_id == 0:
            return None
        if not (-0x80000000 <= rank <= 0x7FFFFFFF):
            return None
        f = self._factions.get(faction_form_id)
        if f is None:
            f = FactionRankState(faction_form_id=faction_form_id)
            self._factions[faction_form_id] = f
        f.rank = rank
        f.last_owner_peer_id = by_peer_id
        f.last_update_ms = now_ms
        return f

    def faction_rank(self, faction_form_id: int) -> Optional[FactionRankState]:
        return self._factions.get(faction_form_id) if faction_form_id else None

    def all_faction_ranks(self) -> list[FactionRankState]:
        return list(self._factions.values())

    # ---------------------------------------------------------- weather (B6.11)

    def record_weather(
        self, weather_form_id: int, by_peer_id: str, now_ms: float
    ) -> bool:
        """Last-write-wins weather form. Returns False if form_id is 0."""
        if weather_form_id == 0:
            return False
        self.weather_form_id = weather_form_id
        self.weather_last_peer_id = by_peer_id
        self.weather_last_update_ms = now_ms
        return True
    ...
    def record_companion(
        ...
        by_peer_id: str,
        now_ms: float,
    ) -> Optional[CompanionState]:
        if actor_form_id == 0:
            return None
        c = self._companions.get(actor_form_id)
        if c is None:
            c = CompanionState(actor_form_id=actor_form_id)
            self._companions[actor_form_id] = c
        c.teammate = teammate
        c.can_do_favor = can_do_favor
        c.last_owner_peer_id = by_peer_id
        c.last_update_ms = now_ms
        return c

    def all_companions(self) -> list[CompanionState]:
        return list(self._companions.values())

    # ---------------------------------------------------------- cell cleared (B6.9)

    def record_cell_cleared(
        self, cell_form_id: int, cleared: bool, by_peer_id: str, now_ms: float
    ) -> bool:
        if cell_form_id == 0:
            return False
        self._cleared_cells[cell_form_id] = cleared
        return True

    def all_cleared_cells(self) -> list[tuple[int, bool]]:
        return list(self._cleared_cells.items())
    ...
    def record_relationship(
        ...
        if actor_a_form_id == 0 or actor_b_form_id == 0:
            return None
        key = (actor_a_form_id, actor_b_form_id)
        r = self._relationships.get(key)
        if r is None:
            r = RelationshipState(
                actor_a_form_id=actor_a_form_id,
                actor_b_form_id=actor_b_form_id)
            self._relationships[key] = r
        r.rank = rank
        r.last_owner_peer_id = by_peer_id
        r.last_update_ms = now_ms
        return r

    def all_relationships(self) -> list[RelationshipState]:
