...
class ServerProtocol(asyncio.DatagramProtocol):
    ...
    def _dispatch(self, session: PeerSession, mtype: int, payload, now_ms: float) -> None:
        ...
        if mtype == MessageType.GLOBAL_VAR_SET:
            ...
            return
        if mtype == MessageType.FACTION_RANK_SET:
            self._handle_faction_rank_set(session, payload, now_ms)
            return
        if mtype == MessageType.XP_GRANT_SET:
            self._handle_xp_grant_set(session, payload, now_ms)
            return
        if mtype == MessageType.PARTY_WARP:
            self._handle_party_warp(session, payload, now_ms)
            return
        if mtype == MessageType.WEATHER_SET:
            self._handle_weather_set(session, payload, now_ms)
            return
        if mtype == MessageType.COMPANION_SET:
            self._handle_companion_set(session, payload, now_ms)
            return
        if mtype == MessageType.CELL_CLEARED_SET:
            self._handle_cell_cleared_set(session, payload, now_ms)
            return
        if mtype == MessageType.RELATIONSHIP_SET:
            self._handle_relationship_set(session, payload, now_ms)
            return
        if mtype == MessageType.TIME_PASS_SET:
            self._handle_time_pass_set(session, payload, now_ms)
            return
        if mtype == MessageType.ITEM_GRANT_SET:
            self._handle_item_grant_set(session, payload, now_ms)
            return
    ...
    def _handle_hello_initial(self, addr: tuple[str, int], payload, now_ms: float) -> None:
        ...
        # Bootstrap: PC faction ranks (B6.7 / Tier B1, chunked)
        self._send_faction_state_bootstrap(session, now_ms)
        # Bootstrap: weather + companions (v21)
        self._send_weather_state_bootstrap(session, now_ms)
        self._send_companion_state_bootstrap(session, now_ms)
        self._send_cell_cleared_bootstrap(session, now_ms)
        self._send_relationship_bootstrap(session, now_ms)
    ...
    def _handle_faction_rank_set(
        self, session: PeerSession, payload, now_ms: float
    ) -> None:
        if not isinstance(payload, FactionRankSetPayload):
            return
        if payload.faction_form_id == 0:
            self._counters["rejections"] += 1
            log.debug("reject FACTION_RANK_SET from %s: faction_form_id=0",
                      session.peer_id)
            return

        updated = self.state.record_faction_rank(
            payload.faction_form_id, payload.rank,
            session.peer_id, now_ms,
        )
        if updated is None:
            self._counters["rejections"] += 1
            log.debug("reject FACTION_RANK_SET from %s: invalid "
                      "(form=0x%X rank=%d)",
                      session.peer_id, payload.faction_form_id, payload.rank)
            return
        session.total_events += 1

        log.info("faction 0x%X rank -> %d by %s",
                 payload.faction_form_id, payload.rank, session.peer_id)

        broadcast = FactionRankBroadcastPayload(
            peer_id=session.peer_id,
            faction_form_id=payload.faction_form_id,
            rank=payload.rank,
            timestamp_ms=payload.timestamp_ms,
        )
        for other in self.state.other_sessions(session.addr):
            raw = other.channel.send_reliable(
                MessageType.FACTION_RANK_BCAST, broadcast, now_ms)
            self._send(other.addr, raw)

    def _handle_xp_grant_set(
        self, session: PeerSession, payload, now_ms: float
    ) -> None:
        """A4: fan-out XP grants. No server persistence — personal levels."""
        if not isinstance(payload, XpGrantSetPayload):
            return
        if payload.amount == 0:
            return
        # Cap absurd values (anti-cheat light).
        if abs(payload.amount) > 1_000_000:
            self._counters["rejections"] += 1
            log.debug("reject XP_GRANT_SET from %s: amount=%d",
                      session.peer_id, payload.amount)
            return
        session.total_events += 1
        log.info("xp grant %d by %s", payload.amount, session.peer_id)
        broadcast = XpGrantBroadcastPayload(
            peer_id=session.peer_id,
        ...
        for other in self.state.other_sessions(session.addr):
            raw = other.channel.send_reliable(
                MessageType.XP_GRANT_BCAST, broadcast, now_ms)
            self._send(other.addr, raw)

    def _handle_party_warp(
        self, session: PeerSession, payload, now_ms: float
    ) -> None:
        """A2: summon party — relay pos to other peers (no persistence)."""
        if not isinstance(payload, PartyWarpPayload):
            return
        session.total_events += 1
        log.info("party warp from %s -> (%.1f,%.1f,%.1f) cell=0x%X",
                 session.peer_id, payload.x, payload.y, payload.z,
                 payload.cell_id)
        broadcast = PartyWarpBroadcastPayload(
            peer_id=session.peer_id,
            x=payload.x, y=payload.y, z=payload.z,
        ...
        for other in self.state.other_sessions(session.addr):
            raw = other.channel.send_reliable(
                MessageType.PARTY_WARP_BCAST, broadcast, now_ms)
            self._send(other.addr, raw)

    def _handle_weather_set(
        self, session: PeerSession, payload, now_ms: float
    ) -> None:
        if not isinstance(payload, WeatherSetPayload):
            return
        if not self.state.record_weather(
                payload.weather_form_id, session.peer_id, now_ms):
            self._counters["rejections"] += 1
            return
        session.total_events += 1
        log.info("weather 0x%X by %s", payload.weather_form_id, session.peer_id)
        broadcast = WeatherBroadcastPayload(
            peer_id=session.peer_id,
            weather_form_id=payload.weather_form_id,
            timestamp_ms=payload.timestamp_ms,
        )
        for other in self.state.other_sessions(session.addr):
            raw = other.channel.send_reliable(
                MessageType.WEATHER_BCAST, broadcast, now_ms)
            self._send(other.addr, raw)

    def _handle_companion_set(
        self, session: PeerSession, payload, now_ms: float
    ) -> None:
        if not isinstance(payload, CompanionSetPayload):
            return
        updated = self.state.record_companion(
            payload.actor_form_id,
            bool(payload.teammate),
            bool(payload.can_do_favor),
            session.peer_id,
            now_ms,
        )
        if updated is None:
            ...
            return
        session.total_events += 1
        log.info("companion 0x%X teammate=%d favor=%d by %s",
                 payload.actor_form_id, payload.teammate, payload.can_do_favor,
                 session.peer_id)
        broadcast = CompanionBroadcastPayload(
            peer_id=session.peer_id,
            actor_form_id=payload.actor_form_id,
            teammate=payload.teammate,
            can_do_favor=payload.can_do_favor,
            timestamp_ms=payload.timestamp_ms,
        )
        for other in self.state.other_sessions(session.addr):
            raw = other.channel.send_reliable(
                MessageType.COMPANION_BCAST, broadcast, now_ms)
            self._send(other.addr, raw)

    def _send_weather_state_bootstrap(
        self, session: PeerSession, now_ms: float
    ) -> None:
        if not self.state.weather_form_id:
            return
        payload = WeatherStateBootPayload(
            weather_form_id=self.state.weather_form_id,
            timestamp_ms=int(self.state.weather_last_update_ms),
        )
        raw = session.channel.send_reliable(
            MessageType.WEATHER_STATE_BOOT, payload, now_ms)
        self._send(session.addr, raw)
        log.info("weather bootstrap %s: form=0x%X",
                 session.peer_id, self.state.weather_form_id)

    def _send_companion_state_bootstrap(
        self, session: PeerSession, now_ms: float
    ) -> None:
        comps = self.state.all_companions()
        if not comps:
            return
        entries = tuple(
            CompanionStateEntry(
                actor_form_id=c.actor_form_id,
                teammate=1 if c.teammate else 0,
                can_do_favor=1 if c.can_do_favor else 0,
            )
            for c in comps
        )
        max_per = CompanionStateBootPayload.MAX_ENTRIES_PER_FRAME
        total_chunks = (len(entries) + max_per - 1) // max_per
        log.info("companion bootstrap %s: %d entries in %d chunk(s)",
                 session.peer_id, len(entries), total_chunks)
        for i in range(total_chunks):
            chunk = entries[i * max_per : (i + 1) * max_per]
            payload = CompanionStateBootPayload(
                entries=chunk, chunk_index=i, total_chunks=total_chunks)
            raw = session.channel.send_reliable(
                MessageType.COMPANION_STATE_BOOT, payload, now_ms)
            self._send(session.addr, raw)

    def _handle_cell_cleared_set(
        self, session: PeerSession, payload, now_ms: float
    ) -> None:
        if not isinstance(payload, CellClearedSetPayload):
            return
        cleared = bool(payload.cleared)
        if not self.state.record_cell_cleared(
                payload.cell_form_id, cleared, session.peer_id, now_ms):
            self._counters["rejections"] += 1
            return
        session.total_events += 1
        log.info("cell 0x%X cleared=%d by %s",
                 payload.cell_form_id, int(cleared), session.peer_id)
        broadcast = CellClearedBroadcastPayload(
            peer_id=session.peer_id,
            cell_form_id=payload.cell_form_id,
            cleared=1 if cleared else 0,
            timestamp_ms=payload.timestamp_ms,
        )
        for other in self.state.other_sessions(session.addr):
            raw = other.channel.send_reliable(
                MessageType.CELL_CLEARED_BCAST, broadcast, now_ms)
            self._send(other.addr, raw)

    def _send_cell_cleared_bootstrap(
        self, session: PeerSession, now_ms: float
    ) -> None:
        cells = self.state.all_cleared_cells()
        if not cells:
            return
        entries = tuple(
            CellClearedStateEntry(cell_form_id=cid, cleared=1 if cl else 0)
            for cid, cl in cells
        )
        max_per = CellClearedBootPayload.MAX_ENTRIES_PER_FRAME
        total_chunks = (len(entries) + max_per - 1) // max_per
        log.info("cell-cleared bootstrap %s: %d entries in %d chunk(s)",
                 session.peer_id, len(entries), total_chunks)
        for i in range(total_chunks):
            chunk = entries[i * max_per : (i + 1) * max_per]
            payload = CellClearedBootPayload(
                entries=chunk, chunk_index=i, total_chunks=total_chunks)
            raw = session.channel.send_reliable(
                MessageType.CELL_CLEARED_BOOT, payload, now_ms)
            self._send(session.addr, raw)

    def _handle_relationship_set(
        self, session: PeerSession, payload, now_ms: float
    ) -> None:
        if not isinstance(payload, RelationshipSetPayload):
            return
        if payload.actor_a_form_id == 0 or payload.actor_b_form_id == 0:
            self._counters["rejections"] += 1
            return
        session.total_events += 1
        self.state.record_relationship(
            payload.actor_a_form_id, payload.actor_b_form_id,
            payload.rank, session.peer_id, now_ms)
        log.info("relationship 0x%X->0x%X rank=%d by %s",
                 payload.actor_a_form_id, payload.actor_b_form_id,
                 payload.rank, session.peer_id)
        broadcast = RelationshipBroadcastPayload(
            peer_id=session.peer_id,
            actor_a_form_id=payload.actor_a_form_id,
            actor_b_form_id=payload.actor_b_form_id,
            rank=payload.rank,
            timestamp_ms=payload.timestamp_ms,
        )
        for other in self.state.other_sessions(session.addr):
            raw = other.channel.send_reliable(
                MessageType.RELATIONSHIP_BCAST, broadcast, now_ms)
            self._send(other.addr, raw)

    def _send_relationship_bootstrap(
        self, session: PeerSession, now_ms: float
    ) -> None:
        rels = self.state.all_relationships()
        if not rels:
            return
        entries = tuple(
            RelationshipStateEntry(
                actor_a_form_id=r.actor_a_form_id,
                actor_b_form_id=r.actor_b_form_id,
                rank=r.rank)
            for r in rels
        )
        max_per = RelationshipBootPayload.MAX_ENTRIES_PER_FRAME
        total_chunks = (len(entries) + max_per - 1) // max_per
        log.info("relationship bootstrap %s: %d entries in %d chunk(s)",
                 session.peer_id, len(entries), total_chunks)
        for i in range(total_chunks):
            chunk = entries[i * max_per : (i + 1) * max_per]
            payload = RelationshipBootPayload(
                entries=chunk, chunk_index=i, total_chunks=total_chunks)
            raw = session.channel.send_reliable(
                MessageType.RELATIONSHIP_STATE_BOOT, payload, now_ms)
            self._send(session.addr, raw)

    def _handle_time_pass_set(
        self, session: PeerSession, payload, now_ms: float
    ) -> None:
        """Fan-out Wait/Sleep time jumps so peers advance the same hours."""
        if not isinstance(payload, TimePassSetPayload):
            return
        if payload.hours == 0 or abs(payload.hours) > 720:  # 30 days cap
            self._counters["rejections"] += 1
            return
        session.total_events += 1
        log.info("time pass %dh by %s", payload.hours, session.peer_id)
        broadcast = TimePassBroadcastPayload(
            peer_id=session.peer_id,
            hours=payload.hours,
            timestamp_ms=payload.timestamp_ms,
        )
        for other in self.state.other_sessions(session.addr):
            raw = other.channel.send_reliable(
                MessageType.TIME_PASS_BCAST, broadcast, now_ms)
            self._send(other.addr, raw)

    def _handle_item_grant_set(
        self, session: PeerSession, payload, now_ms: float
    ) -> None:
        """Fan-out silent PC AddItem (quest rewards / script grants)."""
        if not isinstance(payload, ItemGrantSetPayload):
            return
        if payload.item_form_id == 0 or payload.item_form_id == 0xF:
            self._counters["rejections"] += 1
            return
        if payload.count <= 0 or payload.count > 99:
            self._counters["rejections"] += 1
            return
        session.total_events += 1
        log.info("item grant form=0x%X count=%d silent=%d by %s",
                 payload.item_form_id, payload.count, payload.silent,
                 session.peer_id)
        broadcast = ItemGrantBroadcastPayload(
            peer_id=session.peer_id,
        ...
        for other in self.state.other_sessions(session.addr):
            raw = other.channel.send_reliable(
                MessageType.ITEM_GRANT_BCAST, broadcast, now_ms)
            self._send(other.addr, raw)
    ...
    def _send_faction_state_bootstrap(
        self, session: PeerSession, now_ms: float
    ) -> None:
        factions = self.state.all_faction_ranks()
        if not factions:
            log.debug("no faction ranks to bootstrap for %s", session.peer_id)
            return
        entries = tuple(
            FactionRankStateEntry(faction_form_id=f.faction_form_id, rank=f.rank)
            for f in factions
        )
        max_per = FactionStateBootPayload.MAX_ENTRIES_PER_FRAME
        total_chunks = (len(entries) + max_per - 1) // max_per
        log.info("faction-state bootstrap %s: %d entries in %d chunk(s)",
                 session.peer_id, len(entries), total_chunks)
        for i in range(total_chunks):
            chunk = entries[i * max_per : (i + 1) * max_per]
            payload = FactionStateBootPayload(
                entries=chunk, chunk_index=i, total_chunks=total_chunks)
            raw = session.channel.send_reliable(
                MessageType.FACTION_STATE_BOOT, payload, now_ms)
            self._send(session.addr, raw)
