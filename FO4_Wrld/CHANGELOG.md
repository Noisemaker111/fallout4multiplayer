...
## Steam invites over Spacewar — M1 (2026-08-03)

Two friends on two different home networks can now play with a Steam invite
and nothing else. **No LAN IP, no `fw_config.ini` editing, no port
forwarding.** Protocol stays **v25** — not one wire byte changed.

### Shape

`FoM.exe` becomes the Steam shell for the session: it holds a **Spacewar
(AppID 480)** lobby, receives invites, and runs a **Steam P2P ⇄ loopback UDP
tunnel** for as long as the game is up. Fallout 4 and `net/server/main.py`
are untouched — both sides now always use `server = 127.0.0.1:31337` and FoM
moves the datagrams across the internet.

- **Host:** one dedicated loopback socket **per remote peer**, so the server's
  addr-keyed session table keeps seeing N distinct peers (required — a single
  shared socket would collapse everyone into one session)
- **Join:** binds `127.0.0.1:31337` and impersonates the server locally
- The bridge forwards frames **opaquely**; a protocol bump needs no change in
  `fw_launcher`

Chose Steam-under-the-protocol (Option A) over Steam-for-matchmaking-only
(Option B): endpoint exchange still loses to symmetric NAT, and Steam already
runs a relay. Chose `ISteamNetworkingMessages` over `ISteamNetworkingSockets`
— connectionless send-to-a-SteamID is the same shape as the socket it
replaces, and we already have our own reliability layer.

Steam lives in **FoM, not `dxgi.dll`**: Fallout 4 already runs its own
SteamAPI session under its own AppID, and a second init inside the injected
DLL is a fight with no upside. Cost is one loopback hop on the joiner.

### No vendored Steamworks SDK

`steam_api64.dll` exports the flat C API, so all ~25 calls bind via
`GetProcAddress` — the same technique `fw_native/src/steam/steam_id.cpp`
already used, extended to matchmaking + networking. Callbacks go through
`SteamAPI_ManualDispatch_*` (Valve's path for non-C++ bindings).
**`SteamAPI_RunCallbacks()` must never be called in the FoM process.**

Needs SDK **1.47+**. Fallout 4's own 2015-vintage DLL is detected and
rejected; the pack ships its own
(`tools/player_setup/Get-SteamworksRuntime.ps1`).

### Identity

`client_id` now derives from SteamID64 as `"s" + base36`
(`76561198090749127` → `skxuogunoclj`, 12 chars vs the 15-char cap). Kills
the "both friends shipped as `player_A` → `peer_id_taken`" collision.

### The flow

| Host | Friend |
|------|--------|
| Run FoM, press **1**. The Steam friend picker opens by itself. | Click **Accept**. Fallout 4 opens. |

The friend does not open FoM first, does not press anything in it, and never
types an IP. They can also join with no invite at all: right-click the host
in the Steam friends list → **Join Game**.

**Resident agent.** Steam only delivers invites to a process already running
as the AppID, and for 480 it would otherwise launch Valve's Spacewar. So FoM
keeps a windowless helper parked in Steam's callback queue; on an invite it
runs the whole join itself (lobby → version check → tunnel → `fw_config.ini`
→ launch FO4), shows its window, and returns to standby when the game exits.
One `HKCU\...\Run` entry named `FalloutWorld (FoM)`, registered after the
first successful Steam session, announced out loud, removable via `[5]` or
`--quit-agent`.

**Rich presence.** Host publishes `connect` = `+connect_lobby <id>`, which is
what makes the friends-list *Join Game* button appear.
`GameRichPresenceJoinRequested_t` (337) and `GameLobbyJoinRequested_t` (333)
land in the same place.

**Single instance.** Two processes claiming AppID 480 is a coin flip over
which gets the invite, so a named mutex makes the agent the only Steam
session and double-clicking `FoM.exe` hands its command over a named pipe.
The pipe server runs on its own thread and answers `Busy` rather than
queueing behind a session — and all client I/O is overlapped with an explicit
timeout, because `CallNamedPipe`'s timeout covers only waiting for a free
instance and its reply read blocks forever (a wedged agent would otherwise
hang every `FoM.exe` the player double-clicks).

### UX

`[1] Host session` / `[2] Join a friend` / `[3] LAN only` /
`[4] Install files only` / `[5] Steam invites: ON|OFF`. A base36 **session
code** is the fallback when the overlay misbehaves. LAN path unchanged and
still used automatically when `SteamAPI_Init` fails. FoM exits ~5s after
Fallout 4 does, so nobody is left with an orphan tunnel.

New flags: `--agent`, `--quit-agent`, `--no-agent`, `--steam-check`.

### Also

- `fw_native/src/net/transport.h` — `ITransport` seam; `client.cpp` no longer
  touches a socket directly, so a future in-process `SteamTransport` is a new
  file plus one line
- `fw_native/src/net/protocol_version.h` — single source of truth for the wire
  version, now shared by `protocol.h` and the launcher (which advertises it as
  lobby data so a mismatched joiner is rejected **before** FO4 launches)
- `FoM.exe --steam-check` — full Steam bring-up diagnostic

### Honest caveats

The overlay says **Spacewar**, because it is. Players must own Spacewar (free;
run once from the library if `CreateLobby` fails). A background helper has to
exist for cold invites to work at all — that is a consequence of not owning
the AppID, not a design preference. FoM.exe is held open while the helper
runs, so `--quit-agent` before replacing it. 480 is a development carrier,
not a shipping AppID.

Full design + limitations: `docs/STEAM_SPACEWAR.md`.

Tests: `fw_launcher/tests/bridge_test.cpp` (19 checks — byte-identical round
trip, per-peer source ports, oversize drop, stranger rejection),
`net/tests/test_protocol_version_lockstep.py`, 401 pytest green.

---

## v25 silent PC AddItem grants + companion party pull (2026-08-03)

### ITEM_GRANT_SET / BCAST (0x04A1 / 0x04A2)

- Observe Papyrus `ObjectReference.AddItem` @ **`0x1402B40`** (reg LEA r9)
- Emit when: self is local PC, count 1..99, item ≠ caps `0xF`, and either
  **abSilent==true** OR formType **KEYM (0x2F)**
- Apply: engine AddItem on local PC under `ApplyingRemoteGuard`
- Dedup: single-count grants skipped if PC already holds the form (SetStage script overlap)

Policy: open-world loot UI uses silent=false → not mirrored (personal loot / one-shot world disable stay separate). Keys always grant so co-op doors stay openable.

Tests: `net/tests/test_item_grant.py`.

### Companion party-teleport pull

Local teammate formIDs tracked on `SetPlayerTeammate`. After F8 / `PARTY_WARP`
PC teleport, known companions are `MoveTo`'d near the player (best-effort ring
offsets). Continuous companion AI ownership remains XL.

---

## v23/v24 relationship + Wait/Sleep + MINIMAL economy (2026-08-03)

### RELATIONSHIP_SET / BCAST (0x0481 / 0x0482) + BOOT (0x0028, v24)

- Observe Papyrus `Actor.SetRelationshipRank` @ `0x138B150`
- Server last-write-wins state + fan-out + **late-join RELATIONSHIP_STATE_BOOT**
- Apply via papyrus native under `ApplyingRemoteGuard`

### TIME_PASS_SET / BCAST (0x0491 / 0x0492)

- Observe `Game.PassTime` @ `0x13C1B70` (Wait/Sleep funnel)
- Server rejects `hours == 0` or `|hours| > 720`
- Apply bumps `GameHour` (`0x38`) + `GameDaysPassed` (`0x39`) directly

### MINIMAL co-op economy kept on

container / put / lock / kill / pickup / equip / door remain compiled under
`FW_MINIMAL` (see `minimal_exclude.txt` — combat AI tier still out).

### Docs / tests

- `docs/COOP_1TO1_GAP.md` — honest 1:1 inventory
- `net/tests/test_relationship_timepass.py` — 10 tests

---

## B6.2 lights/toggles + B6.9 cell-cleared (2026-08-03) — protocol v22

### B6.2 — lights / world toggles

Activate worker form filter expanded beyond doors:

- Keep door/ACTI set (`0x1F/20/24/29`)
- Add candidates `0x21`, `0x28` (light-adjacent / furniture interactables)

Same **DOOR_OP** wire (toggle re-Activate). Log tag: `door/light/toggle`.

### B6.9 — cell cleared

| Piece | Detail |
|-------|--------|
| Flag | `TESObjectCELL+0x134` (u8) — from IsCleared leaf |
| Observe | Papyrus `SetCleared` `0x13AB820` |
| Apply | worker `0x37F130` |
| Poll | every 3s on current cell; emit on **cleared=true** edge |
| Wire | `CELL_CLEARED_SET/BCAST 0x0471/72`, boot `0x0027` |

Tests: `net/tests/test_cell_cleared.py`.

---

## B6.10 one-shot loot + B6.0 doors + B3 cell-travel follow (2026-08-03)

### B6.10 — world pickup under FW_MINIMAL

`pickup_hook` (`INV_ADD_FROM_WORLD` `0x500430`) was already shipping
`ACTOR_EVENT DISABLE` so peers disable the world REFR. It was compiled out of
MINIMAL. **Now enabled under MINIMAL** so bobbleheads / magazines / floor loot
cannot be double-picked in the classic co-op track.

### B6.0 — doors under FW_MINIMAL

`door_hook` (Activate worker) similarly kept for open/close sync during
MINIMAL play.

### B3 — load-door / cell-travel party follow

When local `parentCell.formID` changes (load door, fast travel, coc), after a
**2.5 s debounce** the client auto-sends **PARTY_WARP** to the new position so
peers follow (same path as F9 summon). Log: `[pos] CELL_TRAVEL … party follow`.

Toggle: `kAutoPartyFollowOnCellChange` in `player_pos_hook.cpp` (default true).

B6.9 cell-cleared flag (engine extra-data) still pending RE.

---

## B6.11 weather + B6.8 companion (2026-08-03) — protocol v21

### Weather

| Piece | Detail |
|-------|--------|
| Poll | Sky getter `0x128CB0`, current weather `Sky+0x48`, every 5 s |
| Observe | `Weather.SetActive` Papyrus `0x144F950` |
| Apply | worker `0x651A60(sky, weather, override=1, accelerate=1)` |
| Wire | `WEATHER_SET/BCAST 0x0451/52`, boot `0x0025` |

### Companion (recruit/dismiss flag)

| Piece | Detail |
|-------|--------|
| Observe | `Actor.SetPlayerTeammate` `0x1391550` |
| Apply | worker `0xDA7170` |
| Wire | `COMPANION_SET/BCAST 0x0461/62`, boot `0x0026` chunked |

Definition of done (partial): both peers share active weather form within a
few seconds; recruit via SetPlayerTeammate is mirrored (AI ownership of
companion path is still N-branch / later).

Tests: `net/tests/test_weather_companion.py`.

---

## Sprint: A2 party teleport + A4 proximity XP + B6.11 GameHour (2026-08-03)

Protocol **v20**. Three co-op milestones in one pass.

### A2 — Party travel

| Key | Action |
|-----|--------|
| **F8** | Teleport **me** to primary remote peer last POS (client-local) |
| **F9** | **Summon party** — `PARTY_WARP` → peers `PARTY_WARP_BCAST` → MoveTo me |

- Engine: `player_teleport_to` → Actor::MoveTo with **skip_player_check=0**,
  doProcessUpdate=1 (real PC cell/camera path).
- Main-thread queue `FW_MSG_PARTY_TELEPORT`.
- Module: `hooks/party_ux_hook` (kept under FW_MINIMAL).

### A4 — Proximity XP

| Opcode | Role |
|--------|------|
| `XP_GRANT_SET` `0x0431` | C→S after local `Game.RewardPlayerXP` |
| `XP_GRANT_BCAST` `0x0432` | S→peers; apply only if dist &lt; **4096** u |

- Observe: Papyrus `RewardPlayerXP` **`0x13C1EA0`**
- Apply: same native under `ApplyingRemoteGuard` (no echo)
- Radius: `offsets::PROXIMITY_XP_RADIUS_UNITS` (tune later)

### B6.11 — Time of day (partial)

- Poll **GameHour** TESGlobal form **`0x38`** every 2 s from pos thread
- Pushes via existing **GLOBAL_VAR_SET** path (no new opcodes)
- Weather form / ForceWeather **not** in this slice

### Tests

`net/tests/test_party_xp_sync.py` — wire + SET→BCAST for XP and party warp.

### Hotkeys (live)

```
F8  →  fw_native.log: [party_ux] F8: teleport → peer=...
F9  →  server: party warp from ... ; peer: PARTY_WARP_BCAST + player_teleport_to
XP  →  [worldstate] Game.RewardPlayerXP ; peer in range: XP_GRANT_BCAST apply
```

---

## B6.7 player faction rank (2026-08-03) — Tier B1 partial

First shippable slice of Tier B “story doesn’t break”: **shared PC faction
membership/rank** (not full dialogue trees).

### Protocol v19

| Opcode | Direction | Payload |
|--------|-----------|---------|
| `FACTION_RANK_SET` `0x0421` | C→S | `<IiQ` faction_form_id, rank i32, ts |
| `FACTION_RANK_BCAST` `0x0422` | S→peers | FixedClientId + same |
| `FACTION_STATE_BOOT` `0x0024` | S→joiner | ChunkHeader + `<Ii` entries |

`PROTOCOL_VERSION` **18 → 19**. Server last-write-wins per faction form id;
bootstrap on join.
...
| Piece | RVA / path |
|-------|------------|
| Observe | Papyrus `Actor.SetFactionRank` **`0x138AC30`** (local PC only) |
| Apply worker | **`0xDA3AC0`** `(Actor*, TESFaction*, int rank)` on local PC |
| Dispatch | `FW_MSG_FACTION_RANK_APPLY` + pre-hwnd flush |

Filtered to `get_local_player()` so NPC faction churn does not flood the wire.

### Tests

`net/tests/test_faction_sync.py` — state, wire, SET→BCAST, bootstrap, reject
zero form. **150** protocol/quest/faction tests green with full suite slice.

### Not in this slice

- `RemoveFromFaction` observe (scripts that only remove without re-set)
- Dialogue topic / forcegreet replication
- Companion recruit (Tier B2)

---

## B4 QuestStage end-to-end (2026-08-03) — Tier A3

Closes ROADMAP **3.1** / product Tier **A3**: when peer A advances a quest
stage, peer B applies the same stage; join bootstrap restores server-held
stages. GlobalVar path corrected for classic at the same time.

### Classic 1.10.163 RVAs (re-derived)

Prior `PAPYRUS_*` constants pointed mid-function / wrong (stale next-gen pass).
Re-found via Papyrus native registration LEA scan on
`Fallout4.exe.unpacked.exe`:

| Symbol | Old (wrong) | New (1.10.163) |
|--------|-------------|----------------|
| `PAPYRUS_GLOBALVAR_SETVALUE_RVA` | `0x11459E0` | **`0x13F0A40`** |
| `PAPYRUS_QUEST_SETSTAGE_RVA` | `0x1185DD0` | **`0x1449550`** |

Also documented `TESQUEST_CURRENT_STAGE_OFF = 0x2B4` (u16, written by stage
helper `0x5D71E0`).

### Native client

- **Observe:** `worldstate_hook` detours SetValue + SetCurrentStageID; emits
  `GLOBAL_VAR_SET` / `QUEST_STAGE_SET` (skips when `tls_applying_remote`).
- **Apply GlobalVar:** unchanged direct write `TESGlobal+0x30` (any thread).
- **Apply QuestStage:** `engine::apply_quest_stage` → Papyrus native; **main
  thread only** (TLS-affine workers) via `FW_MSG_QUEST_STAGE_APPLY` queue +
  pre-hwnd flush (same race class as lock bootstrap).
- **Bootstrap:** `QUEST_STATE_BOOT` chunks decoded → same apply queue.
- **MINIMAL:** `worldstate_hook` kept in the FW_MINIMAL build (Tier A3 needs
  it); removed from `minimal_exclude.txt`.

### Server / protocol

Unchanged — already had SET/BCAST/BOOT + last-write-wins
`record_quest_stage` + tests (`136` protocol/quest tests green).

### Live smoke (operator)

1. Server first; deploy minimal `dxgi.dll`; two FO4 clients load same campaign.
2. A advances any early quest stage → `fw_native.log` on A:
   `[worldstate] Quest.SetCurrentStageID form=… stage=…`
3. Server log: `quest … set stage …`
4. B log: `QUEST_STAGE_BCAST` + `engine: apply_quest_stage …`
5. Late join: B receives `QUEST_STATE_BOOT` and applies recorded stages.
